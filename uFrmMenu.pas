...
type
  TFrmMenu = class(TForm)
    ...
    procedure IPBT1Click(Sender: TObject);
...
uses uFrmAgendaCompromissos, uFrmGrupo, uFrmSubGrupo, uFrmConfiguracoes,
  ...
  uFrmLivroLMC, uFrmContas, uFrmCadBancos, uFrmGerarFaturas, uFrmMenuNFe,
  uFrmGeraNFe;
...
const
  Msg = 'Faltam %d dias para a aplicação ser bloqueada. Deseja liberar agora?';
var
  Dias: Integer;
begin
// faça a propriedade Active igual a False, mesmo que já esteja assim no Object Inspector

  EvAppProtect1.Active := False;

  // escolha um número para ser o Identificador Único da aplicação

  EvAppProtect1.IdApplication := 31415927;

   // se sua aplicação NÃO possuir auto-liberação, zere as propriedades

  EvAppProtect1.EvalDays := 0;

  EvAppProtect1.EvalLoads := 0;

 // escolha um local para salvar as informações no registro

  EvAppProtect1.RegKey := '\SOFTWARE\AretiAutomacao\Sisconet\';
  EvAppProtect1.SaveType :=stFile;

// finalmente, ative o componente.

  EvAppProtect1.Active := True;


 //avisar o vencimento
 Dias := Round(EvAppProtect1.DateEnd - Date);
  if (Dias <= 5) and (MessageDlg(Format(Msg, [Dias]), mtConfirmation, mbYesNoCancel, 0) = mrYes) then
    EvAppProtect1.ResetLockApplication;
 //fim-avisar o vencimento
...
procedure TFrmMenu.IPBT1Click(Sender: TObject);
begin
