...
uses
  ...
  System.Actions, uFrameBotaoHorizontal, frxClass, frxDBSet, udmProdutos ;
...
type
    ...
    procedure GriItensDrawColumnCell(Sender: TObject; const Rect: TRect;
...
uses uLibrary, uFrmClientes, uDmWorkCom, uFrmCadClassPedido,
  ...
  uFrmCadProdutos, uDMClientes, uFrmProdutoCores,
  uFrmProdutoTamanho;
...
procedure TFrmLancPedidos.GriItensDrawColumnCell(Sender: TObject;
...
begin
 ...
 { with GriItens do
  begin
    DmProdutos.Dados_Produto(DataItens.DataSet.FieldByName('IDPRODUTO').AsInteger) ;
    if DMprodutos.CdsProdutos.FieldByName('estoque').as <0  then
    begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          //acrescentar o CSOSN
         // DataItens.DataSet.FieldByName('CSOSN').AsString :=  DmProdutos.CdsProdutos.FieldByname('CSOSN_INTERNO').Value   ;
          if aDesconto < 0 then
          begin
//            DataItens.DataSet.FieldByName('ACRESCIMO').AsFloat     := abs(aDesconto);
//            DataItens.DataSet.FieldByName('DESCONTO').AsFloat     := 0;

            //alteracao rondado 02/05/2018
...
//            DataItens.DataSet.FieldByName('DESCONTO').AsFloat     := aDesconto;
        ...
        end;    //rondado fim da confirmação

         //rondado 02/05/2018 - forcar o valor do produto ficar o mesmo  - nao funciona
        {DmProdutos.CdsProdutos.Edit ;
        DmProdutos.CdsProdutos.FieldByName('PRECO_VENDA').AsCurrency :=ValorUnitAntigo;
        DmProdutos.CdsProdutos.ApplyUpdates(0);
        DMPRODUTOS.CdsProdutos.Refresh;
        DMProdutos.CdsProdutos.Close;    }
...
begin
  inherited;
  // alteracao rondado 10/05/2018 remover o if do dav impresso
...
begin
  ...
  //MODIFICADA PARA INCLUIR ESTADUAL E MUNICIPA
  ValorCarga := ((ValorTotalItem * DmProdutos.CdsNCM.FieldByName('ALIQ_NACIONAL').AsFloat) / 100)+
  ((ValorTotalItem * DmProdutos.CdsNCM.FieldByName('ALIQ_ESTADUAL').AsFloat) / 100)+
  ((ValorTotalItem * DmProdutos.CdsNCM.FieldByName('ALIQ_MUNICIPAL').AsFloat) / 100);
  // grava no pedido
  if not(DataWork.DataSet.State in [dsInsert,dsEdit]) then
     DataWork.DataSet.Edit;
