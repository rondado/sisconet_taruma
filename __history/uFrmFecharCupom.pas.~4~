...
begin
  ...
  begin
    MessageDlg('É necessário selecionar uma forma da pagamento vinculada a Cartão!', mtInformation, [mbOK], 0);
    Exit;
  end;
  //
  aCartoes := RetornaNumCartoesFechamento(DataCupom.DataSet.FieldByName('ID').AsInteger);
  ...
  begin
    sVincular := 'N';
    if Confirmacao('Deseja emitir documento vinculado para o pagamento?') then
       sVincular := 'S';
    sFormaPag := 2;
    ActIncluirExecute(Sender);
  end
