...
uses
  ...
  uFrameBotaoVertical, ComCtrls, ActnList, DB, Grids, DBGrids, Menus, IniFiles,
...
type
  TFrmCadProdutos = class(TFrmCadPadrao)
    ...
    procedure DbAliqssimplesKeyPress(Sender: TObject; var Key: Char);
  private
...
//fabricio fim
//fabricio: calcula preço de venda
...
procedure TFrmCadProdutos.DbAliqssimplesKeyPress(Sender: TObject;
  var Key: Char);
begin
  inherited;
  if key = #13 then
...
begin
  ...
  //RONDADO SE NÃO HOUVER EAN
  //if DbEan13.Text ='' then
 /// begin
   // DbEan13.Text := DbID.Text;
 // end;
