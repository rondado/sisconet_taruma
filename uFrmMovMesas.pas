...
uses
  ...
  Buttons, DBCtrls, Menus, ComCtrls, System.Actions, WinSpool,Inifiles, uTrocaCaracteresEspeciais;
...
uses uDmWorkCom, uDmProdutos, uLibrary, uFrmAbreRegistra, uFuncoesPAFECF,
  uFrmProdutos_multiplos, uFrmAbreRegistra_multiplos;
...
begin
  ...
  //Rondado deixar check box marcado
  checkbox1.Checked:=True;
  checkbox2.checked:=True ;
  checkbox3.checked:=True;
  //
 { if DMWorkCom.CdsEmpresaCNPJ_CPF.AsString ='16988039000107' then
  begin
  ...
  begin
     if DmProdutos.Dados_Produto(DataMesaItens.DataSet.FieldByName('ID_PRODUTO').AsInteger) then
      begin
           ...
           Memo1.Text := Memo1.Text +' ' + nmmesa + ':   ' + DateTimeToStr(Now) +#13#10 ;
           Memo1.Text := Memo1.Text + '-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*'+#13#10 ;
           Memo1.Text := Memo1.Text + '-------------------------------ITEM INDIVIDUAL------------------------------'+#13#10 ;
       ...
       { Memo1.Text:= Memo1.Text + #13#10 +
        ...
        Memo1.Text:= Memo1.Text + #13#10 + 'OBS.:  ';       }
          ...
          Memo1.Text := Memo1.Text + #13#10;
          Memo1.Text := Memo1.Text +  DataMesaItens.DataSet.FieldByName('ID_PRODUTO').AsString + ' - '+ DataMesaItens.DataSet.FieldByName('DESCRICAO').AsString +#13#10+
         'Quantidade: '+ FormatFloat('0.00',  DataMesaItens.DataSet.FieldByName('QUANT').AsFloat);

               if CheckBox2.Checked then
                // Memo1.Text := Memo1.Text + #13#10 + 'R$ UNIT: ' +FormatFloat('0.00', DataMesaItensAgrupa.DataSet.FieldByName('PRECO_UNIT').AsFloat);
                 Memo1.Text := Memo1.Text + '  ' + ' X  R$: ' +FormatFloat('0.00',  DataMesaItens.DataSet.FieldByName('PRECO_TOTAL').AsFloat/DataMesaItens.DataSet.FieldByName('QUANT').AsFloat);

               if CheckBox1.Checked then
                 //Memo1.Text := Memo1.Text + #13#10 + 'R$ SUBT: ' + FormatFloat('0.00', DataMesaItensAgrupa.DataSet.FieldByName('PRECO_TOTAL').AsFloat) +#13#10;
                Memo1.Text := Memo1.Text +  '  ' + ' = R$ ' + FormatFloat('0.00', DataMesaItens.DataSet.FieldByName('PRECO_TOTAL').AsFloat) +#13#10;
                Memo1.Text := Memo1.Text + '-----------------------------------------------------------------------------------------';
...
begin
  ...
  {*if DmWorkCom.CdsConfig.FieldByName('GRADE_MULTIPLA').AsString = 'S'   then
  begin
     Application.CreateForm(TFrmProdutos_multiplos,FrmProdutos_multiplos);
      try
      FrmProdutos_multiplos.sIDMesa := 0;
      FrmProdutos_multiplos.ShowModal;
      RecalculaTotalMesa;
      DataMesas.DataSet.Last;
     finally
      FreeAndNil(FrmProdutos_multiplos);
     end;
  end }

  if  MessageDlg('É venda de produto com ADICIONAIS (composto)?',mtConfirmation,[mbyes,mbno],0)=mryes  then
  ...
  begin
     {Application.CreateForm(TFrmProdutos_multiplos,FrmProdutos_multiplos);
      ...
      Application.CreateForm(TFrmAbreRegistra_multiplos,FrmAbreRegistra_multiplos);
      try
      FrmAbreRegistra_multiplos.sIDMesa := 0;
      FrmAbreRegistra_multiplos.ShowModal;
      RecalculaTotalMesa;
      DataMesas.DataSet.Last;
     finally
      FreeAndNil(FrmAbreRegistra_multiplos);
     end;
...
begin
  ...
  begin
    Application.CreateForm(TFrmAbreRegistra_Multiplos,FrmAbreRegistra_Multiplos);
    try
      FrmAbreRegistra_Multiplos.sIDMesa := DataMesasAbertas.DataSet.FieldByName('ID_MESA').AsInteger;
      FrmAbreRegistra_Multiplos.ShowModal;
      RecalculaTotalMesa;
      DataMesas.DataSet.Locate('ID',FrmAbreRegistra_Multiplos.sIDMesa,[]);
    finally
      FreeAndNil(FrmAbreRegistra_Multiplos);
    end;
  end
...
begin
  inherited;
    ...
    if dmWorkCom.PAFECF.gAgrupaVenda then
      ...
      DmWorkcom.QryMesasItensAgrupa.SQL.Add ('where mesas_itens.id_mesa = '+ intToSTR(sIDMesa));
     ...
     // DmWorkcom.QryMesasItensAgrupa.Params.ParamByName('pID').AsInteger := sIDMesa;

      DmWorkcom.QryMesasItensAgrupa.Open;
      //DmWorkcom.QryMesasItensAgrupa.Last;
      //DmWorkcom.QryMesasItensAgrupa.first ;
       if DataMesaItensAgrupa.DataSet.RecordCount > 0 then
      ...
      begin
         ...
         begin
           ...
           begin
             ...
             //rondado não pegar produto cancelado

             if DataMesaItens.DataSet.FieldByName('CANCELADO').AsString  ='N' then
             begin

             if DmProdutos.Dados_Produto(DataMesaItens.DataSet.FieldByName('ID_PRODUTO').AsInteger) then
             begin
               {Memo1.Text := Memo1.Text + 'Quantidade: '+ FormatFloat('0.00',
               DataMesaItens.DataSet.FieldByName('QUANT').AsFloat) + '  UN:' + DmProdutos.CdsProdutos.FieldByName('DESCRICAO').AsString;}
               Memo1.Text := Memo1.Text +   DmProdutos.CdsProdutos.FieldByName('DESCRICAO').AsString +#13#10+
               'Quantidade: '+ FormatFloat('0.00',  DataMesaItens.DataSet.FieldByName('QUANT').AsFloat);

               if CheckBox2.Checked then
                // Memo1.Text := Memo1.Text + #13#10 + 'R$ UNIT: ' +FormatFloat('0.00', DataMesaItens.DataSet.FieldByName('PRECO_UNIT').AsFloat);
                 Memo1.Text := Memo1.Text + '  ' + ' X  R$: ' +FormatFloat('0.00', DataMesaItens.DataSet.FieldByName('PRECO_UNIT').AsFloat);

               if CheckBox1.Checked then
                 //Memo1.Text := Memo1.Text + #13#10 + 'R$ SUBT: ' + FormatFloat('0.00', DataMesaItens.DataSet.FieldByName('PRECO_TOTAL').AsFloat) +#13#10;
                 Memo1.Text := Memo1.Text +  '  ' + ' = R$ ' + FormatFloat('0.00', DataMesaItens.DataSet.FieldByName('PRECO_TOTAL').AsFloat) +#13#10;
                 Memo1.Text := Memo1.Text + '-----------------------------------------------------------------------------------------';
