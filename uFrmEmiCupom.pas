...
uses
  ...
  jpeg, SqlExpr, ACBrDevice, OleCtrls, SHDocVw, DBClient, pcnConversao,
  unitVidaLink, System.Actions;
...
type
  TFrmEmiCupom = class(TFrmPadrao)
    ...
    procedure labFecharMesasClick(Sender: TObject);
  private
    { Private declarations }
    //Rondado - variavel criada para resolver duplicacao de EAN em caso de A Granel
    aux2 :String ;
...
Uses uDmWorkCom,uFuncoesPAFECF,uLibrary, StrUtils,uFrmFecharCupom,
  ...
  uDmAuxiliar, uDmPosto, ACBrDFeConfiguracoes, uFrmListaContigencia,
...
//    ActMenuFiscal.Enabled := false;
    ...
    //rondado liberação para emissão de pendentes 01/03/2017
...
//procedimento para incluir item
procedure TFrmEmiCupom.ActIncluirItemExecute(Sender: TObject);
var AuxCodPro, aQuant,aCodigo, aMensagem, pesogramas : String;
    aPosicao : Integer;
...
//           aCodigo := ZeroEsquerda(aCodigo,13);
       //alteração rondado
      //  if Length(aCodigo) <= 6  then //original verifica o tamanho
        if aCodigo = '' then   begin
           aProdOk := DmProdutos.Dados_Produto(StrToInt(aCodigo));
        end
        else   begin
           aProdOk := DmProdutos.Dados_Produto3(aCodigo, aux2) ;
           // aProdOk := DmProdutos.Dados_Produto(aux2);
           aux2 :='';
        end;
        if not(aProdOk) then
        begin
        ...
        begin
          ...
          begin
            ...
            try
              ...
              if not(DataCupom.DataSet.State in [dsInsert,dsEdit]) then
                 DataCupom.DataSet.Edit;
              DataCupom.DataSet.FieldByName('valor_carga_trib').AsFloat := DataCupom.DataSet.FieldByName('valor_carga_trib').AsFloat + ValorCargaFed +ValorCargaEst + ValorCargaMun;
              DataCupom.DataSet.FieldByName('VAL_CARGA_FED').AsFloat := DataCupom.DataSet.FieldByName('VAL_CARGA_FED').AsFloat + ValorCargaFed;
              DataCupom.DataSet.FieldByName('VAL_CARGA_EST').AsFloat := DataCupom.DataSet.FieldByName('VAL_CARGA_EST').AsFloat + ValorCargaEst;
              DataCupom.DataSet.FieldByName('VAL_CARGA_MUN').AsFloat := DataCupom.DataSet.FieldByName('VAL_CARGA_MUN').AsFloat + ValorCargaMun;
              Grava_Dados(DataCupom.DataSet);
...
//fabricio listapedido
            aContingenciaOff := false;
if not ListaPedido then //fablista
...
//fabricio aqui
          ...
          //rondado observacoes, se for CF-e SAT
          else if sPAFNFCe = 2 then

          begin
            BlockInput(false);

            //Gerar o SAT (Carregar os dados da aplicacao para posteriormente criar o xml)
              ...
              begin
                ...
                try
                  if DmWorkCom.sSAT_EXT_RESUMIDO = 'S' then
                     ...
                     //rondado modificar a ultima msg de extrato
                     begin
                     DmAcbr.ACBrSATExtratoESCPOS1.SoftwareHouse :='CFe-SAT gerado por NFCe.exe ';
                     DmAcBr.ACBrSAT1.ImprimirExtrato;
                     //rondado tentiva de liberação de porta
            ...
            begin
              ...
              begin
                ...
                begin
                  ...
                  //if DmWorkCom.Dados_Caixa(SVData) then        // original
                  // rondado pegar a data do caixa aberto
                  DmWorkcom.Dados_Caixa_Aberto;

                  if DmWorkCom.Dados_Caixa(DmWorkCom.CdsCaixa.FieldByName('DATA_ABERTURA').AsDateTime) then

                  begin
                    DmWorkCom.Dados_CaixaMov(0);
                    DmWorkCom.CdsCaixaMov.Append;

                    //rondado modificacao para sair o nro do pedido e sessao do sat
                    if sPAFNFCe =2 then
                    begin
                        DmWorkCom.CdsCaixaMov.FieldByName('HISTORICO').AsString := 'EMISSÃO CFe: '+
                        'Pedido '+ InttoSTR(DataCupom.DataSet.FieldByName('ID').AsInteger) + ' Nro. Sessao= '+
                         IntToStr(DmAcBr.ACBrSAT1.Resposta.numeroSessao) ;
                    end;

                    //rondado modificação qdo for F1 lista de pedido

                     if sPAFNFCe =3 then
                    begin
                        DmWorkCom.CdsCaixaMov.FieldByName('HISTORICO').AsString := 'Pedido Nro: '
                         + InttoSTR(DataCupom.DataSet.FieldByName('ID').AsInteger)
                    end;
                    ...
                    //rondado inserir data da movimentação do caixa aberto
                    DmWorkcom.CdsCaixaMov.FieldByName('DATA_MOV').AsDateTime := DmWorkCom.CdsCaixa.FieldByName('DATA_ABERTURA').AsDateTime;
...
//          if DmAcBr.ACBrECF1.Estado = estLivre then
      ...
      //habilitar novamente CFe ao inves da lista de pedido
      ListaPedido:=false;
      sPAFNFCe:=2;
    end;
  end
...
begin
  ...
  //rondado desabilitar o fechamento de mesa para inclusao de itens avulsos
  ...
  try
    ...
    begin
      if DmProdutos.Dados_Produto(FrmProdutos.sIDProd) then
         aux2 := InttoSTR(FrmProdutos.sIDProd) ;
...
procedure TFrmEmiCupom.labFecharMesasClick(Sender: TObject);
begin
  inherited;
...
begin
  ...
  //rondado desativa a lista de pedido e ativa a emissão pelo sat
