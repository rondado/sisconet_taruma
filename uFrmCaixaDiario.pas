...
var aValorAbre, aMotivoSangria:String;
    aValor:Double;
begin
  inherited;
  //
  ...
  begin
    try
      ...
      aValor := StrToFloat(aValorAbre);
      DataCaixaMov.DataSet.Append;
      DataCaixaMov.DataSet.FieldByName('HISTORICO').AsString := 'SANGRIA DE CAIXA' + '- MOTIVO: ' +aMotivoSangria ;
      DataCaixaMov.DataSet.FieldByName('ORIGEM').AsString    := 'MOVIMENTO';
...
begin
 ...
 // DmWorkCom.Dados_Caixa(SVData);  //pega a data atual do computador

 //rondado pegar a data do caixa em aberto
 DmWorkCOm.Dados_Caixa_Aberto;


 //rondado pegar a data do caixa aberto
  DmWorkCom.Dados_Caixa(DataCaixa.DataSet.FieldByName('DATA_ABERTURA').AsDateTime);

 // DmWorkCom.Dados_CaixaMov(DataCaixa.DataSet.FieldByName('ID').AsInteger);  original
 //rondado buscar por data corrente
 DmWorkCom.Dados_CaixaMov(DataCaixa.DataSet.FieldByName('DATA_ABERTURA').AsDateTime);
...
begin
  ...
  //Alteraçao para não pegar o valor da abertura  Rondado
  CdsCaixaAuxTOTALMOV.AsCurrency      := sTotalReceita-sTotalDespesa;
  CdsCaixaAux.Post;
