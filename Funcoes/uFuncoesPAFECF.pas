...
uses
  ...
  ACBrDevice, ACBrBAL, DBClient, uMensagem,winspool;
...
type
   ...
   Public
    ...
    gArqTXTDav, gTXTAutomatico,gTipoConsulta, gAgrupaVenda : Boolean;
    ...
    procedure EmiteGerencialSangriaNFCe(pCaixa,pOperador:String;pData:TDateTime;pValor:Double; motivo:String);
    procedure EmiteGerencialSSuprimentoNFCe(pCaixa,pOperador:String;pData:TDateTime;pValor:Double;motivo:String);
...
begin
  ...
  begin
    ...
    //rondado tipo de consulta por codigo 1, 0 descricao
    gTipoConsulta      := ArqINI.ReadBool('GERAL','TIPO_CONSULTA',false);

    //rondado agrupa produto (mesas, vendas..)
    gAgrupaVenda      := ArqINI.ReadBool('GERAL','AGRUPA_VENDA',false);
    //
...
begin
  ...
  try
   ...
   // DmAcBr.ACBrECF1.PafMF_ArqMF(pArquivo); //original
...
begin
  Result := true;
  try
    //DmAcBr.ACBrECF1.PafMF_ArqMFD(pArquivo);  // original
         DmAcBr.ACBrECF1.PafMF_ArqMFD_Binario(pArquivo);
  except
    on E:Exception do
    begin
...
procedure TFuncoesPAF.EmiteGerencialSangriaNFCe(pCaixa, pOperador: String; pData: TDateTime; pValor: Double; motivo: String);
var prn:TCharPrinter;
  Handle: THandle;
...
begin
  ...
  //RONDADO MODIFICANDO A IMPRESSADO DE SANGRIA DE CAIXA
  ...
  begin
   ...
   // Cria o objeto de impressora
    ...
    TextoUnicode := '                SANGRIA DE CAIXA            ' + #13#10;
    ...
    TextoUnicode := 'CAIXA: '+pCaixa+#13#10;
    TextoAnsiString := AnsiString(TextoUnicode);
    WritePrinter(Handle, PAnsiChar(TextoAnsiString), Length(TextoAnsiString), CaracteresImpressos);

    TextoUnicode := 'OPERADOR: '+pOperador+#13#10;
    TextoAnsiString := AnsiString(TextoUnicode);
    WritePrinter(Handle, PAnsiChar(TextoAnsiString), Length(TextoAnsiString), CaracteresImpressos);

    TextoUnicode := 'DATA: '+FormatDateTime('DD/MM/YYYY',pData)+' - HORARIO: '
    +FormatDateTime('hh:nn',pData)+#13#10;
    TextoAnsiString := AnsiString(TextoUnicode);
    WritePrinter(Handle, PAnsiChar(TextoAnsiString), Length(TextoAnsiString), CaracteresImpressos);


    TextoUnicode := 'VALOR: '+FormatFloat('0.00',pValor)+#13#10;
    TextoAnsiString := AnsiString(TextoUnicode);
    WritePrinter(Handle, PAnsiChar(TextoAnsiString), Length(TextoAnsiString), CaracteresImpressos);

    TextoUnicode := 'Motivo: '+motivo+#13#10;
    TextoAnsiString := AnsiString(TextoUnicode);
    WritePrinter(Handle, PAnsiChar(TextoAnsiString), Length(TextoAnsiString), CaracteresImpressos);
...
procedure TFuncoesPAF.EmiteGerencialSSuprimentoNFCe(pCaixa,
  pOperador: String; pData: TDateTime; pValor: Double; motivo:String);
var
prn:TCharPrinter;
  Handle: THandle;
  CaracteresImpressos: DWORD;
...
begin
  ...
  Prn.Free; }
   ...
   //RONDADO MODIFICANDO A IMPRESSADO DE SUPRIMENTO DE CAIXA
  Arquivo      := TIniFile.Create(ExtractFilePath(ParamStr(0)) + 'cupom.ini');
  nomecupom    := Arquivo.ReadString('IMPRESSORA','NOME',' ');
  Arquivo.Free;
  ...
  begin
   ...
   // Cria o objeto de impressora
    ...
    //Nenhum controle é aplicado a impressora.
      StartPagePrinter(Handle);
    TextoUnicode := '                SUPRIMENTO DE CAIXA            ' + #13#10;
    TextoAnsiString := AnsiString(TextoUnicode);
    WritePrinter(Handle, PAnsiChar(TextoAnsiString), Length(TextoAnsiString), CaracteresImpressos);
