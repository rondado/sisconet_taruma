...
uses
  ...
  pcnRede, ACBrSATClass, ACBrPosPrinter, ACBrBlocoX, ACBrBlocoX_Comum,
  pnfsConversao, winspool, uTrocaCaracteresEspeciais;
...
type
  TDmWorkCom = class(TDataModule)
    ...
    QryPedidosCNPJ_OPERADORA: TStringField;
    DspMesasItensAgrupa: TDataSetProvider;
    QryMesasItensAgrupa: TSQLQuery;
    CdsMesasItensAgrupa: TClientDataSet;
    CdsMesasItensAgrupaID_MESA: TIntegerField;
    CdsMesasItensAgrupaID_PRODUTO: TIntegerField;
    CdsMesasItensAgrupaQUANT: TFloatField;
    CdsMesasItensAgrupaPRECO_UNIT: TFloatField;
    CdsMesasItensAgrupaPRECO_TOTAL: TFloatField;
    CdsMesasItensAgrupaCANCELADO: TStringField;
    CdsMesasItensAgrupaDESCRICAO: TStringField;
    QryMesasItensAgrupaID_MESA: TIntegerField;
    QryMesasItensAgrupaID_PRODUTO: TIntegerField;
    QryMesasItensAgrupaDESCRICAO: TStringField;
    QryMesasItensAgrupaQUANT: TFloatField;
    QryMesasItensAgrupaPRECO_UNIT: TFloatField;
    QryMesasItensAgrupaPRECO_TOTAL: TFloatField;
    QryMesasItensAgrupaCANCELADO: TStringField;
    ...
    procedure DspPedidoClassGetTableName(Sender: TObject; DataSet: TDataSet;
      var TableName: String);
    ...
    procedure Dsp60AGetTableName(Sender: TObject; DataSet: TDataSet;
      var TableName: String);
    procedure DspControleDocGetTableName(Sender: TObject; DataSet: TDataSet;
      var TableName: String);
    ...
    procedure DspPedidoItensGetTableName(Sender: TObject; DataSet: TDataSet;
      var TableName: String);
    ...
    procedure DspDadosTecnicosGetTableName(Sender: TObject; DataSet: TDataSet;
      var TableName: String);
    ...
    procedure DspMesasGetTableName(Sender: TObject; DataSet: TDataSet;
      var TableName: String);
    procedure DspMesasAbertasGetTableName(Sender: TObject; DataSet: TDataSet;
      var TableName: String);
  ...
  private
    ...
    function Le_Linha_Arquivo(fTexto, fCaracter: string;
      fPosicao: Integer): string;
  ...
  public
    ...
    sSalvaArq: Boolean;
    sLogoMarca, sPathArq, sCertificado, sSenha, sSerial, sUFWS, sUFWSAux, sHost,
      sPorta, sUsuario, sSenhaWS, sUsarWSAux: String;
    ...
    sNFSeCertSenha, sNFSeNumSerie, sNFSePathLogs, sNFSeSchemas, sNFSeArqINI,
      ...
      sNFSeEmitRazao, sNFSeProxyHost, sNFSeProxyPorta, sNFSeProxyUser,
      sNFSeProxySenha, sNFSeLogoMarca, sNFSePrestLogo, sNFSePrefeitura,
      sNFSeSmtpHost, sNFSeSmtpPort, sNFSeSmtpUser, sNFSeSmtpPass,
      sNFSeEmailRemetente: String;
    sNFSeTipoAmb: Integer;
    sNFSeSalvar, sNFSeSalvarSoap, sNFSeVisualizar, sNFSeEmailTLS,
      sNFSeEmailSSL: Boolean;
    ...
    sPATH_ARQ_LOG_SAT, sNOME_DLL_SAT, sCOD_ATV_SAT, sCOD_UF_SAT, sSALVAR_CFES,
      sSALVAR_CEF_CANC, sSALVAR_ENVIO_SAT, sSAT_UTF8, sSAT_FORMAT_XML,
      sSAT_VERSAO, sSAT_SEPARAR_CNPJ, sSAT_SEPARAR_MES, sSAT_WIFI_SSID,
      sSAT_WIFI_SENHA, sSAT_PPPoE_USER, sSAT_PPPoE_LOGIN, sSAT_REDE_IP,
      sSAT_REDE_USER, sSAT_REDE_LOGIN, sSAT_IMP_PREVIEW, sSAT_IMP_NOME: String;
    ...
    sAMBIENTE_SAT, sSAT_PAG_CODIGO, sSAT_TIPO_REDE, sSAT_PROTOCOLO_REDE,
      sSAT_WIFI_SEG, sSAT_REDE_PROXY, sSAT_REDE_PORTA, sSAT_IMP_LARGURA,
      sSAT_MODELO, sSAT_IMP_TOPO, sSAT_IMP_FUNDO, sSAT_IMP_ESQUERDA,
      sSAT_IMP_DIREITA: Integer;
    ...
    sPATH_REMESSA, sPATH_RETORNO, sPATH_LOGOTIPO, sPATH_GERARPDF, sMENSAGEM,
      sESPECIE, sMOEDA, sACEITE, sCARTEIRA, sLOCAL_PAGTO, sINSTRUCAO_1,
      sINSTRUCAO_2: String;
    sDIAS_PROTESTO, sCONT_REMESSA: Integer;
    xSolucaoPendentes: Boolean;
    function Dados_Agenda(fIDFunc: Integer; fDatai, fDataf: TDateTime;
      fConcluido: String): Boolean;
    procedure retorna_Descricao(var codigo, Descricao: String;
      var Dt_Inicio: TDateTime; fUF: string);
    function Dados_PedidoClassificacao(fTipo: Integer = -1; fID: Integer = 0;
      Lista: Boolean = false): Boolean;
    ...
    function Dados_Pedido(pDatai, pDataf: TDateTime; pCaixa: String = '';
      pTipo: Integer = 0; ApenasConfirmados: Boolean = true): Boolean; overload;
    function Dados_PedidoFP(fID: Integer; pOrdemConf: Boolean = false)
      : Boolean; overload;
    ...
    function Dados_OS(pDatai, pDataf: TDateTime; pStatus: Integer)
      : Boolean; overload;
    function Dados_OSHoras(pID: Integer): Boolean; overload;
    function Dados_PedidoItens(fID: Integer): Boolean; overload;
    function Dados_PedidoItens(pDatai, pDataf: TDateTime; pCaixa: String = '';
      pTipo: Integer = 0; ApenasConfirmados: Boolean = true): Boolean; overload;
    ...
    function Gerar_Contas(fTipo, fIDPedido, fIDOS, fIDContato,
      fQuantPagtos: Integer; fFormaPag, fContato, fDocContato: String;
      fDataBase: TDateTime; fValor: Double): Boolean;
    ...
    //rondado alteração pegar por data da movimentacao
    function Dados_CaixaMov(fID: Tdatetime): Boolean;

    //rondado funcao para pegar a data do caixa aberto
    procedure Dados_Caixa_Aberto();
    ...
    function Grava_MovCaixa(fHistorico, fOrigem, fDC: String;
    ...
    function Dados_60M(pDatai, pDataf: TDateTime; pCaixa: String = '';
    ...
    function Dados_DocECF(pDatai, pDataf: TDateTime; pCaixa: String = '')
      : Boolean; overload;
    ...
    procedure ProcessaRegistro10(pDatai, pDataf: TDateTime;
      pEstrutura, pNatureza, pFinalidade: String);
    procedure ProcessaRegistro11;
    procedure ProcessaRegistro60m(pDatai, pDataf: TDateTime;
      p60d, p60i, p60r: Boolean);
    ...
    procedure GerarRegistro10(pDatai, pDataf: TDateTime;
      pConvenio, pNatureza, pFinalidade: String);
    ...
    procedure GerarRegistro60M(pDatai, pDataf: TDateTime; p60d: Boolean;
    ...
    function RegistrosECF(fNomeArquivo: String; fDatai, fDataf: TDateTime;
      ApenasProdMovto: Boolean = false): Boolean;
    procedure CriaRegistroU1;
    procedure CriaRegistroA2(pDatai, pDataf: TDateTime);
    procedure CriaRegistroP2(pDatai, pDataf: TDateTime;
      ApenasProdMovto: Boolean);
    procedure CriaRegistroE2(pDatai, pDataf: TDateTime;
      ApenasProdMovto: Boolean);
    ...
    function VendasIdentificadas(fNomeArquivo, CNPJCPF: String;
      Mes, Ano: Integer): Boolean;
    procedure CriaRegistroZ1;
    procedure CriaRegistroZ2;
    procedure CriaRegistroZ3;
    procedure CriaRegistroZ4(var TotalReg: Integer; pDatai, pDataf: TDateTime;
      pCNPJCPF: String);
    ...
    function VerificaMD5Registro(pTipo, pID: Integer;
      pID1: Integer = 0): Boolean;
    ...
    function IncluirCupomExecute(var Mensagem: String; pDAV: Boolean;
      pPV: Boolean = false): Boolean;
    ...
    function CalcularTributoPedido(pIDPedido: Integer;
      pCST, pCFOP: String): Boolean;
    procedure GravaChaveNFEPedido(pChaveNFe, pProtocol, pRecibo, NumLote,
      NomeArqXML, Status_NFe: String; TipoEmiss, pIDPedido: Integer;
    ...
    procedure GravaFechamentoCaixaDiario(DataFechamento: TDateTime = 0;
      Dinheiro: Double = 0; Cartao: Double = 0; Cheque: Double = 0;
      Outros: Double = 0);
    ...
    function VerificaCaixaAberto(var NomeOperador: String;
      var DataCaixa: TDateTime): Boolean;
    function LocalizaIDMesa(pNumMesa: String): Integer;
    function InsereItemCupom(pQuant, pAcres, pDesc: Double;
      pPercentual: Boolean = false): Boolean;
    procedure GravaPDVNaoSincronizado;
    function VerificaReducaoZGravada(CRZ, NumSerie: String): Boolean;
    function GravaContaAdicionalPagtoParcial(pIDConta: Integer;
      pValor: Double): Boolean;
    function GravaDadosReducaoArqSPED(pArquivo: String): Boolean;
    procedure RetornaCSTCFOPItemCompra(var CST: String; var CFOP: String;
      var Aliquota: Double; var REDUCAOBASE: Double; UF: String;
      IDProd: Integer);
    function GerarNFe(pID: Integer; Apropria: Boolean; NFCe: Boolean = false;
      ContingenciaOff: Boolean = false): Boolean;
    function GerarSAT(pID: Integer): Boolean;
    function RetornaValoresTributarios(var abase_icms, avalor_icms,
      abase_icms_st, avalor_icms_st, abase_pis, avalor_pis, abase_cofins,
      avalor_cofins, abase_ipi, avalor_ipi, abase_iss, avalor_iss: Double;
      pID: Integer): Boolean;
    ...
    function CapturaQuantVendidaData(pIDProd: Integer;
      pDataMov: TDateTime): Double;
    ...
    procedure VerificaGeraAbsProdCombustivel(IDPedido: Integer;
      NumeroDoc: String);
    function RetornaBicoProduto(pIDProd: Integer): String;
    procedure VerificaAlteraStatusAbastecimento(pIDPedido: Integer;
      Cancelamento: Boolean = false);
    procedure GravaReducaoZNaoAutomatica;
    procedure GravaRegistroA2(DataMov: TDateTime; Descricao: String;
    ...
    procedure Grava_DadosSAT(pIDPedido: Integer; pChaveSat: String;
    ...
    function VerificaProdutoGrade(IDProduto, IDCor, IDTamanho: Integer)
      : Boolean;
    function ClonarProdutoCodTamanho(IDProduto, IDCor,
    ...
    procedure GravaQuitacaoConta(pDataBaixa: TDateTime; pValor: Double;
      pNossoNum: String);
...
uses uDmConexao, uLibrary, ACBrECF, ACBrIBPTax, pcnNFe, uDMClientes,
  ACBrECFClass, uDmAuxiliar, uDmPosto, DateUtils, ACBrNFeWebServices,
  pcnCFe, ACBrSAT, uDmProdutos;
...
procedure TDmWorkCom.DspAgendaGetTableName(Sender: TObject; DataSet: TDataSet;
  var TableName: String);
begin
  TableName := 'AGENDA';
...
procedure TDmWorkCom.DspTipoEndGetTableName(Sender: TObject; DataSet: TDataSet;
  var TableName: String);
begin
...
function TDmWorkCom.Dados_Agenda(fIDFunc: Integer; fDatai, fDataf: TDateTime;
  fConcluido: String): Boolean;
var
  aux: String;
begin
...
procedure TDmWorkCom.retorna_Descricao(var codigo, Descricao: String;
  var Dt_Inicio: TDateTime; fUF: string);
begin
  if fUF = 'AM' then
  begin
...
function TDmWorkCom.Dados_PedidoClassificacao(fTipo: Integer = -1;
  fID: Integer = 0; Lista: Boolean = false): Boolean;
begin
  ...
  begin
    ...
    if Lista then
      QryPedidoClass.SQL.Add('and classificacao.exibe_lista = ' +
        QuotedStr('S'))
  end
  else
...
procedure TDmWorkCom.DspFormaPagGetTableName(Sender: TObject; DataSet: TDataSet;
  var TableName: String);
begin
...
function TDmWorkCom.Dados_OS(pDatai, pDataf: TDateTime;
...
function TDmWorkCom.Dados_PedidoItens(pDatai, pDataf: TDateTime;
  pCaixa: String = ''; pTipo: Integer = 0;
  ApenasConfirmados: Boolean = true): Boolean;
var
  Datai, Dataf: String;
begin
  Datai := FormatDateTime('MM/DD/YYYY', pDatai);
...
procedure TDmWorkCom.DspContasGetTableName(Sender: TObject; DataSet: TDataSet;
  var TableName: String);
begin
  TableName := 'CONTAS';
...
begin
  ...
  begin
    MessageDlg('É necessário informar o produto. verifique!!!', mtWarning,
      [mbOK], 0);
    Abort;
  end;
  // Dados_Produto(DataSet.FieldByName('ID_PRODUTO').AsInteger);
  DataSet.FieldByName('DESCRICAO').AsString :=
    Retorna_DescricaoProduto(DataSet.FieldByName('ID_PRODUTO').AsInteger);
...
procedure TDmWorkCom.DspChequesGetTableName(Sender: TObject; DataSet: TDataSet;
  var TableName: String);
begin
  TableName := 'CHEQUES';
...
begin
  ...
  begin
    MessageDlg('Não é permitido movimentação de pedidos para o mes fechado!!!',
      mtWarning, [mbOK], 0);
    Abort;
  end;
  ...
  begin
    DataSet.FieldByName('PEDIDO_NUM').AsString :=
      ZeroEsquerda(IntToStr(aNumPed), 10);
  end
  else
  begin
    InformaNumped;
  ...
  DataSet.FieldByName('IDCLIENTE').AsInteger :=
    CdsConfigCLIENTE_PADRAO.AsInteger;
  DataSet.FieldByName('ID_VENDEDOR').AsInteger :=
    CdsConfigVENDEDOR_PADRAO.AsInteger;
  if sPAFNFCe = 0 then
    DataSet.FieldByName('ID_CLASSIFICACAO').AsInteger :=
      CdsConfigCLASSIFICA_PADRAO.AsInteger
  else
    DataSet.FieldByName('ID_CLASSIFICACAO').AsInteger :=
      CdsConfigCLASSIFICA_NFCE.AsInteger;
  DataSet.FieldByName('ID_FORMAPAG').AsInteger :=
    CdsConfigFORMAPAG_PADRAO.AsInteger;
...
begin
  if (CdsPedidosIDCLIENTE.IsNull) or (CdsPedidosIDCLIENTE.AsInteger = 0) or
    ...
    (CdsPedidosID_CLASSIFICACAO.IsNull) or
    (CdsPedidosID_CLASSIFICACAO.AsInteger = 0) or (CdsPedidosTIPODOC.IsNull) or
    (CdsPedidosTIPODOC.AsInteger < 0) or (CdsPedidosPEDIDO_NUM.IsNull) or
    (CdsPedidosPEDIDO_NUM.AsString = '') or (CdsPedidosID_FORMAPAG.IsNull) or
    (CdsPedidosID_FORMAPAG.AsInteger = 0) then
  begin
    MessageDlg('Os campos indicados com * são de preenchimento obrigatórios.',
      mtWarning, [mbOK], 0);
    Abort;
  end;
...
procedure TDmWorkCom.DspEmpresaGetTableName(Sender: TObject; DataSet: TDataSet;
  var TableName: String);
begin
  TableName := 'EMPRESA';
...
procedure TDmWorkCom.DspConfigGetTableName(Sender: TObject; DataSet: TDataSet;
  var TableName: String);
begin
  TableName := 'CONFIG';
...
function TDmWorkCom.Movimenta_Estoque(fTipoMov: Integer;
  fItens: TDataSet): Boolean;
var
...
begin
  ...
  try
    ...
    begin
      ...
      if (DmProdutos.CdsProdutos.FieldByName('CONTROLAR_ESTOQUE')
        .AsString = 'S') and (fItens.FieldByName('CANCELADO').AsString = 'N')
      then
      begin
        if DmProdutos.CdsProdutos.FieldByName('COMBUSTIVEL').AsString <> 'S'
        then
        begin
          // retorna tipo de produto
          aTipoProd := Retorna_TipoProduto(fItens.FieldByName('IDPRODUTO')
            .AsInteger);
          ...
          if aControla then
          begin
            aEstAtual := Retorna_EstoqueAtual(fItens.FieldByName('IDPRODUTO')
              .AsInteger);
            if fTipoMov = 0 then // baixar
              aNovoEstoque := aEstAtual - fItens.FieldByName
                ('QUANTIDADE').AsFloat
            ...
            DmProdutos.CdsProdutos.FieldByName('ESTOQUE').AsFloat :=
              aNovoEstoque;
            DmProdutos.CdsProdutos.FieldByName('DATA_ESTOQUE').AsDateTime
              := SVData;
            DmProdutos.CdsProdutos.FieldByName('HORA_ESTOQUE').AsDateTime
              := SVHora;
            Grava_Dados(DmProdutos.CdsProdutos);
            // grava no produto
            Grava_MovEstoque(fItens.FieldByName('IDPRODUTO').AsInteger,
              aNovoEstoque);
            // grava tabela de movimento de estoque
            DmProdutos.Grava_TabMovEst(fTipoMov, fItens.FieldByName('IDPRODUTO')
              .AsInteger, fItens.FieldByName('QUANTIDADE').AsFloat);
          end;
          // se for produto composto da baixa na composição
          if aTipoProd = 1 then
          begin
            // processa baixa acrescimo produto composto
            DmProdutos.Dados_ProdComposto(fItens.FieldByName('IDPRODUTO')
              .AsInteger);
            while not DmProdutos.CdsProdutoComposto.Eof do
            begin
              // captura estoque atual do produto composto;
              aEstAtual := Retorna_EstoqueAtual
                (DmProdutos.CdsProdutoCompostoID_PRODUTO.AsInteger);
              if fTipoMov = 0 then // baixar
                aNovoEstoque := aEstAtual -
              ...
              Grava_MovEstoque
                ...
                aNovoEstoque);
              // grava tabela de movimento de estoque
              DmProdutos.Grava_TabMovEst(fTipoMov,
                fItens.FieldByName('IDPRODUTO').AsInteger,
                fItens.FieldByName('QUANTIDADE').AsFloat *
                DmProdutos.CdsProdutoCompostoQUANTIDADE.AsFloat);
              // proximo produto
...
function TDmWorkCom.Gerar_Contas(fTipo, fIDPedido, fIDOS, fIDContato,
  fQuantPagtos: Integer; fFormaPag, fContato, fDocContato: String;
  fDataBase: TDateTime; fValor: Double): Boolean;
var
  i, aDias: Integer;
...
procedure TDmWorkCom.DspComprasGetTableName(Sender: TObject; DataSet: TDataSet;
  var TableName: String);
begin
  TableName := 'COMPRAS';
...
begin
  if (CdsComprasIDFORNECEDOR.IsNull) or (CdsComprasIDFORNECEDOR.AsInteger = 0)
    or (CdsComprasID_CLASSIFICACAO.IsNull) or
    (CdsComprasID_CLASSIFICACAO.AsInteger = 0) or (CdsComprasTIPODOC.IsNull) or
    (CdsComprasTIPODOC.AsInteger < 0) or (CdsComprasID_FORMAPAG.IsNull) or
...
begin
  ...
  DataSet.FieldByName('IDPRODUTO').AsInteger :=
    DmProdutos.CdsProdutosID.AsInteger;
...
var
  ...
  abase_icms, avalor_icms, abase_icms_st, avalor_icms_st, abase_pis, avalor_pis,
    abase_cofins, avalor_cofins, abase_ipi, avalor_ipi, abase_iss,
    avalor_iss: Double;
begin
  if not xAtMD5 then
  begin
    ...
    if CdsPedidoClass.Locate('ID', CdsPedidosID_CLASSIFICACAO.AsInteger, [])
    then
    begin
      ...
      DataSet.FieldByName('TIPO_PEDIDO').AsInteger :=
        CdsPedidoClassTIPO.AsInteger;
    end
    else
    begin
    ...
    if CdsPedidoClassDEVOLUCAO.AsString <> '' then
      DataSet.FieldByName('DEVOLUCAO').AsString :=
        CdsPedidoClassDEVOLUCAO.AsString;
    //
    if CdsPedidoClass.FieldByName('TIPO').AsInteger = 3 then
    begin
      if DataSet.FieldByName('IDCLIENTE').AsInteger = CdsConfigCLIENTE_PADRAO.AsInteger
      then
      begin
        MessageDlg('Não é permitido DAV com cliente padrão!!!', mtWarning,
          [mbOK], 0);
        Abort;
      end;
    end;
    ...
    begin
      if RetornaValoresTributarios(abase_icms, avalor_icms, abase_icms_st,
        avalor_icms_st, abase_pis, avalor_pis, abase_cofins, avalor_cofins,
        abase_ipi, avalor_ipi, abase_iss, avalor_iss, DataSet.FieldByName('ID')
        .AsInteger) then
      begin
    ...
    begin
      DataSet.FieldByName('NUMPV').AsString :=
        ZeroEsquerda(IntToStr(GenID('NUMPV')), 10);
    end;
    ...
    begin
      ...
      begin
        ...
        DataSet.FieldByName('NUMDAV').AsString :=
          ZeroEsquerda(IntToStr(GenID('NUMDAV')), 10);
      end;
    end;
...
procedure TDmWorkCom.DspCaixaGetTableName(Sender: TObject; DataSet: TDataSet;
  var TableName: String);
begin
  TableName := 'CAIXA';
...
procedure TDmWorkCom.DspCaixaMovGetTableName(Sender: TObject; DataSet: TDataSet;
  var TableName: String);
begin
  TableName := 'CAIXA_MOV';
...
//rondado retornar a data do caixa aberto
procedure TDmWorkCom.Dados_Caixa_Aberto();
var texto:String;
begin
  CdsCaixa.Close;
  ...
  Qrycaixa.SQL.Add('where caixa.fechado = ' + QuotedStr('N'));
  texto := QryCaixa.SQL.Text;
  CdsCaixa.Open;
...
function TDmWorkCom.Dados_CaixaMov(fID: TDateTime): Boolean;//TDmWorkCom.Dados_CaixaMov(fID: Integer): Boolean;
begin
  CdsCaixaMov.Close;
  CdsCaixaMov.Params.ParamByName('pData').AsDateTime := fID;
  CdsCaixaMov.Open;
  //
  Result := false;
...
begin
  ...
  begin
    if InputQuery('Número do Pedido', 'Informe o Numero do Pedido', aNumPed)
    then
    begin
      try
        ...
        begin
          MessageDlg('Pedido já cadastrado no aplicativo. verifique!!!',
            mtWarning, [mbOK], 0);
          aControle := 0;
          Result := false;;
        end
    ...
    begin
      if (CdsPedidos.FieldByName('PEDIDO_NUM').IsNull) or
        (CdsPedidos.FieldByName('PEDIDO_NUM').AsString = '') then
        MessageDlg
          ('Será necessário informar o número do pedido para inserir dados!!!',
          mtWarning, [mbOK], 0);
      Result := false;
      aControle := 1;
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        MessageDlg('O produto não pode fica com estoque negativo. Verifique!!!',
          mtWarning, [mbOK], 0);
        Abort;
      end;
    end;
    ...
    if (DataSet.FieldByName('quantidade').AsFloat = 0) or
      (DataSet.FieldByName('valor').AsFloat = 0) then
    begin
      MessageDlg('É necessário informar a quantidade e o preco do produto!!!',
        mtWarning, [mbOK], 0);
      Abort;
    end;
    ...
    begin
      ...
      begin
        if not DmProdutos.Dados_Produto_Aliquota
          (DataSet.FieldByName('idproduto').AsInteger,
          CdsPedidosUF_END_ENTREGA.AsString) then
        begin
          MessageDlg
            ('É necessário cadastrar a aliquota do produto para o estado do cliente!!!',
            mtWarning, [mbOK], 0);
          Abort;
        end;
      end;
    ...
    begin
      if DataSet.FieldByName('VALOR').AsCurrency <
        DmProdutos.CdsProdutos.FieldByName('PRECO_COMPRA').AsCurrency then
      begin
        MessageDlg('O Preco de Venda não pode ser menor que preço de compra!!!',
          mtWarning, [mbOK], 0);
        Abort;
      end;
    end;
    // calcula margem do produto
    if DataSet.FieldByName('VALOR').AsCurrency <> DataSet.FieldByName
      ('VALOR_REAL').AsCurrency then
    begin
      aIndiceProduto := DmProdutos.CdsProdutos.FieldByName
      ...
      DataSet.FieldByName('MARGEM').AsCurrency :=
        TruncaValor(aIndiceProduto, 2);
    end
    else
    begin
      DataSet.FieldByName('MARGEM').AsCurrency :=
        TruncaValor(DmProdutos.CdsProdutos.FieldByName('MARGEM_LUCRO')
        .AsCurrency, 2);
    end;
    DataSet.FieldByName('VALOR_TOTAL').AsFloat :=
      (((DataSet.FieldByName('QUANTIDADE').AsFloat *
      DataSet.FieldByName('VALOR').AsCurrency) - DataSet.FieldByName('DESCONTO')
      .AsFloat) + DataSet.FieldByName('ACRESCIMO').AsFloat);
    ...
    begin
      if Trim(DataSet.FieldByName('CST').AsString) = '' then
        DataSet.FieldByName('CST').AsString :=
          DmProdutos.CdsProduto_Aliquota.FieldByName('CST').AsString;
      if Trim(DataSet.FieldByName('CSOSN').AsString) = '' then
        DataSet.FieldByName('CSOSN').AsString :=
          DmProdutos.CdsProduto_Aliquota.FieldByName('CSOSN').AsString;
      if Trim(DataSet.FieldByName('CFOP').AsString) = '' then
        DataSet.FieldByName('CFOP').AsString :=
          DmProdutos.CdsProduto_Aliquota.FieldByName('CFOP').AsString;
      DataSet.FieldByName('REDUCAO_BASE').AsFloat :=
        DmProdutos.CdsProduto_Aliquota.FieldByName('REDUCAO_BASE').AsFloat;
      DataSet.FieldByName('MVA').AsFloat := DmProdutos.CdsProdutos.FieldByName
        ('MVA').AsFloat;

      // captura a base de calculo
      aBaseCalculo :=
        (((DataSet.FieldByName('QUANTIDADE').AsFloat *
        DataSet.FieldByName('VALOR').AsCurrency) - DataSet.FieldByName
        ('DESCONTO').AsFloat) + DataSet.FieldByName('ACRESCIMO').AsFloat);
      // se tiver redução na base, aplica
      if DmProdutos.CdsProduto_Aliquota.FieldByName('REDUCAO_BASE').AsFloat > 0
      then
      begin
        aBaseCalculo :=
          TruncaValor
          ((aBaseCalculo * ((100 - DmProdutos.CdsProduto_Aliquota.FieldByName
          ('REDUCAO_BASE').AsFloat) / 100)), 2);
      end;
      ...
      DataSet.FieldByName('VALOR_PIS').AsFloat := TruncaValor(((aBaseCalculo * DataSet.FieldByName('ALIQ_PIS').AsFloat)/ 100), DmWorkCom.PAFECF.gDecimais);
      ...
      DataSet.FieldByName('VALOR_COFINS').AsFloat := TruncaValor(((aBaseCalculo * DataSet.FieldByName('ALIQ_COFINS').AsFloat)/ 100), DmWorkCom.PAFECF.gDecimais);
      ...
      DataSet.FieldByName('VALOR_IPI').AsFloat :=  TruncaValor(((aBaseCalculo * DataSet.FieldByName('ALIQ_IPI').AsFloat)/ 100), DmWorkCom.PAFECF.gDecimais);
      ...
      DataSet.FieldByName('VALOR_ICMS_ST').AsFloat := 0;
      // se for tributado
      if (Copy(DmProdutos.CdsProduto_Aliquota.FieldByName('CST').AsString, 2, 2)
        = '00') or (Copy(DmProdutos.CdsProduto_Aliquota.FieldByName('CST')
        .AsString, 2, 2) = '10') or
        (Copy(DmProdutos.CdsProduto_Aliquota.FieldByName('CST').AsString, 2, 2)
        = '20') then
      begin
        ...
        DataSet.FieldByName('VALOR_ICMS').AsFloat :=  TruncaValor(((aBaseCalculo * DataSet.FieldByName('ALIQ_ICMS').AsFloat)/ 100), DmWorkCom.PAFECF.gDecimais);
      end;
      // se for ST calcula a base para ST
      if (Copy(DmProdutos.CdsProduto_Aliquota.FieldByName('CST').AsString, 2, 2)
        = '10') or (Copy(DmProdutos.CdsProduto_Aliquota.FieldByName('CST')
        .AsString, 2, 2) = '60') or
        (Copy(DmProdutos.CdsProduto_Aliquota.FieldByName('CST').AsString, 2, 2)
        = '70') then
      begin
        aBaseCalculo :=
          ((aBaseCalculo * DmProdutos.CdsProdutos.FieldByName('MVA').AsFloat) /
          100) + aBaseCalculo;
        DataSet.FieldByName('BASE_ICMS_ST').AsFloat := aBaseCalculo;
        DataSet.FieldByName('ALIQ_ICMS_ST').AsFloat :=
          DmProdutos.CdsProduto_Aliquota.FieldByName('ALIQUOTA').AsFloat;
        DataSet.FieldByName('VALOR_ICMS_ST').AsFloat :=
          TruncaValor(((aBaseCalculo * DataSet.FieldByName('ALIQ_ICMS_ST')
          .AsFloat) / 100), DmWorkCom.PAFECF.gDecimais);
      end;
    end
    ...
    begin
      if Trim(DataSet.FieldByName('CST').AsString) = '' then
        DataSet.FieldByName('CST').AsString :=
          DmProdutos.CdsProdutos.FieldByName('CST_INTERNO').AsString;
      if Trim(DataSet.FieldByName('CSOSN').AsString) = '' then
        DataSet.FieldByName('CSOSN').AsString :=
          DmProdutos.CdsProdutos.FieldByName('CSOSN_INTERNO').AsString;
      if Trim(DataSet.FieldByName('CFOP').AsString) = '' then
        DataSet.FieldByName('CFOP').AsString :=
          DmProdutos.CdsProdutos.FieldByName('CFOP_INTERNO').AsString;
      DataSet.FieldByName('REDUCAO_BASE').AsFloat :=
        DmProdutos.CdsProdutos.FieldByName('REDUCAO_BASE').AsFloat;
      DataSet.FieldByName('MVA').AsFloat := DmProdutos.CdsProdutos.FieldByName
        ('MVA').AsFloat;
      ...
      begin
        aBaseCalculo :=
          TruncaValor
          ((aBaseCalculo * ((100 - DmProdutos.CdsProdutos.FieldByName
          ('REDUCAO_BASE').AsFloat) / 100)), DmWorkCom.PAFECF.gDecimais);
      end;
      // grava PIS, COFINS e IPI      //
      //onde esta DmWorkCom.PAFECF.gDecimais era 2, alterei para pegar tributos nos valores pequenos
      DataSet.FieldByName('CST_PIS').AsString :=
        DmProdutos.CdsProdutos.FieldByName('CST_PIS').AsString;
      DataSet.FieldByName('BASE_PIS').AsFloat := aBaseCalculo;
      DataSet.FieldByName('ALIQ_PIS').AsFloat :=
        DmProdutos.CdsProdutos.FieldByName('ALIQUOTA_PIS').AsFloat;
      DataSet.FieldByName('VALOR_PIS').AsFloat :=  TruncaValor(((aBaseCalculo * DataSet.FieldByName('ALIQ_PIS').AsFloat)  / 100), DmWorkCom.PAFECF.gDecimais);
      DataSet.FieldByName('CST_COFINS').AsString :=  DmProdutos.CdsProdutos.FieldByName('CST_COFINS').AsString;
      DataSet.FieldByName('BASE_COFINS').AsFloat := aBaseCalculo;
      DataSet.FieldByName('ALIQ_COFINS').AsFloat := DmProdutos.CdsProdutos.FieldByName('ALIQUOTA_COFINS').AsFloat;
      ...
      DataSet.FieldByName('VALOR_ICMS_ST').AsFloat := 0;
      // se for tributado
      if (Copy(DmProdutos.CdsProdutos.FieldByName('CST_INTERNO').AsString, 2, 2)
        = '00') or (Copy(DmProdutos.CdsProdutos.FieldByName('CST_INTERNO')
        .AsString, 2, 2) = '10') or
        (Copy(DmProdutos.CdsProdutos.FieldByName('CST_INTERNO').AsString, 2, 2)
        = '20') then
      begin
        DataSet.FieldByName('BASE_ICMS').AsFloat := aBaseCalculo;
        DataSet.FieldByName('ALIQ_ICMS').AsFloat :=
          DmProdutos.CdsProdutos.FieldByName('ALIQUOTA_ICMS').AsFloat;
        DataSet.FieldByName('VALOR_ICMS').AsFloat :=
          TruncaValor(((aBaseCalculo * DataSet.FieldByName('ALIQ_ICMS').AsFloat)
          / 100), DmWorkCom.PAFECF.gDecimais);
      end;
      // se for ST calcula a base para ST
      if (Copy(DmProdutos.CdsProdutos.FieldByName('CST_INTERNO').AsString, 2, 2)
        = '10') or (Copy(DmProdutos.CdsProdutos.FieldByName('CST_INTERNO')
        .AsString, 2, 2) = '60') or
        (Copy(DmProdutos.CdsProdutos.FieldByName('CST_INTERNO').AsString, 2, 2)
        = '70') then
      begin
    ...
    begin
      if Abs(DataSet.FieldByName('VALOR').AsCurrency -
        DmProdutos.CdsProdutos.FieldByName('PRECO_VENDA').AsCurrency) > 0.01
      then
      begin
        Grava_PrecoProduto(DataSet.FieldByName('idproduto').AsInteger, 1,
          DataSet.FieldByName('VALOR').AsCurrency);
      end;
    end;
    // grava os dados do cupom no item
    DataSet.FieldByName('COO').AsInteger := CdsPedidos.FieldByName('COO')
      .AsInteger;
    DataSet.FieldByName('CCF').AsInteger := CdsPedidos.FieldByName('CCF')
      .AsInteger;
    DataSet.FieldByName('GNF').AsInteger := CdsPedidos.FieldByName('GNF')
      .AsInteger;
    DataSet.FieldByName('NUMSERIEECF').AsString :=
      CdsPedidos.FieldByName('NUMSERIEECF').AsString;
  end;
  // grava MD5 do sainota
  MD5 := RetornaMD5Registro(DataSet);
  DataSet.FieldByName('MD5REGISTRO').AsString := MD5;
...
begin
  ...
  begin
    ...
    //
    if DmWorkCom.CdsContas.FieldByName('VALOR_QUITATO').AsCurrency < aValorTotal
    then
    begin
      // grava conta referente ao pagamento parcial
      DmWorkCom.GravaContaAdicionalPagtoParcial
        (DmWorkCom.CdsContas.FieldByName('ID').AsInteger,
        aValorTotal - DmWorkCom.CdsContas.FieldByName('VALOR_QUITATO')
        .AsCurrency);
    end;
    //
    if sTipoConta = 1 then
    begin
      aHistorico := 'PAGTO CONTA No' + CdsContas.FieldByName('NUMCONTA')
        .AsString + '/' + IntToStr(CdsContas.FieldByName('SERIE').AsInteger) +
        'VENC: ' + DateToStr(CdsContas.FieldByName('DATA_VENC').AsDateTime);
      aOrigem := 'CONTAS A PAGAR';
      aDC := 'D';
    end
    else if sTipoConta = 0 then
    begin
      aHistorico := 'RECBTO CONTA No ' + CdsContas.FieldByName('NUMCONTA')
        .AsString + '/' + IntToStr(CdsContas.FieldByName('SERIE').AsInteger) +
        ', VENC: ' + DateToStr(CdsContas.FieldByName('DATA_VENC').AsDateTime);
    ...
    begin
      ...
      begin
        Grava_MovCaixa(aHistorico, aOrigem, aDC,
  ...
  begin
    VerificaContaAdicionalLancada(DmWorkCom.CdsContas.FieldByName('ID')
      .AsInteger);
    //
    if sTipoConta = 1 then
    begin
      aHistorico := 'ESTORNO DE CONTA No' + CdsContas.FieldByName('NUMCONTA')
        .AsString + '/' + IntToStr(CdsContas.FieldByName('SERIE').AsInteger) +
        'VENC: ' + DateToStr(CdsContas.FieldByName('DATA_VENC').AsDateTime);
      aOrigem := 'CONTAS A PAGAR';
      aDC := 'C';
...
begin
  ...
  begin
    if Abs(DataSet.FieldByName('VALOR').AsCurrency -
      DmProdutos.CdsProdutos.FieldByName('PRECO_COMPRA').AsCurrency) > 0.01 then
    begin
      Grava_PrecoProduto(DataSet.FieldByName('idproduto').AsInteger, 0,
        DataSet.FieldByName('VALOR').AsCurrency);
    end;
  end;
...
begin
  ...
  if not DMWorkCOm.FechamentoMesa then
  begin
    ...
    begin
      ...
      begin
          InputQuery('Peso','Insira o peso em GRAMAS:',PesoGramas);
      ...
      begin
        if Application.MessageBox('Valor inválido - Deseja digitar outro Peso?', 'Erro', MB_ICONQUESTION + mb_yesno) =idYes then
       begin
        valorValido :=False;
        Pesogramas:='';
       end
    ...
    begin
     if Dmprodutos.CdsProdutos.FieldByName('UNIDADE').AsString <> 'UN' then
			  begin
			  valorValido:= false;
			  while not ValorValido do
			  begin
				  InputQuery('Quantidade','Insira a quantidade deseja:',quantidade);
			  if IsNumeric(Quantidade) then
			  begin
				 pQuant  := StrToFloat(quantidade);
				 valorValido:=True;
			  end
			  else
			  begin
  ...
  begin
      //   DMWOrkcom.FechamentoMesa := false;
  ...
  try
    ...
    //
    if ((pQuant * DmProdutos.CdsProdutos.FieldByName('PRECO_VENDA').AsFloat) <=
      pDesc) then
    begin
...
begin
  ...
  try
    ...
    if aAliquota = '' then
    begin
      MessageDlg('Aliquota do produto não esta definida!!!', mtWarning,
        [mbOK], 0);
      Result := false;
      Abort;
    end;
    ...
    // define descricao
    aDescricao := Trim(Copy(DmProdutos.CdsProdutos.FieldByName('descricao')
      .AsString, 1, 29));
    // define valor
    aValor := TruncaValor(CdsPedidoItens.FieldByName('valor').AsFloat,
      PAFECF.gDecimais);
    // verifica desconto
    tipoDescAcre := 'D';
    ...
    begin
      tipoDescAcre := 'D';
      aDesconto := TruncaValor((CdsPedidoItens.FieldByName('desconto')
        .AsFloat), 2);
    end;
    if (CdsPedidoItens.FieldByName('acrescimo').AsFloat > 0) then
    begin
      tipoDescAcre := 'A';
      aDesconto := TruncaValor((CdsPedidoItens.FieldByName('acrescimo')
        .AsFloat), 2);
    end;
    //
    if not DmWorkCom.PAFECF.ImprimeItemECF
      (DmProdutos.CdsProdutos.FieldByName('Unidade').AsString,
      DmProdutos.CdsProdutos.FieldByName('ean13').AsString, aDescricao,
      aAliquota, aTipoQuant, tipoDescAcre, aQuantidade, aValor, aDesconto) then
    begin
      Result := false;
      Abort;
    end;
...
begin
  ...
  try
    ...
    begin
      Result := Result + TruncaValor(Query.FieldByName('valor').AsFloat,
        PAFECF.gDecimais);
      Query.Next;
    end;
    Query.Close;
...
begin
  ...
  try
    ...
    Query.SQL.Clear;
    Query.SQL.Add
      ('select sum(pedido_itens.valor*pedido_itens.quantidade) as valor from pedido_itens');
    Query.SQL.Add('where pedido_itens.idpedido = :pID');
    Query.SQL.Add('and pedido_itens.cancelado = ' + QuotedStr('N'));
    Query.Params.ParamByName('pID').AsInteger := pIDCupom;
    Query.Open;
...
function TDmWorkCom.RetornaAMenorDiferencaDoPercDesconto
  (pIDCupom: Integer): Double;
var
  Query: TSQLQuery;
begin
  Result := 0;
...
begin
  ...
  if Dados_Contador then
    //se contador for pessoa física
    if CdsContadorCNPJ.AsString = ''  then
    begin
          sCNPJContador := CdsContadorCPF_CONTADOR.AsString;
    end
     else
     begin
...
//rondado selecao multipla de produtos
grade_multipla :=  CdsConfigGRADE_MULTIPLA.AsString  ;
...
begin
  ...
  begin
    ...
    begin
      MessageDlg('Produto sem classificação. Verifique!!!', mtWarning,
        [mbOK], 0);
      Result := false;
    end;
    if (DmProdutos.CdsProdutos.FieldByName('TRIBUTACAO').AsInteger = 2) or
    ...
    begin
      ...
      begin
        MessageDlg('É necessário cadastrar a aliquota do produto!!!', mtWarning,
          [mbOK], 0);
        Result := false;
      end;
    end;
    ...
    begin
      MessageDlg('Produto sem preço de venda. Verifique!!!', mtWarning,
        [mbOK], 0);
      Result := false;
    end
  end;
...
begin
  ...
  if pCRZ = 0 then
  begin
    Qry60M.SQL.Add('where registro60m.data_movimento >= ' +
      QuotedStr(AuxDatai));
  ...
  begin
    ...
    Qry60M.SQL.Add('and registro60m.seriaecf = ' +
      QuotedStr(DmAcBr.ACBrECF1.NumSerie));
  end;
  // parametros
  Cds60M.FetchParams;
...
function TDmWorkCom.Dados_DocECF(pDatai, pDataf: TDateTime;
  pCaixa: String = ''): Boolean;
var
  Datai, Dataf: String;
begin
  Datai := FormatDateTime('MM/DD/YYYY', pDatai);
...
procedure TDmWorkCom.ProcessaRegistro10(pDatai, pDataf: TDateTime;
  pEstrutura, pNatureza, pFinalidade: String);
var
  Linha: String;
begin
  Linha := '10';
  Linha := Linha + ZeroEsquerda(FiltraNumero(CdsEmpresa.FieldByName('CNPJ_CPF')
    .AsString), 14);
  Linha := Linha + AlinhaTXT(CdsEmpresa.FieldByName('INSCRICAO_RG')
    .AsString, 1, 14);
  Linha := Linha + AlinhaTXT
    (Trim(Copy(CdsEmpresa.FieldByName('RAZAO_SOCIAL').AsString, 1, 35)), 1, 35);
  ...
  Linha := Linha + ZeroEsquerda(FiltraNumero(CdsEmpresa.FieldByName('TELEFAX')
    .AsString), 10);
...
begin
  Linha := '11';
  Linha := Linha + AlinhaTXT(Copy(CdsEmpresa.FieldByName('ENDERECO').AsString,
    1, 34), 1, 34);
  Linha := Linha + ZeroEsquerda(FiltraNumero(CdsEmpresa.FieldByName('NUMERO')
    .AsString), 5);
  Linha := Linha + AlinhaTXT(CdsEmpresa.FieldByName('COMPLEMENTO')
    .AsString, 1, 22);
  Linha := Linha + AlinhaTXT(Copy(CdsEmpresa.FieldByName('BAIRRO').AsString, 1,
    15), 1, 15);
  Linha := Linha + ZeroEsquerda(FiltraNumero(CdsEmpresa.FieldByName('CEP')
    .AsString), 8);
  Linha := Linha + AlinhaTXT(Copy(CdsEmpresa.FieldByName('CONTATO').AsString, 1,
    28), 1, 28);
  Linha := Linha + ZeroEsquerda(FiltraNumero(CdsEmpresa.FieldByName('TELEFONE')
    .AsString), 12);
  writeLn(sArquivo, Linha);

end;

procedure TDmWorkCom.ProcessaRegistro60m(pDatai, pDataf: TDateTime;
  p60d, p60i, p60r: Boolean);
var
...
begin
  ...
  begin
    ...
    begin
      Linha := '60M';
      Linha := Linha + FormatDateTime('YYYYMMDD',
        DmWorkCom.Cds60M.FieldByName('DATA_EMISSAO').AsDateTime);
      Linha := Linha + AlinhaTXT(DmWorkCom.Cds60M.FieldByName('SERIAECF')
        .AsString, 1, 20);
      Linha := Linha + AlinhaTXT(DmWorkCom.Cds60M.FieldByName('CAIXA')
        .AsString, 1, 3);
      Linha := Linha + '2D';
      Linha := Linha + ZeroEsquerda
        (IntToStr(DmWorkCom.Cds60M.FieldByName('COO_INICIAL').AsInteger), 6);
      Linha := Linha + ZeroEsquerda
        (IntToStr(DmWorkCom.Cds60M.FieldByName('COO_FINAL').AsInteger), 6);
      Linha := Linha + ZeroEsquerda
        (IntToStr(DmWorkCom.Cds60M.FieldByName('CONTADOR_REDUCAO')
        .AsInteger), 6);
      Linha := Linha + ZeroEsquerda
        (IntToStr(DmWorkCom.Cds60M.FieldByName('CONTADOR_REINICIO')
        .AsInteger), 3);
      aux := FormatFloat('0.00',
        DmWorkCom.Cds60M.FieldByName('VENDA_BRUTA').AsFloat);
      aux := FiltraNumero(aux);
      Linha := Linha + ZeroEsquerda(aux, 16);
      aux := FormatFloat('0.00',
        DmWorkCom.Cds60M.FieldByName('GRANDE_TOTAL').AsFloat);
      aux := FiltraNumero(aux);
      Linha := Linha + ZeroEsquerda(aux, 16);
      ...
      begin
        ...
        begin
          ...
          Linha := Linha + AlinhaTXT(DmWorkCom.Cds60M.FieldByName('SERIAECF')
            .AsString, 1, 20);
          Linha := Linha +
            AlinhaTXT(DmWorkCom.Cds60A.FieldByName('ALIQUOTA_TRIB')
            .AsString, 1, 4);
          aux := FormatFloat('0.00',
            DmWorkCom.Cds60A.FieldByName('TOTALIZADOR').AsFloat);
          aux := FiltraNumero(aux);
      ...
      begin
        ...
        Q.SQL.Clear;
        Q.SQL.Add(
          'select produtos.ean13,produtos.aliquota_icms,pedido.data_pedido,');
        Q.SQL.Add('produtos.tributacao,');
        Q.SQL.Add('sum(pedido_itens.quantidade) as quantidade,');
        Q.SQL.Add(
          'sum((pedido_itens.quantidade*pedido_itens.valor) - pedido_itens.desconto) as valor');
        Q.SQL.Add('from produtos');
        Q.SQL.Add(
          'left outer join pedido_itens on produtos.id = pedido_itens.idproduto');
        Q.SQL.Add(
          'left outer join pedido on pedido.id = pedido_itens.idpedido');
        ...
        Q.SQL.Add(
          'group by produtos.ean13,produtos.aliquota_icms,pedido.data_pedido,produtos.tributacao');
        Q.Params.ParamByName('pData').AsDate :=
          Cds60M.FieldByName('DATA_MOVIMENTO').AsDateTime;
        Q.Open;
        while not Q.Eof do
        begin
          Linha := '60D';
          Linha := Linha + FormatDateTime('YYYYMMDD',
            Q.FieldByName('data_pedido').AsDateTime);
          Linha := Linha + AlinhaTXT(DmWorkCom.Cds60M.FieldByName('SERIAECF')
            .AsString, 1, 20);
          ...
          case Q.FieldByName('tributacao').AsInteger of
            ...
            2:
              begin
                aux := FormatFloat('0.00',
                  Q.FieldByName('aliquota_icms').AsFloat);
                aux := FiltraNumero(aux);
              end;
            3:
              aux := 'N';
          ...
          if (Q.FieldByName('tributacao').AsInteger = 2) or
          ...
          //
          aux := FormatFloat('0.00',
            (Q.FieldByName('valor').AsFloat * Q.FieldByName('aliquota_icms')
            .AsFloat) / 100);
          aux := FiltraNumero(aux);
          Linha := Linha + ZeroEsquerda(aux, 13);
      ...
      begin
        ...
        Q.SQL.Clear;
        Q.SQL.Add(
          'select pedido.data_pedido,pedido.coo,pedido_itens.num_item,produtos.ean13,');
        Q.SQL.Add(
          'pedido_itens.quantidade,pedido_itens.valor,produtos.aliquota_icms,');
        ...
        begin
          ...
          Linha := Linha + '2D';
          Linha := Linha + ZeroEsquerda
            (IntToStr(Q.FieldByName('COO').AsInteger), 6);
          Linha := Linha + ZeroEsquerda
            (IntToStr(Q.FieldByName('NUM_ITEM').AsInteger), 3);
          Linha := Linha + AlinhaTXT(Q.FieldByName('EAN13').AsString, 1, 14);
          aux := FormatFloat('0.000', Q.FieldByName('quantidade').AsFloat);
          aux := FiltraNumero(aux);
          Linha := Linha + ZeroEsquerda(aux, 13);
          aValor := (Q.FieldByName('valor').AsFloat *
            ...
            Q.FieldByName('desconto').AsFloat;
      ...
      begin
        ...
        Q.SQL.Add('and pedido.caixa = ' + QuotedStr(DmWorkCom.PAFECF.gCaixa));
        Q.SQL.Add(
          'group by produtos.ean13,produtos.aliquota_icms,produtos.tributacao');
        Q.Params.ParamByName('pData').AsDate :=
          Cds60M.FieldByName('DATA_MOVIMENTO').AsDateTime;
        Q.Open;
        while not Q.Eof do
...
begin
  ...
  Q.SQL.Clear;
  Q.SQL.Add(
    'select r.data_emissao,r.modelo, r.serie, r.total,r.aliq_icms,p.tributacao,');
  ...
  begin
    ...
    Linha := Linha + AlinhaTXT('', 1, 14);
    Linha := Linha + FormatDateTime('YYYYMMDD', Q.FieldByName('DATA_EMISSAO')
      .AsDateTime);
    ...
    begin
      ...
      Linha := Linha + ZeroEsquerda(aux, 13);
      aux := FormatFloat('0.00',
        ((Q.FieldByName('total').AsFloat * Q.FieldByName('aliq_icms')
        .AsFloat) / 100));
      aux := FiltraNumero(aux);
      Linha := Linha + ZeroEsquerda(aux, 12);
      aux := FormatFloat('0.00', 0);
...
begin
  ...
  Q.SQL.Clear;
  Q.SQL.Add(
    'select produtos.ean13,registro61.descricao,registro61.quantidade,');
...
begin
  ...
  Q.SQL.Add(
    'left outer join pedido_itens on pedido.id = pedido_itens.idpedido');
...
begin
  ...
  begin
    Linha := '90';
    Linha := Linha + ZeroEsquerda
      (FiltraNumero(CdsEmpresa.FieldByName('CNPJ_CPF').AsString), 14);
    Linha := Linha + AlinhaTXT(CdsEmpresa.FieldByName('INSCRICAO_RG')
      .AsString, 1, 14);
    Linha := Linha + DmAuxiliar.CdsRegAuxVPRegistro.AsString;
    Linha := Linha + ZeroEsquerda
      (IntToStr(DmAuxiliar.CdsRegAuxVPTotal.AsInteger), 8);
...
begin
  ...
  DmAuxiliar.CdsRegAuxVPTotal.AsInteger :=
    DmAuxiliar.CdsRegAuxVPTotal.AsInteger + pTotal;
...
begin
  ...
  begin
    ...
    DataSet.FieldByName('ID_REG60M').AsInteger := Cds60M.FieldByName('ID')
      .AsInteger;
    DataSet.FieldByName('CAIXA').AsString := PAFECF.gCaixa;
...
procedure TDmWorkCom.DspContadorGetTableName(Sender: TObject; DataSet: TDataSet;
  var TableName: String);
begin
  TableName := 'CONTADOR';
...
procedure TDmWorkCom.DspOSHorasGetTableName(Sender: TObject; DataSet: TDataSet;
  var TableName: String);
begin
  TableName := 'OS_HORAS';
...
begin
  DataSet.FieldByName('TEMPO_GASTO').AsDateTime :=
    DataSet.FieldByName('HORA_INICIO').AsDateTime -
...
begin
  if DataSet.FieldByName('STATUS').AsInteger = 5 then
    DataSet.FieldByName('TEMPO_GASTO').AsDateTime :=
      RetornaHorasTrabalhada(DataSet.FieldByName('ID').AsInteger);
  if CdsOSHoras.RecordCount > 0 then
    DataSet.FieldByName('REALIZADO').AsString :=
      RetornaServicoRealizado(DataSet.FieldByName('ID').AsInteger);
...
function TDmWorkCom.RegistrosECF(fNomeArquivo: String;
  fDatai, fDataf: TDateTime; ApenasProdMovto: Boolean = false): Boolean;
begin
  Result := true;
...
begin
  try
    ...
    begin
      ...
      xCdsRelCaixa.FieldByName('DESCRICAO').AsString :=
        UpperCase(DmAcBr.ACBrECF1.FormasPagamento[i].Descricao);
      xCdsRelCaixa.FieldByName('VL_INFORMADO').AsFloat := 0;
      xCdsRelCaixa.FieldByName('VL_IMPRESSORA').AsFloat :=
        DmAcBr.ACBrECF1.FormasPagamento[i].Total;
    ...
    begin
      if (UpperCase(DmAcBr.ACBrECF1.ComprovantesNaoFiscais[i].Descricao)
        = 'SANGRIA') or
        (UpperCase(DmAcBr.ACBrECF1.ComprovantesNaoFiscais[i].Descricao)
        = 'SUPRIMENTO') then
      begin
        xCdsRelCaixa.Append;
        ...
        xCdsRelCaixa.FieldByName('VL_INFORMADO').AsFloat := 0;
        xCdsRelCaixa.FieldByName('VL_IMPRESSORA').AsFloat :=
          DmAcBr.ACBrECF1.ComprovantesNaoFiscais[i].Total;
        xCdsRelCaixa.FieldByName('VL_DIVERGENTE').AsFloat := 0;
        xCdsRelCaixa.Post;
      end;
    end;
  ...
  try
    ...
    begin
      ...
      begin
        xCdsRelCaixa.Edit;
        xCdsRelCaixa.FieldByName('VL_INFORMADO').AsFloat :=
          Q.FieldByName('SUM').AsFloat;
        xCdsRelCaixa.FieldByName('VL_DIVERGENTE').AsFloat :=
          TruncaValor(xCdsRelCaixa.FieldByName('VL_INFORMADO').AsFloat, 2) -
  ...
  begin
    ...
    begin
      ...
      AuxSTR := 'Informado: ' + FormatFloat('0.00',
        xCdsRelCaixa.FieldByName('VL_INFORMADO').AsFloat) + ' - ' +
        'Impressora: ' + FormatFloat('0.00',
        xCdsRelCaixa.FieldByName('VL_IMPRESSORA').AsFloat) + ' - ' +
        'Divergente: ' + FormatFloat('0.00',
        xCdsRelCaixa.FieldByName('VL_DIVERGENTE').AsFloat);
      PAFECF.ImprimeRelatorioGerencial(AuxSTR);
      xCdsRelCaixa.Next;
    end;
...
begin
  ...
  begin
    ...
    if not VerificaMD5Registro(4, DmWorkCom.CdsDadosTecnicos.FieldByName('ID')
      .AsInteger) then
      ModeloECF := RetornaCampoInterroga(ModeloECF, 20);
    //
    Linha := 'R01';
    Linha := Linha + AlinhaTXT(CdsPDVaux.FieldByName('SERIAL_ECF').AsString, 1,
      20); // 20 espacos
    if Trim(CdsPDVaux.FieldByName('LETRA_ADD').AsString) <> '' then
      Linha := Linha + CdsPDVaux.FieldByName('LETRA_ADD').AsString
    else
      Linha := Linha + ' ';
    Linha := Linha + AlinhaTXT(CdsPDVaux.FieldByName('TIPO_ECF')
      .AsString, 1, 7);
    Linha := Linha + AlinhaTXT(CdsPDVaux.FieldByName('MARCA_ECF')
      .AsString, 1, 20);
    Linha := Linha + AlinhaTXT(ModeloECF, 1, 20);
    Linha := Linha + AlinhaTXT(CdsPDVaux.FieldByName('VERSAO_ECF')
      .AsString, 1, 10);
    if CdsPDVaux.FieldByName('DATA_SB').AsDateTime > 0 then
      Linha := Linha + FormatDateTime('YYYYMMDD',
        CdsPDVaux.FieldByName('DATA_SB').AsDateTime)
    else
      Linha := Linha + '        ';
    if CdsPDVaux.FieldByName('HORA_SB').AsDateTime > 0 then
      Linha := Linha + FormatDateTime('HHMMSS', CdsPDVaux.FieldByName('HORA_SB')
        .AsDateTime)
    else
      Linha := Linha + '      ';
    Linha := Linha + AlinhaTXT(CdsPDVaux.FieldByName('CAIXA').AsString, 1, 3);
    Linha := Linha + ZeroEsquerda(CdsEmpresa.FieldByName('CNPJ_CPF')
      .AsString, 14);
    Linha := Linha + AlinhaTXT(CdsEmpresa.FieldByName('INSCRICAO_RG')
      .AsString, 1, 14);
    Linha := Linha + ZeroEsquerda(DmWorkCom.CdsDadosTecnicosCNPJ.AsString, 14);
    Linha := Linha + ZeroEsquerda
      (DmWorkCom.CdsDadosTecnicosINSCEST.AsString, 14);
    Linha := Linha + ZeroEsquerda
      (DmWorkCom.CdsDadosTecnicosINSCMUN.AsString, 14);
    Linha := Linha + AlinhaTXT(Copy(DmWorkCom.CdsDadosTecnicosEMPRESA.AsString,
      1, 40), 1, 40);
    Linha := Linha + AlinhaTXT
      (DmWorkCom.CdsDadosTecnicosDENOMINACAO.AsString, 1, 40);
    Linha := Linha + AlinhaTXT
      (DmWorkCom.CdsDadosTecnicosVERSAO.AsString, 1, 10);
    Linha := Linha + AlinhaTXT(DmWorkCom.CdsDadosTecnicosMD5.AsString, 1, 32);
    Linha := Linha + FormatDateTime('YYYYMMDD', pDatai);
    Linha := Linha + FormatDateTime('YYYYMMDD', pDataf);
    Linha := Linha + AlinhaTXT
      (FiltraNumero(DmWorkCom.CdsDadosTecnicosVERSAO_ERPAFECF.AsString), 1, 4);
...
begin
  ...
  begin
    ...
    begin
      ...
      if not VerificaMD5Registro(5, DmWorkCom.Cds60M.FieldByName('ID').AsInteger)
      then
        ModeloECF := RetornaCampoInterroga(ModeloECF, 20);
      //
      Linha := 'R02';
      Linha := Linha + AlinhaTXT(Cds60M.FieldByName('SERIAECF')
        .AsString, 1, 20);
      if Trim(DmWorkCom.PAFECF.pLetraMFAds) <> '' then
        Linha := Linha + DmWorkCom.PAFECF.pLetraMFAds
      else
      ...
      Linha := Linha + '01';
      Linha := Linha + ZeroEsquerda
        (IntToStr(Cds60M.FieldByName('Contador_Reducao').AsInteger), 6);
      Linha := Linha + ZeroEsquerda(IntToStr(Cds60M.FieldByName('COO_Final')
        .AsInteger), 9);
      Linha := Linha + ZeroEsquerda
        (IntToStr(Cds60M.FieldByName('Contador_Reinicio').AsInteger), 6);
      Linha := Linha + FormatDateTime('YYYYMMDD',
        Cds60M.FieldByName('Data_Movimento').AsDateTime);
      Linha := Linha + FormatDateTime('YYYYMMDD',
        Cds60M.FieldByName('Data_emissao').AsDateTime);
      Linha := Linha + FormatDateTime('HHMMSS',
        Cds60M.FieldByName('Hora_emissao').AsDateTime);
      Linha := Linha + ZeroEsquerda
        (FiltraNumero(FormatFloat('0.00', Cds60M.FieldByName('VENDA_BRUTA')
        .AsFloat)), 14);
      Linha := Linha + 'N';
      writeLn(sArquivo, Linha);
      Cds60M.Next;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if not VerificaMD5Registro(6, Cds60M.FieldByName('ID').AsInteger,
          Cds60A.FieldByName('ID').AsInteger) then
          ModeloECF := RetornaCampoInterroga(ModeloECF, 20);
        Inc(Contador);
        Linha := 'R03';
        Linha := Linha + AlinhaTXT(Cds60A.FieldByName('SERIAECF')
          .AsString, 1, 20);
        if Trim(DmWorkCom.PAFECF.pLetraMFAds) <> '' then
          Linha := Linha + DmWorkCom.PAFECF.pLetraMFAds
        else
        ...
        begin
          Linha := Linha + Cds60A.FieldByName('ALIQUOTA_TRIB').AsString +
            ZeroEsquerda(IntToStr(Contador), 2) + '    ';
        end
        else if (UpperCase(Cds60A.FieldByName('ALIQUOTA_TRIB').AsString)
          = 'DESC') then
          Linha := Linha + 'DT     '
        else if (UpperCase(Cds60A.FieldByName('ALIQUOTA_TRIB').AsString)
          = 'ACRE') then
          Linha := Linha + 'AT     '
        else if (UpperCase(Cds60A.FieldByName('ALIQUOTA_TRIB').AsString) = 'AT')
        then
          Linha := Linha + 'AT     '
        else if (UpperCase(Cds60A.FieldByName('ALIQUOTA_TRIB').AsString)
          = 'CANC') then
          ...
          Linha := Linha + ZeroEsquerda(IntToStr(Contador), 2) + 'T' +
            ZeroEsquerda(Copy(Cds60A.FieldByName('ALIQUOTA_TRIB')
            .AsString, 1, 2), 4);
        Linha := Linha + ZeroEsquerda
          (FiltraNumero(FormatFloat('0.00', Cds60A.FieldByName('TOTALIZADOR')
          .AsFloat)), 13);
        writeLn(sArquivo, Linha);
        Cds60A.Next;
      end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if not VerificaMD5Registro(0, DmWorkCom.CdsPedidos.FieldByName('ID')
        .AsInteger) then
        ModeloECF := RetornaCampoInterroga(ModeloECF, 20);
      Linha := 'R04';
      ...
      Linha := Linha + '01';
      Linha := Linha + ZeroEsquerda
        (IntToStr(CdsPedidos.FieldByName('CCF').AsInteger), 9);
      Linha := Linha + ZeroEsquerda
        (IntToStr(CdsPedidos.FieldByName('COO').AsInteger), 9);
      Linha := Linha + FormatDateTime('YYYYMMDD',
        CdsPedidos.FieldByName('Data_entrega').AsDateTime);
      Linha := Linha + ZeroEsquerda
        (FiltraNumero(FormatFloat('0.00', CdsPedidos.FieldByName('valor')
        .AsFloat)), 14);
      Linha := Linha + ZeroEsquerda
        (FiltraNumero(FormatFloat('0.00', CdsPedidos.FieldByName('desconto')
        .AsFloat)), 13);
      if CdsPedidos.FieldByName('desconto').AsFloat > 0 then
        ...
        Linha := Linha + ' ';
      Linha := Linha + ZeroEsquerda
        (FiltraNumero(FormatFloat('0.00', CdsPedidos.FieldByName('acrescimo')
        .AsFloat)), 13);
      if CdsPedidos.FieldByName('acrescimo').AsFloat > 0 then
        ...
        Linha := Linha + ' ';
      Linha := Linha + ZeroEsquerda
        (FiltraNumero(FormatFloat('0.00', CdsPedidos.FieldByName('valor_total')
        .AsFloat)), 14);
      ...
      begin
        Linha := Linha + AlinhaTXT(CdsPedidos.FieldByName('cliente').AsString,
          1, 40); // 40 ESPACOS
        Linha := Linha + ZeroEsquerda(CdsPedidos.FieldByName('cnpjcpf')
          .AsString, 14); // 14 ESPACOS
      end
      else
      begin
  ...
  begin
    ...
    Linha := Linha + AlinhaTXT('', 1, 20);
    Linha := Linha + '00';
    Linha := Linha + ZeroEsquerda(DmAuxiliar.CdsGeral.FieldByName('NUM_ORDEM')
      .AsString, 9);
    Linha := Linha + ZeroEsquerda(IntToStr(0), 9);
    Linha := Linha + FormatDateTime('YYYYMMDD',
      DmAuxiliar.CdsGeral.FieldByName('DATA_EMISSAO').AsDateTime);
    Linha := Linha + ZeroEsquerda(FiltraNumero(FormatFloat('0.00',
      DmAuxiliar.CdsGeral.FieldByName('VALOR').AsFloat)), 14);
    ...
    Linha := Linha + ZeroEsquerda('0', 13);
    Linha := Linha + ' ';
    Linha := Linha + AlinhaTXT(DmAuxiliar.CdsGeral.FieldByName('nome').AsString,
      1, 40); // 40 ESPACOS
    Linha := Linha + AlinhaTXT(DmAuxiliar.CdsGeral.FieldByName('cnpj_cpf')
      .AsString, 1, 14); // 14 ESPACOS
    writeLn(sArquivo, Linha);
  ...
  begin
    ...
    DmWorkCom.Dados_PedidoItens(pDatai, pDataf, CdsPDVauxCAIXA.AsString,
      0, false);
    while not CdsPedidoItens.Eof do
    begin
      ModeloECF := DmWorkCom.PAFECF.pModeloECF;
      if not VerificaMD5Registro(1, DmWorkCom.CdsPedidoItens.FieldByName
        ('IDPEDIDO').AsInteger, DmWorkCom.CdsPedidoItens.FieldByName('ID')
        .AsInteger) then
        ModeloECF := RetornaCampoInterroga(ModeloECF, 20);
      Inc(Contador);
      DmProdutos.Dados_Produto(CdsPedidoItens.FieldByName('IDPRODUTO')
        .AsInteger);
      Linha := 'R05';
      Linha := Linha + AlinhaTXT(DmWorkCom.PAFECF.pSerial, 1, 20);
      if Trim(DmWorkCom.PAFECF.pLetraMFAds) <> '' then
      ...
      Linha := Linha + AlinhaTXT(ModeloECF, 1, 20);
      Linha := Linha + '01';
      Linha := Linha + ZeroEsquerda(IntToStr(CdsPedidoItens.FieldByName('COO_1')
        .AsInteger), 9);
      Linha := Linha + ZeroEsquerda(IntToStr(CdsPedidoItens.FieldByName('CCF_1')
        .AsInteger), 9);
      Linha := Linha + ZeroEsquerda
        (IntToStr(CdsPedidoItens.FieldByName('NUM_ITEM').AsInteger), 3);
      Linha := Linha + AlinhaTXT(CdsPedidoItens.FieldByName('EAN13').AsString,
        1, 14); // 14 espacos
      Linha := Linha + AlinhaTXT(CdsPedidoItens.FieldByName('DESCRICAO')
        .AsString, 1, 100); // 100 espacos
      Linha := Linha + ZeroEsquerda
        (FiltraNumero(FormatFloat('0.000',
        CdsPedidoItens.FieldByName('QUANTIDADE').AsFloat)), 7);
      Linha := Linha + AlinhaTXT(DmProdutos.CdsProdutos.FieldByName('UNIDADE')
        .AsString, 1, 3); // 3 espaços
      Linha := Linha + ZeroEsquerda
        (FiltraNumero(FormatFloat('0.00', CdsPedidoItens.FieldByName('VALOR')
        .AsFloat)), 8);
      Linha := Linha + ZeroEsquerda
        (FiltraNumero(FormatFloat('0.00', CdsPedidoItens.FieldByName('DESCONTO')
        .AsFloat)), 8);
      Linha := Linha + ZeroEsquerda
        (FiltraNumero(FormatFloat('0.00',
        CdsPedidoItens.FieldByName('ACRESCIMO').AsFloat)), 8);
      Linha := Linha + ZeroEsquerda
        (FiltraNumero(FormatFloat('0.00',
        CdsPedidoItens.FieldByName('VALOR_TOTAL').AsFloat)), 14);
      // Totalizador parcial
      ...
      begin
        ...
        else if (DmProdutos.CdsProdutos.FieldByName('TRIBUTACAO').AsInteger = 1)
        then
          Linha := Linha + 'F' + ZeroEsquerda(IntToStr(Contador), 2) + '    '
        else if (DmProdutos.CdsProdutos.FieldByName('TRIBUTACAO').AsInteger = 3)
        then
          Linha := Linha + 'N' + ZeroEsquerda(IntToStr(Contador), 2) + '    '
        else if (DmProdutos.CdsProdutos.FieldByName('TRIBUTACAO').AsInteger = 2)
        then
        begin
          if CdsTotalizadores.Locate('ALIQUOTA',
            DmProdutos.CdsProdutos.FieldByName('ALIQUOTA_ICMS').AsFloat, [])
          then
            Linha := Linha + CdsTotalizadores.FieldByName('TOTALIZADOR')
              .AsString + 'T' + ZeroEsquerda
              (FiltraNumero(FormatFloat('0.00',
              DmProdutos.CdsProdutos.FieldByName('ALIQUOTA_ICMS').AsFloat)), 4)
          else
            Linha := Linha + '00' + 'T' +
              ZeroEsquerda(FiltraNumero(FormatFloat('0.00',
              DmProdutos.CdsProdutos.FieldByName('ALIQUOTA_ICMS').AsFloat)), 4)
        end
        else if (DmProdutos.CdsProdutos.FieldByName('TRIBUTACAO').AsInteger = 4)
        then
  ...
  begin
    ...
    Inc(aNumItemNF);
    Inc(Contador);
    DmProdutos.Dados_Produto(DmAuxiliar.CdsGeral.FieldByName('IDPRODUTO')
      .AsInteger);
    Linha := 'R05';
    Linha := Linha + AlinhaTXT('', 1, 20);
    Linha := Linha + ' ';
    ...
    Linha := Linha + AlinhaTXT(DmAuxiliar.CdsGeral.FieldByName('IDPRODUTO')
      .AsString, 1, 14); // 14 espacos
    Linha := Linha + AlinhaTXT(DmAuxiliar.CdsGeral.FieldByName('DESCRICAO')
      .AsString, 1, 100); // 100 espacos
    Linha := Linha + ZeroEsquerda(FiltraNumero(FormatFloat('0.00',
      DmAuxiliar.CdsGeral.FieldByName('QUANTIDADE').AsFloat)), 7);
    Linha := Linha + AlinhaTXT(Trim(DmAuxiliar.CdsGeral.FieldByName('UNIDADE')
      .AsString), 1, 3); // 3 espaços
    Linha := Linha + ZeroEsquerda(FiltraNumero(FormatFloat('0.00',
      DmAuxiliar.CdsGeral.FieldByName('VALOR').AsFloat)), 8);
    Linha := Linha + ZeroEsquerda(FiltraNumero(FormatFloat('0.00', 0)), 8);
    Linha := Linha + ZeroEsquerda(FiltraNumero(FormatFloat('0.00', 0)), 8);
    Linha := Linha + ZeroEsquerda(FiltraNumero(FormatFloat('0.00',
      DmAuxiliar.CdsGeral.FieldByName('TOTAL').AsFloat)), 14);
    // Totalizador parcial
    ...
    begin
      if CdsTotalizadores.Locate('ALIQUOTA',
        DmAuxiliar.CdsGeral.FieldByName('ALIQ_ICMS').AsFloat, []) then
        Linha := Linha + CdsTotalizadores.FieldByName('TOTALIZADOR').AsString +
          'T' + ZeroEsquerda(FiltraNumero(FormatFloat('0.00',
          DmAuxiliar.CdsGeral.FieldByName('ALIQ_ICMS').AsFloat)), 4)
      else
        Linha := Linha + '00' + 'T' +
          ZeroEsquerda(FiltraNumero(FormatFloat('0.00',
          DmAuxiliar.CdsGeral.FieldByName('ALIQ_ICMS').AsFloat)), 4)
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        NumSerie := RetornaSerialECF(CdsControleDoc.FieldByName('COO')
          .AsInteger, CdsControleDoc.FieldByName('DATA').AsDateTime);
        ModeloECF := RetornaModeloECF(NumSerie);
        if not VerificaMD5Registro(7, DmWorkCom.CdsControleDoc.FieldByName('ID')
          .AsInteger) then
          ModeloECF := RetornaCampoInterroga(ModeloECF, 20);
        Linha := 'R06';
        ...
        Linha := Linha + '01';
        Linha := Linha + ZeroEsquerda
          (IntToStr(CdsControleDoc.FieldByName('COO').AsInteger), 9);
        Linha := Linha + ZeroEsquerda
          (IntToStr(CdsControleDoc.FieldByName('GNF').AsInteger), 6);
        if (CdsControleDoc.FieldByName('SIGLA').AsString <> 'CN') and
          (CdsControleDoc.FieldByName('SIGLA').AsString <> 'CC') then
          Linha := Linha + ZeroEsquerda
            (IntToStr(CdsControleDoc.FieldByName('GRG').AsInteger), 6)
        else
          Linha := Linha + ZeroEsquerda('', 6);
        Linha := Linha + ZeroEsquerda
          (IntToStr(CdsControleDoc.FieldByName('CDC').AsInteger), 4);
        Linha := Linha + CdsControleDoc.FieldByName('SIGLA').AsString;
        Linha := Linha + FormatDateTime('YYYYMMDD',
          CdsControleDoc.FieldByName('DATA').AsDateTime);
        Linha := Linha + FormatDateTime('HHMMSS',
          CdsControleDoc.FieldByName('HORA').AsDateTime);
        writeLn(sArquivo, Linha);
      end;
      CdsControleDoc.Next;
    end;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ModeloECF := DmWorkCom.PAFECF.pModeloECF;
        if not VerificaMD5Registro(8, CdsPedidos.FieldByName('ID').AsInteger,
          CdsPedidoFP.FieldByName('ID').AsInteger) then
          ModeloECF := RetornaCampoInterroga(ModeloECF, 20);
        //
        Linha := 'R07';
        Linha := Linha + AlinhaTXT(DmWorkCom.PAFECF.pSerial, 1, 20);
        ...
        Linha := Linha + AlinhaTXT(CdsPedidoFP.FieldByName('DESCRICAO')
          .AsString, 1, 15); // 15 espacos
        Linha := Linha + ZeroEsquerda
          (FiltraNumero(FormatFloat('0.00', CdsPedidoFP.FieldByName('valor')
          .AsFloat - CdsPedidoFP.FieldByName('troco').AsFloat)), 13);
        Linha := Linha + CdsPedidos.FieldByName('CANCELADO').AsString;
...
function TDmWorkCom.Le_Linha_Arquivo(fTexto, fCaracter: string;
  fPosicao: Integer): string;
var
  aux: string;
  i, Contador: Integer;
...
procedure TDmWorkCom.DspPedidosGetTableName(Sender: TObject; DataSet: TDataSet;
  var TableName: String);
begin
  TableName := 'PEDIDO'
...
procedure TDmWorkCom.DspPedidoFPGetTableName(Sender: TObject; DataSet: TDataSet;
  var TableName: String);
begin
...
begin
  ...
  DataSet.FieldByName('IDPEDIDO').AsInteger := CdsPedidos.FieldByName('ID')
    .AsInteger;
  DataSet.FieldByName('VALOR').AsFloat := 0;
...
function TDmWorkCom.VerificaMD5Registro(pTipo, pID: Integer;
  pID1: Integer = 0): Boolean;
var
...
begin
  ...
  DataSet.FieldByName('DATA_EMISSAO').AsDateTime :=
    Cds60M.FieldByName('DATA_EMISSAO').AsDateTime;
  DataSet.FieldByName('HORA_EMISSAO').AsDateTime :=
    Cds60M.FieldByName('HORA_EMISSAO').AsDateTime;
  DataSet.FieldByName('DATA_MOVIMENTO').AsDateTime :=
    Cds60M.FieldByName('DATA_MOVIMENTO').AsDateTime;
  DataSet.FieldByName('SERIAECF').AsString :=
    Cds60M.FieldByName('SERIAECF').AsString;
  DataSet.FieldByName('CONTADOR_REINICIO').AsInteger :=
    Cds60M.FieldByName('CONTADOR_REINICIO').AsInteger;
  DataSet.FieldByName('CONTADOR_REDUCAO').AsInteger :=
    Cds60M.FieldByName('CONTADOR_REDUCAO').AsInteger;
  // grava MD5 do sainota
  MD5 := RetornaMD5Registro(DataSet);
  DataSet.FieldByName('MD5REGISTRO').AsString := MD5;
...
begin
  ...
  try
    ...
    Query.Open;
    Result := Result + TruncaValor(Query.FieldByName('total').AsFloat,
      PAFECF.gDecimais);
    Query.Close;
  finally
    Query.Free;
...
procedure TDmWorkCom.DspMesasGetTableName(Sender: TObject; DataSet: TDataSet;
  var TableName: String);
begin
  TableName := 'MESAS';
...
begin
  ...
  if pDataBase > 0 then
    QryMesasAbertas.SQL.Add('and mesas_abertas.data <= ' +
      QuotedStr(aDataBase));
  //
...
begin
  ...
  DataSet.FieldByName('ID_MESA').AsInteger := CdsMesasAbertas.FieldByName('ID')
    .AsInteger;
  DataSet.FieldByName('CANCELADO').AsString := 'N';
  ...
  try
    ...
    begin
      ...
      begin
        ...
        PAFECF.ImprimeRelatorioGerencial
          ('      * * * Conferencia de Mesa * * *      ');
        aux := 'Data: ' + FormatDateTime('DD/MM/YYYY', Mesa.FieldByName('DATA')
          .AsDateTime) + ' - ' + 'Hora: ' + FormatDateTime('HH:MM:SS',
          Mesa.FieldByName('HORA').AsDateTime);
        ...
        begin
          aux := ZeroEsquerda(IntToStr(Item.FieldByName('ID_PRODUTO')
            .AsInteger), 5) + ' - ' +
            Copy(Item.FieldByName('DESCRICAO').AsString, 1, 32);
          PAFECF.ImprimeRelatorioGerencial(aux);
          aux := FormatFloat('0.000', Item.FieldByName('QUANT').AsFloat) +
            '   x   ' + FormatFloat('0.000', Item.FieldByName('PRECO_UNIT')
            .AsFloat) + '   =   ' + FormatFloat('0.000',
            Item.FieldByName('PRECO_TOTAL').AsFloat);
          PAFECF.ImprimeRelatorioGerencial(aux);
          if Item.FieldByName('CANCELADO').AsString = 'S' then
          begin
            aux := 'Produto ' + ZeroEsquerda
              (IntToStr(Item.FieldByName('ID_PRODUTO').AsInteger), 5) +
              ' - Cancelado';
            PAFECF.ImprimeRelatorioGerencial(aux);
          end;
          ...
          begin
            aux := 'Produto ' + ZeroEsquerda
              (IntToStr(Item.FieldByName('ID_PRODUTO').AsInteger), 5) +
              ' - Transf.da mesa: ' + Item.FieldByName
              ('NUMERO_MESA_ORIG').AsString;
            PAFECF.ImprimeRelatorioGerencial(aux);
          end;
          Item.Next;
        end;
        aux := '----------------------------------------';
        PAFECF.ImprimeRelatorioGerencial(aux);
        aux := 'Total da Mesa: ' + FormatFloat('0.00',
          Mesa.FieldByName('VALOR').AsFloat);
        PAFECF.ImprimeRelatorioGerencial(aux);
        aux := '----------------------------------------';
        PAFECF.ImprimeRelatorioGerencial(aux);
...
var
  Mesa, Item: TSQLQuery;
  aux: String;
  ValorTotalItem, ValorCarga,ValorCargaFed,ValorCargaEst,ValorCargaMun: Double;
begin
  Result := 0;
  Mesa := TSQLQuery.Create(nil);
  Item := TSQLQuery.Create(nil);
  try
    ...
    begin
      // grava pedido
      CdsPedidos.Append;
      if DmClientes.Dados_Contato(CdsPedidos.FieldByName('IDCLIENTE').AsInteger)
      then
        PreencheDadosCliente(0);
      CdsPedidos.FieldByName('DATA_PEDIDO').AsDateTime :=
        Mesa.FieldByName('DATA').AsDateTime;
      CdsPedidos.FieldByName('HORA_PEDIDO').AsDateTime :=
        Mesa.FieldByName('HORA').AsDateTime;
      CdsPedidos.FieldByName('DATA_ENTREGA').AsDateTime :=
        Mesa.FieldByName('DATA').AsDateTime;
      CdsPedidos.FieldByName('HORA_ENTREGA').AsDateTime :=
        Mesa.FieldByName('HORA').AsDateTime;
      CdsPedidos.FieldByName('VALOR').AsFloat :=
        Mesa.FieldByName('VALOR').AsFloat;
      CdsPedidos.FieldByName('VALOR_TOTAL').AsFloat :=
        Mesa.FieldByName('VALOR').AsFloat;
      CdsPedidos.FieldByName('CER').AsInteger :=
        Mesa.FieldByName('CER_CONFERENCIA').AsInteger;
      CdsPedidos.FieldByName('COO_CONF_MESA').AsInteger :=
        Mesa.FieldByName('COO').AsInteger;
      CdsPedidos.FieldByName('ID_MESA').AsInteger := Mesa.FieldByName('ID')
        .AsInteger;
      Grava_Dados(CdsPedidos);
      ...
      if dmWorkCom.PAFECF.gAgrupaVenda then
        begin
           Item.Close;
           Item.SQL.Clear;
           Item.SQL.Add('select mesas_itens.id_mesa, mesas_itens.id_produto, mesas_itens.id_produto, produtos.descricao, sum(mesas_itens.quant) as quant, ');
           Item.SQL.Add(' mesas_itens.preco_unit, sum(mesas_itens.preco_total) as preco_total, mesas_itens.cancelado ');
           Item.SQL.Add('from mesas_itens inner join produtos on (mesas_itens.id_produto = produtos.id) ');
           Item.SQL.Add('where mesas_itens.id_mesa = '+ intToSTR(pIDMesa));
           Item.SQL.Add('and mesas_itens.cancelado <> '+QuotedStr('S'));
           Item.SQL.Add('group by mesas_itens.id_produto,   produtos.descricao,mesas_itens.id_mesa, mesas_itens.id_produto, mesas_itens.preco_unit, mesas_itens.cancelado') ;
           Item.SQL.Add(' order by mesas_itens.id_produto ') ;
           Item.Open;
        end
      else
      begin
      ...
      while not Item.Eof do
        ...
        //rondado insere no cupom - tela de vendas
        InsereItemCupom(Item.FieldByName('QUANT').AsFloat, 0, 0);

        CdsPedidoItens.Edit;
        CdsPedidoItens.FieldByName('VALOR').AsFloat :=
          Item.FieldByName('PRECO_UNIT').AsFloat;
        CdsPedidoItens.FieldByName('VALOR_TOTAL').AsFloat :=
          Item.FieldByName('PRECO_TOTAL').AsFloat;
        CdsPedidoItens.FieldByName('CANCELADO').AsString :=
          Item.FieldByName('CANCELADO').AsString;
        Grava_Dados(CdsPedidoItens);
        //
        ValorTotalItem :=
          TruncaValor((((CdsPedidoItens.FieldByName('QUANTIDADE').AsFloat *
          CdsPedidoItens.FieldByName('VALOR').AsFloat) -
          CdsPedidoItens.FieldByName('DESCONTO').AsFloat) +
          CdsPedidoItens.FieldByName('ACRESCIMO').AsFloat),
       ...
       // ValorCarga :=((ValorTotalItem * DmProdutos.CdsNCM.FieldByName('ALIQ_NACIONAL').AsFloat) / 100);
        ValorCarga:= ValorCargaFed+  ValorCargaEst;
        // grava no pedido
        if not(CdsPedidos.State in [dsInsert, dsEdit]) then
          CdsPedidos.Edit;
...
begin
  ...
  begin
    ...
    begin
      DataSet.FieldByName('TRANSFERIDA').AsString := 'Mesa Transf. ' +
        DataSet.FieldByName('NUMERO_MESA_ORIG').AsString;
    end;
  end;
end;
...
begin
  ...
  if pTipo = 0 then
    QryDavPV.SQL.Add('and (p.numdav <> ' + QuotedStr('') + ' or p.numpv <> ' +
      QuotedStr('') + ')')
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            DmClientes.Dados_Contato(CdsPedidos.FieldByName('IDCLIENTE')
              .AsInteger);
            ...
            begin
              ...
              Endereco := DmClientes.CdsContatos.FieldByName
                ('END_ENTREGA').AsString;
              Numero := DmClientes.CdsContatos.FieldByName
                ('NUM_END_ENTREGA').AsString;
              Complemento := DmClientes.CdsContatos.FieldByName
                ('COMPL_ENT').AsString;
              Bairro := DmClientes.CdsContatos.FieldByName
                ('BAIRRO_END_ENTREGA').AsString;
              Cidade := DmClientes.CdsContatos.FieldByName
                ('CID_END_ENTREGA').AsString;
              Cep := DmClientes.CdsContatos.FieldByName
                ('CEP_END_ENTREGA').AsString;
              UF := DmClientes.CdsContatos.FieldByName('UF_END_ENT').AsString;
              CPF := DmClientes.CdsContatos.FieldByName('CNPJ_CPF').AsString;
              Identidade := DmClientes.CdsContatos.FieldByName
                ('INSC_RG').AsString;
              Placa := '';
              Odometro := '';
            end;
            ...
            begin
              ...
              if DmWorkCom.RegistraItemECF
                (FloatToStr(CdsPedidoItens.FieldByName('Quantidade').AsFloat))
              then
              begin
                ...
                if CdsPedidoItens.FieldByName('CANCELADO').AsString = 'S' then
                begin
                  aNumItem := IntToStr(CdsPedidoItens.FieldByName('NUM_ITEM')
                    .AsInteger);
            ...
            CdsPedidoFP.FieldByName('VALOR').AsFloat :=
              CdsPedidos.FieldByName('VALOR_TOTAL').AsFloat;
            CdsPedidoFP.FieldByName('TROCO').AsFloat := 0;
            CdsPedidoFP.FieldByName('ID_FORMAPAG_CUPOM').AsInteger :=
            ...
            PAFECF.RegistraFormaPagtoCupom(DmWorkCom.PAFECF.gDINHEIRO,
              TruncaValor(CdsPedidos.FieldByName('VALOR_TOTAL').AsFloat,
              DmWorkCom.PAFECF.gDecimais));
            ...
            PercCarga :=
              ((CdsPedidos.FieldByName('VALOR_CARGA_TRIB').AsFloat * 100) /
              CdsPedidos.FieldByName('VALOR_TOTAL').AsFloat);
            ...
            begin
              ...
              aCodMinasLegal :=
                Copy(FiltraNumero(DmWorkCom.CdsEmpresaCNPJ_CPF.AsString), 1, 8);
              aCodMinasLegal := aCodMinasLegal + FormatDateTime('DDMMYYYY',
                CdsPedidos.FieldByName('DATA_ENTREGA').AsDateTime);
              aCodMinasLegal := aCodMinasLegal +
                FloatToStr(TruncaValor(CdsPedidos.FieldByName('VALOR_TOTAL')
                .AsFloat, 0));
              aMensagem := aMensagem + 'MINAS LEGAL: ' + aCodMinasLegal +
                #13 + #10;
            end;
            // verifica opção de Minas Legal
            ...
            begin
              aMensagem := aMensagem + 'ESTABELECIMENTO INCLUIDO NO PROGRAMA DE'
                + #13 + #10;
              aMensagem := aMensagem + 'CONCESSAO DE CREDITOS - LEI Nº 4.159/08'
                + #13 + #10;
              aMensagem := aMensagem + 'NOTA LEGAL: ICMS: ' +
                FormatFloat('0.00', PercCarga) + #13 + #10;
            end;
            if DmWorkCom.CdsEmpresaUF.AsString = 'PB' then
            begin
              aCodCupomMania :=
                Copy(FiltraNumero(DmWorkCom.CdsEmpresaCNPJ_CPF.AsString), 1,
                14) + ' ';
              aCodCupomMania := aCodCupomMania + FormatDateTime('DDMMYYYY',
                CdsPedidos.FieldByName('DATA_ENTREGA').AsDateTime) + ' ';
              aCodCupomMania := aCodCupomMania +
                FiltraNumero(FormatFloat('0.00',
                CdsPedidos.FieldByName('VALOR_TOTAL').AsFloat)) + ' ';
              aCodCupomMania := aCodCupomMania +
                FiltraNumero(CdsPedidos.FieldByName('CNPJ_CPF').AsString);
              aMensagem := aMensagem + 'PARAIBA LEGAL - RECEITA CIDADÃ' +
                #13 + #10;
              aMensagem := aMensagem + 'TORPEDO PREMIADO: ' + aCodCupomMania
                + #13 + #10;
            end;
            // exibe carga tributaria
             aMensagem := aMensagem + 'Val Aprox trib. Fed. R$' +
              FormatFloat(DmWorkCom.PAFECF.gMascaraDec,
              CdsPedidos.FieldByName('VAL_CARGA_FED').AsFloat) + ' Est. R$' +
              FormatFloat(DmWorkCom.PAFECF.gMascaraDec,
              CdsPedidos.FieldByName('VAL_CARGA_EST').AsFloat) + ' Mun. R$' +
              FormatFloat(DmWorkCom.PAFECF.gMascaraDec,
              CdsPedidos.FieldByName('VAL_CARGA_MUN').AsFloat) + ' Fonte IBPT'
              + #13 + #10;
...
function TDmWorkCom.IncluirCupomExecute(var Mensagem: String; pDAV: Boolean;
  pPV: Boolean = false): Boolean;
var
  aMensagem: String;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        begin
          MessageDlg('Impressora já esta em estado de venda. Verifique!',
            mtWarning, [mbOK], 0);
          Result := false;
          Exit;
        end;
    ...
    begin
      ...
      begin
        ...
        CdsPedidos.FieldByName('NUM_END_ENTREGA').AsString :=
          ClienteCupom.Numero;
        CdsPedidos.FieldByName('COMPL_END_ENTREGA').AsString :=
          ClienteCupom.Complemento;
        CdsPedidos.FieldByName('BAIRRO_END_ENTREGA').AsString :=
          ClienteCupom.Bairro;
        CdsPedidos.FieldByName('CID_END_ENTREGA').AsString :=
          ClienteCupom.Cidade;
  ...
  begin
    ...
    begin
      ...
      begin
        MessageDlg('Verifique a existencia de cupom fiscal em aberto!',
          mtWarning, [mbOK], 0);
        Result := false;
      end;
    end;
    ...
    try
      ...
      begin
        ...
        begin
          ...
          Linha := Linha + FormatFloat('0.000', i.FieldByName('QUANTIDADE')
            .AsFloat) + '|';
          Linha := Linha + FormatFloat('0.000', i.FieldByName('ESTOQUE')
            .AsFloat) + '|';
          writeLn(Arq, Linha);
          i.Next;
        end;
...
function TDmWorkCom.Encerrantes(pDatai, pDataf: TDateTime;
  pArquivo: String): Boolean;
var
...
begin
  ...
  try
    ...
    try
      ...
      Q.SQL.Add('and a.data >= ' + QuotedStr(FormatDateTime('mm/dd/yyyy',
        pDatai)));
      Q.SQL.Add('and a.data <= ' + QuotedStr(FormatDateTime('mm/dd/yyyy',
        pDataf)));
      Q.Open;
      ...
      begin
        ...
        // Identificação
        AuxTXT := 'C1';
        AuxTXT := AuxTXT + ZeroEsquerda
          (FiltraNumero(Trim(CdsEmpresa.FieldByName('CNPJ_CPF').AsString)), 14);
        AuxTXT := AuxTXT +
          AlinhaTXT(FiltraNumero(Trim(CdsEmpresa.FieldByName('INSCRICAO_RG')
          .AsString)), 1, 14);
        AuxTXT := AuxTXT + AlinhaTXT('', 1, 14);
        AuxTXT := AuxTXT +
          AlinhaTXT(FiltraNumero(Trim(CdsEmpresa.FieldByName('RAZAO_SOCIAL')
          .AsString)), 1, 50);
        ...
        begin
          ...
          AuxTXT := 'C2';
          AuxTXT := AuxTXT + ZeroEsquerda
            (FiltraNumero(Trim(CdsEmpresa.FieldByName('CNPJ_CPF')
            .AsString)), 14);
          AuxTXT := AuxTXT +
            AlinhaTXT(IntToStr(Q.FieldByName('idproduto').AsInteger), 1, 15);
          AuxTXT := AuxTXT +
            AlinhaTXT(IntToStr(Q.FieldByName('tanque_numero').AsInteger), 1, 3);
          AuxTXT := AuxTXT +
            AlinhaTXT(IntToStr(Q.FieldByName('bomba_numero').AsInteger), 1, 3);
          AuxTXT := AuxTXT +
            AlinhaTXT(IntToStr(Q.FieldByName('bico_concentrador')
            .AsInteger), 1, 3);
          AuxTXT := AuxTXT + AlinhaTXT(Copy(aCombustivel, 1, 19), 1, 20);
          AuxTXT := AuxTXT + FormatDateTime('YYYYMMDD',
            Q.FieldByName('data').AsDateTime);
          AuxTXT := AuxTXT + FormatDateTime('HHMMSS',
            Q.FieldByName('hora').AsDateTime);
          AuxTXT := AuxTXT + ZeroEsquerda
            (FiltraNumero(FormatFloat('0.000', Q.FieldByName('encerrante')
            .AsFloat - Q.FieldByName('litros').AsFloat)), 15);
          AuxTXT := AuxTXT + ZeroEsquerda
            (FiltraNumero(FormatFloat('0.000', Q.FieldByName('encerrante')
            .AsFloat)), 15);
          AuxTXT := AuxTXT + AlinhaTXT(aStatus, 1, 10);
          AuxTXT := AuxTXT + AlinhaTXT(Q.FieldByName('numserieecf')
            .AsString, 1, 10);
          AuxTXT := AuxTXT + FormatDateTime('YYYYMMDD',
            Q.FieldByName('data_emissao').AsDateTime);
          AuxTXT := AuxTXT + FormatDateTime('HHMMSS',
            Q.FieldByName('hora_emissao').AsDateTime);
          AuxTXT := AuxTXT +
            AlinhaTXT(IntToStr(Q.FieldByName('coo_cupom').AsInteger), 1, 6);
          if Q.FieldByName('status_abastec').AsInteger = 3 then
            AuxTXT := AuxTXT +
              AlinhaTXT(IntToStr(Q.FieldByName('notafiscal_num')
              .AsInteger), 1, 6)
          else
            AuxTXT := AuxTXT + AlinhaTXT(IntToStr(0), 1, 6);
          AuxTXT := AuxTXT + ZeroEsquerda
            (FiltraNumero(FormatFloat('0.000', Q.FieldByName('litros')
            .AsFloat)), 10);
          writeLn(aArquivo, AuxTXT);
          //
          Q.Next;
    ...
    try
      ...
      begin
        ...
        begin
          AuxTXT := '#AP:';
          AuxTXT := AuxTXT + 'B' +
            ZeroEsquerda(IntToStr(Q.FieldByName('bico_concentrador')
            .AsInteger), 2) + ' ';
          AuxTXT := AuxTXT + 'EI' +
            ZeroEsquerda(FormatFloat('0.000', Q.FieldByName('encerrante')
            .AsFloat - Q.FieldByName('litros').AsFloat), 11) + ' ';
          AuxTXT := AuxTXT + 'EF' +
            ZeroEsquerda(FormatFloat('0.000', Q.FieldByName('encerrante')
            .AsFloat), 11) + ' ';
          AuxTXT := AuxTXT + 'V' + FormatFloat('0.000', Q.FieldByName('litros')
            .AsFloat) + ' ';
...
var
  ...
  aQuantidade, aQuebraIn, aQuebraOut, aVespeb, AuxVespeb, aEnc_Out,
    aEnc_In: Double;
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          EncConcentrador := DmPosto.RetornaEncBicoConcentrador
        ...
        aMovAbastec := DmPosto.VerificaMovtoBicoAbast
          (DmAcBr.ACBrECF1.DataMovimento, SQL.FieldByName('bico_concentrador')
          .AsString);
        aMovEmissao := DmPosto.VerificaMovimentoBico(aDataZ,
          SQL.FieldByName('bico_concentrador').AsString);
        aMovEmisNF := DmPosto.VerificaMovimentoBicoNF(aDataZ,
          SQL.FieldByName('bico_concentrador').AsString);
        ...
        aEncInicial := DmPosto.RetornaEncerranteUltZ(aDataZ,
          SQL.FieldByName('bico_concentrador').AsString);
        aEncInicial := TruncaValor(aEncInicial, 3);
        // captura o vespeb caso exista
        DmPosto.RetornaDadosVespeb(aVespeb, aEnc_Out, aEnc_In,
          DmPosto.CdsBicos.FieldByName('ID').AsInteger, aDataZ);
        ...
        begin
          ...
          aEncFinal := DmPosto.RetornaEncerranteFim
            (DmAcBr.ACBrECF1.DataMovimento, SQL.FieldByName('bico_concentrador')
            .AsString);
        ...
        if (EncConcentrador <> aEncFinal) and (EncConcentrador > 0) and
          (aEncFinal > 0) then
        begin
        ...
        // verifica se tem
        // DmPosto.Dados_Bicos(SQL.FieldByName('bico_concentrador').AsString);
        if DmPosto.RetornaDadosDescontinuidade(aQuebraIn, aQuebraOut,
          aHoraQuebra, DmPosto.CdsBicos.FieldByName('ID').AsInteger,
        ...
        begin
          ...
          begin
            aQuantidade := DmPosto.RetornaQuantVendidaPeriodo
              (DmAcBr.ACBrECF1.DataMovimento, aHoraQuebra,
              SQL.FieldByName('bico_concentrador').AsString, 0);
          ...
          AuxTXT := '#CE:';
          AuxTXT := AuxTXT + 'B' +
            ZeroEsquerda(IntToStr(SQL.FieldByName('bico_concentrador')
            .AsInteger), 2) + '  ';
          AuxTXT := AuxTXT + 'EI' +
            ZeroEsquerda(FormatFloat('0.000', aEncInicial), 11) + ' ';
          AuxTXT := AuxTXT + 'EF' +
            ZeroEsquerda(FormatFloat('0.000', aQuebraOut), 11) + ' ';
          ...
          begin
            aQuantidade := DmPosto.RetornaQuantVendidaPeriodo
              (DmAcBr.ACBrECF1.DataMovimento, aHoraQuebra,
              SQL.FieldByName('bico_concentrador').AsString, 1);
            aQuantidade := TruncaValor(aQuantidade, 3);
          end
          else
            aQuantidade := 0;
          ...
          AuxTXT := AuxTXT + 'B' +
            ...
            .AsInteger), 2) + '  ';
          AuxTXT := AuxTXT + 'EI' +
            ZeroEsquerda(FormatFloat('0.000', aQuebraIn), 11) + ' ';
          AuxTXT := AuxTXT + 'EF' +
            ZeroEsquerda(FormatFloat('0.000', aEncFinal), 11) + ' ';
          AuxTXT := AuxTXT + 'V' + FormatFloat('0.000', aQuantidade);
          PAFECF.ImprimeRelatorioGerencial(AuxTXT);
        end
        else
        begin
          ...
          begin
            ...
            aQuantidade := DmPosto.RetornaQuantVendida(aDataZ,
              SQL.FieldByName('bico_concentrador').AsString);
            aQuantidade := TruncaValor(aQuantidade, 3);
          end
          else
...
{$ENDIF}
  // Geral
  DmAcBr.ACBrNFe1.configuracoes.Geral.FormaEmissao :=
    StrToTpEmis(OK, IntToStr(sFormas));
...

{
  ...
  begin
    if sUsarWSAux = 'S' then
       if sUFWSAux = 'MS' then
       begin
           DmAcBr.ACBrNFe1.Configuracoes.Geral.CSC              := sToken;
           DmAcBr.ACBrNFe1.Configuracoes.Geral.idCSC                := sIDToken;
       end
       else
       begin
...
//fabricio fim
  ...
  DmAcBr.ACBrNFe1.configuracoes.Arquivos.PathSchemas :=
  ...
  DmAcBr.ACBrNFe1.configuracoes.Geral.FormatoAlerta :=
    'Campo:%DESCRICAO% - %MSG%';
  // WebServices
  ...
  DmAcBr.ACBrNFe1.configuracoes.WebServices.Ambiente :=
    StrToTpAmb(OK, IntToStr(sAmbiente + 1));
  ...
  begin
    ...
    DmAcBr.ACBrNFeDANFEFR1.FastFile := ExtractFilePath(Application.ExeName) +
      'Relatorios\DANFeRetrato.fr3';
...
function TDmWorkCom.CalcularTributoPedido(pIDPedido: Integer;
  pCST, pCFOP: String): Boolean;
var
  QItens, QGrava: TSQLQuery;
  ...
  try
    ...
    try
      ...
      begin
        ...
        aValIPI := 0;
        // localiza tributação
        if DmProdutos.Dados_Produto_Aliquota(QItens.FieldByName('IDProduto')
          .AsInteger, QItens.FieldByName('UF').AsString) then
        begin
          // calcula o imposto
          ...
          if (Copy(DmProdutos.CdsProduto_AliquotaCST.AsString, 2, 2) = '60')
          then
          begin
            ...
            begin
              aBaseST :=
                ((QItens.FieldByName('valor_total').AsFloat *
                QItens.FieldByName('MVA').AsFloat) / 100) +
                QItens.FieldByName('valor_total').AsFloat;
          ...
          aValCofins := (QItens.FieldByName('valor_total').AsFloat *
            QItens.FieldByName('aliquota_cofins').AsFloat) / 100;
          aValIPI := (QItens.FieldByName('valor_total').AsFloat *
            QItens.FieldByName('aliquota_ipi').AsFloat) / 100;
        end;
        // grava dados
        QGrava.Close;
        QGrava.Params.ParamByName('pCST').AsString :=
          DmProdutos.CdsProduto_AliquotaCST.AsString;
        QGrava.Params.ParamByName('pCFOP').AsString :=
          DmProdutos.CdsProduto_AliquotaCFOP.AsString;
        QGrava.Params.ParamByName('pMVA').AsFloat :=
          QItens.FieldByName('MVA').AsFloat;
        ...
        QGrava.Params.ParamByName('pID').AsInteger := QItens.FieldByName('ID')
          .AsInteger;
        QGrava.ExecSQL;
        // proximo item
        QItens.Next;
      ...
      begin
        ...
        QGrava.Params.ParamByName('pBase_Icms').AsFloat :=
          QItens.FieldByName('Base_Icms').AsFloat;
        QGrava.Params.ParamByName('pValor_icms').AsFloat :=
          QItens.FieldByName('Valor_Icms').AsFloat;
        QGrava.Params.ParamByName('pBase_icms_st').AsFloat :=
          QItens.FieldByName('Base_Icms_st').AsFloat;
        QGrava.Params.ParamByName('pValor_icms_st').AsFloat :=
          QItens.FieldByName('Valor_Icms_st').AsFloat;
        QGrava.Params.ParamByName('pValor_pis').AsFloat :=
          QItens.FieldByName('Valor_pis').AsFloat;
        QGrava.Params.ParamByName('pValor_cofins').AsFloat :=
          QItens.FieldByName('Valor_Cofins').AsFloat;
        QGrava.Params.ParamByName('pValor_ipi').AsFloat :=
          QItens.FieldByName('Valor_ipi').AsFloat;
...
procedure TDmWorkCom.GravaChaveNFEPedido(pChaveNFe, pProtocol, pRecibo, NumLote,
  NomeArqXML, Status_NFe: String; TipoEmiss, pIDPedido: Integer;
  Data_Autoriza, Hora_Autoriza: TDateTime);
var
  Query: TSQLQuery;
...
procedure TDmWorkCom.DspCFOPGetTableName(Sender: TObject; DataSet: TDataSet;
  var TableName: String);
begin
  TableName := 'CFOP';
...
begin
  ...
  try
    ...
    Q.SQL.Clear;
    Q.SQL.Add(
      'select coalesce(max(pedido_itens.ef),0) as total from pedido_itens');
...
Var
  ...
  aVBruta, aGTotal, aCancelados, aDescontos, aSubTrib, aIsentos, aNTrib,
    aTotalizador, aNaoFiscal, aAcrescimos, Suprimentos, Sangrias: Double;
  ...
  try
    ...
    try
      ...
      if FileExists(ExtractFilePath(Application.ExeName) +
        'DadosUltimaReducaoZ.ini') then
        SysUtils.DeleteFile(ExtractFilePath(Application.ExeName) +
          'DadosUltimaReducaoZ.ini');
      aStringList.SaveToFile(ExtractFilePath(Application.ExeName) +
        'DadosUltimaReducaoZ.ini');
      // Lendo o dados
      ...
      aSubTrib := aIni.ReadFloat('OutrasICMS',
      ...
      aCancelados := aCancelados + aIni.ReadFloat('Totalizadores',
      ...
      begin
        try
          ...
          MessageDlg('Erro ao inserir registrto 60. Verifique!!!', mtError,
            [mbOK], 0);
          Result := false;
        end;
      end;
      ...
      begin
        aAliquota := ZeroEsquerda(IntToStr(i + 1), 2) + 'T' +
          ZeroEsquerda(FiltraNumero(FormatFloat('0.00',
          DmAcBr.ACBrECF1.Aliquotas[i].Aliquota)), 4);
      ...
      begin
        ...
        aAliquota := 'OPNF';
        aNaoFiscal := aNaoFiscal +
          DmAcBr.ACBrECF1.TotalizadoresNaoTributados[i].Total;
      end;
...
begin
  try
    ...
    Q.SQL.Add('c.data_quit = ' + QuotedStr(FormatDateTime('dd/mm/yyyy',
      pDataBaixa)));
    Q.SQL.Add('c.valor_quitato = ' + FormatFloat('0.00', pValor));
    Q.SQL.Add('where c.nosso_numero = ' + QuotedStr(pNossoNum));
    Q.ExecSQL;
  finally
    FreeAndNil(Q);
  end;
...
function TDmWorkCom.VerificaCaixaAberto(var NomeOperador: String;
  var DataCaixa: TDateTime): Boolean;
var
  Q: TSQLQuery;
begin
...
begin
  Linha := 'U1';
  Linha := Linha + ZeroEsquerda(CdsEmpresa.FieldByName('CNPJ_CPF').AsString,
    14); // 14 espacos
  Linha := Linha + AlinhaTXT(CdsEmpresa.FieldByName('INSCRICAO_RG').AsString, 1,
    14); // 14 espaços
  Linha := Linha + AlinhaTXT(CdsEmpresa.FieldByName('INSC_MUNIC').AsString, 1,
    14); // 14 espaços
  Linha := Linha + AlinhaTXT(CdsEmpresa.FieldByName('RAZAO_SOCIAL').AsString, 1,
    50); // 50 espaços
  writeLn(sArquivo, Linha);
  ...
  try
    ...
    begin
      ...
      begin
        ...
        Linha := 'A2';
        Linha := Linha + FormatDateTime('YYYYMMDD',
          SQL.FieldByName('DATA_MOVIMENTO').AsDateTime);
        Linha := Linha + AlinhaTXT(MeioPagto, 1, 25);
        Linha := Linha + SQL.FieldByName('COD_TIPO').AsString;
        Linha := Linha + ZeroEsquerda
          (FiltraNumero(FormatFloat('0.00', SQL.FieldByName('VALOR')
          .AsCurrency)), 12);
        writeLn(sArquivo, Linha);
        SQL.Next;
      end;
...
begin
  ...
  begin
    ...
    if not DmWorkCom.VerificaMD5Registro(3,
      DmProdutos.CdsProdutos2.FieldByName('ID').AsInteger) then
      Unidade := RetornaCampoInterroga(Unidade, 6);
    Linha := 'P2';
    Linha := Linha + AlinhaTXT(CdsEmpresa.FieldByName('CNPJ_CPF').AsString, 1,
      14); // 14 espacos
    Linha := Linha + AlinhaTXT(DmProdutos.CdsProdutos2.FieldByName('Ean13')
      .AsString, 1, 14); // 14 espacos
    Linha := Linha + AlinhaTXT(DmProdutos.CdsProdutos2.FieldByName('Descricao')
      .AsString, 1, 50); // 50 espacos
    ...
    case DmProdutos.CdsProdutos2.FieldByName('TRIBUTACAO').AsInteger of
      ...
      2:
        begin
          ...
          aux := FormatFloat('0.00', DmProdutos.CdsProdutos2.FieldByName
            ('Aliquota_icms').AsFloat);
        end;
      3:
        begin
...
begin
  ...
  begin
    ...
    begin
      if DmProdutos.Dados_Produto(DmAuxiliar.CdsListaEstoqueIDPRODUTO.AsInteger)
      then
      begin
        if DmProdutos.Dados_Estoque(DmProdutos.CdsProdutos.FieldByName('ID')
          .AsInteger, sIDEmpresa) then
        ...
        if not DmWorkCom.VerificaMD5Registro(3,
          DmAuxiliar.CdsListaEstoqueIDPRODUTO.AsInteger) then
          Unidade := RetornaCampoInterroga(Unidade, 6);
        Linha := 'E2';
        Linha := Linha + AlinhaTXT(CdsEmpresa.FieldByName('CNPJ_CPF').AsString,
          1, 14); // 14 espacos
        Linha := Linha + AlinhaTXT(DmProdutos.CdsProdutos.FieldByName('Ean13')
          .AsString, 1, 14); // 14 espacos
        Linha := Linha + AlinhaTXT(DmProdutos.CdsProdutos.FieldByName
          ('Descricao').AsString, 1, 50); // 50 espacos
        Linha := Linha + AlinhaTXT(Unidade, 1, 6); // 6 espacos
        if aEstoque <= 0 then
          Linha := Linha + '-'
        else
  ...
  begin
    ...
    while not DmProdutos.CdsProdutos2.Eof do
    begin
      if DmProdutos.Dados_Estoque(DmProdutos.CdsProdutos2.FieldByName('ID')
        .AsInteger, sIDEmpresa) then
        aEstoque := DmProdutos.CdsEstoque.FieldByName('ESTOQUE_DIA').AsFloat
      else
        aEstoque := DmProdutos.CdsProdutos.FieldByName('Estoque').AsFloat;
      ...
      Linha := Linha + AlinhaTXT(DmProdutos.CdsProdutos2.FieldByName('Ean13')
        .AsString, 1, 14); // 14 espacos
      Linha := Linha + AlinhaTXT(DmProdutos.CdsProdutos2.FieldByName
        ('Descricao').AsString, 1, 50); // 50 espacos
      Linha := Linha + AlinhaTXT(Unidade, 1, 6); // 6 espacos
      if aEstoque <= 0 then
        Linha := Linha + '-'
...
begin
  ...
  begin
    Dados_DadosTecnicos;
    Linha := 'E3';
    Linha := Linha + AlinhaTXT(CdsPDV.FieldByName('SERIAL_ECF')
      .AsString, 1, 20);
    ...
    Linha := Linha + AlinhaTXT(CdsPDV.FieldByName('MODELO_ECF')
      .AsString, 1, 20);
    Linha := Linha + FormatDateTime('YYYYMMDD',
      CdsDadosTecnicos.FieldByName('DATA_ESTOQUE').AsDateTime);
    Linha := Linha + FormatDateTime('HHMMSS',
      CdsDadosTecnicos.FieldByName('HORA_ESTOQUE').AsDateTime);
    writeLn(sArquivo, Linha);
  end;
end;
...
begin
  ...
  QDav.SQL.Add('and (pedido.numdav is not null or pedido.numdav <> ' +
    QuotedStr('') + ')');
  ...
  begin
    ...
    begin
      ...
      if not VerificaMD5Registro(0, QDav.FieldByName('ID').AsInteger) then
        ModeloECF := RetornaCampoInterroga(ModeloECF, 20);
      aux := 'D2';
      aux := aux + ZeroEsquerda(CdsEmpresa.FieldByName('CNPJ_CPF').AsString,
        14); // 14 espacos
      ...
      begin
        ...
        aux := aux + ZeroEsquerda(QDav.FieldByName('numdav').AsString,
          10) + '   ';
        aux := aux + FormatDateTime('YYYYMMDD', QDav.FieldByName('data_pedido')
          .AsDateTime);
        ...
        aux := aux + ZeroEsquerda
          (IntToStr(QDav.FieldByName('COO').AsInteger), 9);
        aux := aux + ZeroEsquerda(DmWorkCom.PAFECF.gCaixa, 3);
        aux := aux + AlinhaTXT(Copy(QDav.FieldByName('cliente').AsString, 1,
          40), 1, 40);
    ...
    begin
      ...
      begin
        ...
        if not VerificaMD5Registro(1, QDav.FieldByName('ID').AsInteger,
          CdsPedidoItens.FieldByName('ID').AsInteger) then
        ...
        aux := aux + ZeroEsquerda
          (IntToStr(CdsPedidoItens.FieldByName('NUM_ITEM').AsInteger), 3);
        aux := aux + AlinhaTXT(ZeroEsquerda(CdsPedidoItens.FieldByName('EAN13')
          .AsString, 6), 1, 14);
        aux := aux + AlinhaTXT(aDescricao, 1, 100);
        aux := aux + ZeroEsquerda(FiltraNumero(FormatFloat('0.000',
          CdsPedidoItens.FieldByName('QUANTIDADE').AsFloat)), 7);
        aux := aux + AlinhaTXT(DmProdutos.CdsProdutos.FieldByName('UNIDADE')
          .AsString, 1, 3);
        aux := aux + ZeroEsquerda(FiltraNumero(FormatFloat('0.00',
          CdsPedidoItens.FieldByName('VALOR').AsFloat)), 8);
        aux := aux + ZeroEsquerda(FiltraNumero(FormatFloat('0.00',
          CdsPedidoItens.FieldByName('DESCONTO').AsFloat)), 8);
        aux := aux + ZeroEsquerda(FiltraNumero(FormatFloat('0.00',
          CdsPedidoItens.FieldByName('ACRESCIMO').AsFloat)), 8);
        aux := aux + ZeroEsquerda(FiltraNumero(FormatFloat('0.00',
          CdsPedidoItens.FieldByName('VALOR_TOTAL').AsFloat)), 14);
        ...
        else if DmProdutos.CdsProdutos.FieldByName('TRIBUTACAO').AsInteger = 0
        then
        begin
          Inc(aCont1);
...
begin
  ...
  QDav.SQL.Add('select pedido.id,pedido.pedido_num,pedido.data_pedido,');
  QDav.SQL.Add
    ('pedido.valor_total,pedido.coo,pedido.numero_doc,pedido.cliente,');
  QDav.SQL.Add('pedido.cnpjcpf from pedido');
  QDav.SQL.Add('where pedido.data_pedido between :pdatai and :pdataf');
  QDav.SQL.Add('and pedido.confirmada = ' + QuotedStr('S'));
  QDav.SQL.Add('and pedido.cancelado = ' + QuotedStr('N'));
  QDav.SQL.Add('and pedido.modelo = ' + QuotedStr('65'));
  QDav.SQL.Add('and (pedido.numero_doc is not null or pedido.numero_doc <> ' +
    QuotedStr('') + ')');
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        aux := aux + AlinhaTXT(ZeroEsquerda(QDav.FieldByName('numero_doc')
          .AsString, 6), 1, 13);
        aux := aux + FormatDateTime('YYYYMMDD', QDav.FieldByName('data_pedido')
          .AsDateTime);
  ...
  try
    ...
    begin
      ...
      begin
        ...
        AuxTXT := AuxTXT + ZeroEsquerda
          (FiltraNumero(Trim(CdsEmpresa.FieldByName('CNPJ_CPF').AsString)), 14);
        AuxTXT := AuxTXT + AlinhaTXT(Q.FieldByName('bomba_numero')
          .AsString, 1, 3);
        AuxTXT := AuxTXT + AlinhaTXT(Q.FieldByName('bico_concentrador')
          .AsString, 1, 3);
        AuxTXT := AuxTXT + FormatDateTime('YYYYMMDD',
          Q.FieldByName('data').AsDateTime);
        AuxTXT := AuxTXT + FormatDateTime('HHMMSS', Q.FieldByName('hora')
          .AsDateTime);
        AuxTXT := AuxTXT + AlinhaTXT(Copy(Q.FieldByName('motivo').AsString, 1,
          49), 1, 50);
        AuxTXT := AuxTXT + ZeroEsquerda
          (FiltraNumero(Trim(Q.FieldByName('CNPJ').AsString)), 14);
        AuxTXT := AuxTXT + ZeroEsquerda
          (FiltraNumero(Trim(Q.FieldByName('CPF').AsString)), 11);
        AuxTXT := AuxTXT + AlinhaTXT(Q.FieldByName('num_lacre_out')
          .AsString, 1, 15);
        AuxTXT := AuxTXT + AlinhaTXT(Q.FieldByName('num_lacre_in')
          .AsString, 1, 15);
        AuxTXT := AuxTXT + ZeroEsquerda
          (FiltraNumero(FormatFloat('0.000', Q.FieldByName('encerrante_out')
          .AsFloat)), 15);
        AuxTXT := AuxTXT + ZeroEsquerda
          (FiltraNumero(FormatFloat('0.000', Q.FieldByName('encerrante_in')
          .AsFloat)), 15);
...
begin
  ...
  try
    ...
    Q.SQL.Add('a.hora,a.encerrante,a.litros,a.status_abastec,a.numserieecf,');
    Q.SQL.Add(
      'a.data_emissao,a.hora_emissao,a.coo_cupom,a.data_abast,a.hora_abastec');
    ...
    begin
      ...
      begin
        ...
        AuxTXT := AuxTXT +
          AlinhaTXT(IntToStr(Q.FieldByName('idproduto').AsInteger), 1, 15);
        AuxTXT := AuxTXT +
          AlinhaTXT(ZeroEsquerda(IntToStr(Q.FieldByName('tanque_numero')
          .AsInteger), 2), 1, 3);
        AuxTXT := AuxTXT +
          AlinhaTXT(ZeroEsquerda(IntToStr(Q.FieldByName('bomba_numero')
          .AsInteger), 2), 1, 3);
        AuxTXT := AuxTXT +
          AlinhaTXT(ZeroEsquerda(IntToStr(Q.FieldByName('bico_concentrador')
          .AsInteger), 2), 1, 3);
        AuxTXT := AuxTXT + AlinhaTXT(Copy(aCombustivel, 1, 19), 1, 20);
        AuxTXT := AuxTXT + FormatDateTime('YYYYMMDD',
          Q.FieldByName('data_abast').AsDateTime);
        AuxTXT := AuxTXT + FormatDateTime('HHMMSS',
          Q.FieldByName('hora_abastec').AsDateTime);
        AuxTXT := AuxTXT + ZeroEsquerda
          (FiltraNumero(FormatFloat('0.000', Q.FieldByName('encerrante').AsFloat
          - Q.FieldByName('litros').AsFloat)), 15);
        AuxTXT := AuxTXT + ZeroEsquerda
          (FiltraNumero(FormatFloat('0.000', Q.FieldByName('encerrante')
          .AsFloat)), 15);
        AuxTXT := AuxTXT + AlinhaTXT(aStatus, 1, 10);
        ...
        begin
          ...
          AuxTXT := AuxTXT + FormatDateTime('HHMMSS',
            Q.FieldByName('hora_emissao').AsDateTime);
          AuxTXT := AuxTXT + ZeroEsquerda
            (IntToStr(Q.FieldByName('coo_cupom').AsInteger), 9);
        end
        else
        begin
        ...
        if Trim(Q.FieldByName('notafiscal_num').AsString) <> '' then
          AuxTXT := AuxTXT + ZeroEsquerda
            (IntToStr(Q.FieldByName('notafiscal_num').AsInteger), 6)
        else
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        aux := 'S2';
        aux := aux + AlinhaTXT(CdsEmpresa.FieldByName('CNPJ_CPF').AsString, 1,
          14); // 14 espacos
        aux := aux + FormatDateTime('YYYYMMDD', Q.FieldByName('data')
          .AsDateTime);
        aux := aux + FormatDateTime('HHMMSS', Q.FieldByName('hora').AsDateTime);
        aux := aux + ZeroEsquerda(FiltraNumero(Q.FieldByName('numero_mesa')
          .AsString), 13);
        aux := aux + ZeroEsquerda(FiltraNumero(FormatFloat('0.00',
          Q.FieldByName('VALOR').AsFloat)), 13);
    ...
    begin
      ...
      begin
        aux := 'S3';
        aux := aux + AlinhaTXT(CdsEmpresa.FieldByName('CNPJ_CPF')
          .AsString, 1, 14);
        aux := aux + FormatDateTime('YYYYMMDD', Q.FieldByName('data')
          .AsDateTime);
        aux := aux + FormatDateTime('HHMMSS', Q.FieldByName('hora').AsDateTime);
        ...
        aux := aux + ZeroEsquerda(FiltraNumero(FormatFloat('0.00',
          i.FieldByName('quant').AsFloat)), 7);
        aux := aux + AlinhaTXT(i.FieldByName('unidade').AsString, 1, 3);
        aux := aux + ZeroEsquerda(FiltraNumero(FormatFloat('0.00',
          i.FieldByName('preco_unit').AsFloat)), 8);
...
begin
  ...
  begin
    ...
    DmPosto.Dados_Bicos(DmPosto.CdsAbastecimentos1.FieldByName('NUM_BICO')
      .AsString);
    ...
    DmProdutos.Dados_Produto(DmPosto.CdsTanques.FieldByName('ID_PRODUTO')
      .AsInteger);
    ...
    begin
      ...
      Encerrante := DmPosto.CdsAbastecimentos1.FieldByName
        ('ENCERRANTE').AsFloat;
      Litros := DmPosto.CdsAbastecimentos1.FieldByName('LITROS').AsFloat;
      Preco_Unit := DmPosto.CdsAbastecimentos1.FieldByName
        ('PRECO_UNIT').AsFloat;
      Valor_Total := DmPosto.CdsAbastecimentos1.FieldByName
        ('VALOR_TOTAL').AsFloat;
      ...
      NumeroDoc := DmPosto.CdsAbastecimentos1.FieldByName
        ('NOTAFISCAL_NUM').AsString;
    end;
    // localiza dados do NCM
    DmProdutos.Dados_NCM(DmProdutos.CdsProdutos.FieldByName('CODIGO_NCM')
      .AsString, DmProdutos.CdsProdutos.FieldByName('EXCECAO_NCM').AsInteger);
    InsereItemCupom(DmPosto.CdsAbastecimentos1.FieldByName('LITROS')
      .AsFloat, 0, 0);
    ValorTotalItem :=
      TruncaValor((((CdsPedidoItens.FieldByName('QUANTIDADE').AsFloat *
      CdsPedidoItens.FieldByName('VALOR').AsFloat) - CdsPedidoItens.FieldByName
      ('DESCONTO').AsFloat) + CdsPedidoItens.FieldByName('ACRESCIMO').AsFloat),
      PAFECF.gDecimais);
    // calcula a carga a ser adicionada
    ValorCargaFed :=
      ((ValorTotalItem * DmProdutos.CdsNCM.FieldByName('ALIQ_NACIONAL')
      .AsFloat) / 100);
    ValorCargaEst :=
      ((ValorTotalItem * DmProdutos.CdsNCM.FieldByName('ALIQ_ESTADUAL')
      .AsFloat) / 100);
    ValorCargaMun :=
      ((ValorTotalItem * DmProdutos.CdsNCM.FieldByName('ALIQ_MUNICIPAL')
      .AsFloat) / 100);
    aValorPedido := DmPosto.CdsAbastecimentos1.FieldByName
      ('VALOR_TOTAL').AsFloat;
  end;
  CdsPedidos.Edit;
  ...
  CdsPedidos.FieldByName('NUMERO_DOC').AsString :=
    DmPosto.CdsAbastecimentos1.FieldByName('NOTAFISCAL_NUM').AsString;
...
procedure TDmWorkCom.DspReg61GetTableName(Sender: TObject; DataSet: TDataSet;
  var TableName: String);
begin
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            Cds60M.FieldByName('DATA_EMISSAO').AsDateTime :=
              StrToDate(Copy(auxString, 1, 2) + '/' + Copy(auxString, 3, 2) +
              '/' + Copy(auxString, 5, 4));
            Cds60M.FieldByName('DATA_MOVIMENTO').AsDateTime :=
              StrToDate(Copy(auxString, 1, 2) + '/' + Copy(auxString, 3, 2) +
              '/' + Copy(auxString, 5, 4));
            Cds60M.FieldByName('SERIAECF').AsString := cNumSerie;
            Cds60M.FieldByName('CONTADOR_REINICIO').AsString :=
              DmAcBr.ACBrECF1.NumCRO;
            Cds60M.FieldByName('CONTADOR_REDUCAO').AsString :=
              DmAcBr.ACBrECF1.NumCRZ;
            Cds60M.FieldByName('COO_INICIAL').AsInteger :=
              StrToInt(Le_Linha_Arquivo(cLinhaArquivo, '|', 5));
            Cds60M.FieldByName('COO_FINAL').AsInteger :=
              StrToInt(Le_Linha_Arquivo(cLinhaArquivo, '|', 6));
          ...
          begin
            // |C420|F1|32,00|||
            Cds60A.Append;
            Cds60A.FieldByName('ALIQUOTA_TRIB').AsString :=
              Le_Linha_Arquivo(cLinhaArquivo, '|', 3);
...
procedure TDmWorkCom.DspPaisGetTableName(Sender: TObject; DataSet: TDataSet;
  var TableName: String);
begin
  TableName := 'COD_PAIS';
end;

function TDmWorkCom.GerarNFe(pID: Integer; Apropria: Boolean;
  NFCe: Boolean = false; ContingenciaOff: Boolean = false): Boolean;
var
  aNumIten: Integer;
...
begin
  ...
  try
    ...
    begin
      CdsPedidos.Edit;
      CdsPedidos.FieldByName('NUMERO_DOC').AsString :=
        ZeroEsquerda(IntToStr(RetornaNumNfe(aModelo, sAmbiente + 1)), 6);
      Grava_Dados(CdsPedidos);
    end;
    // gera a NFe
...
//fabricio fim
      ...
      begin
        ...
        begin
          if Trim(CdsEmpresaUF.AsString)
            = Trim(CdsPedidosUF_END_ENTREGA.AsString) then
      ...
      begin
        Ide.tpImp := tiRetrato;
        if length(Trim(FiltraNumero(CdsPedidos.FieldByName('CNPJCPF').AsString))
          ) > 11 then
        begin
          if Trim(FiltraNumero(CdsPedidos.FieldByName('INSCEST').AsString)) <> ''
          then
          begin
            Ide.indFinal := cfNao;
            Ide.indPres := pcNao;
      ...
      Emit.EnderEmit.cMun :=
        StrToInt(DmWorkCom.CdsEmpresaCOD_MUNICIPIO.AsString);
      ...
      if DmClientes.CdsContatosCODCIDADE.AsString <> '' then
        Dest.EnderDest.cMun :=
          StrToInt(DmClientes.CdsContatosCODCIDADE.AsString)
      else
      ...
      if FiltraNumero(DmClientes.CdsContatosCODCIDADE.AsString) <> '' then
        Entrega.cMun :=
          StrToInt(FiltraNumero(DmClientes.CdsContatosCODCIDADE.AsString))
      else
        Entrega.cMun := 0;
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                if (DmWorkCom.CdsEmpresaCRT.AsInteger = 0) then
                begin
                  CSOSN := StrToCSOSNIcms(aOK,
                    DmWorkCom.CdsPedidoItensCSOSN.AsString);
                end;
                // regime normal ou excesso
                if (DmWorkCom.CdsEmpresaCRT.AsInteger = 1) or
                  (DmWorkCom.CdsEmpresaCRT.AsInteger = 2) then
                begin
                  CST := StrToCSTICMS(aOK,
                    DmWorkCom.CdsPedidoItensCST.AsString);
                end;
                //
              ...
              begin
                ...
                begin
                  CST := StrToCSTPIS(aOK,
                    DmWorkCom.CdsPedidoItensCST_PIS.AsString);
              ...
              begin
                with COFINS do
                begin
                  CST := StrToCSTCOFINS(aOK,
                    DmWorkCom.CdsPedidoItensCST_COFINS.AsString);
      ...
      begin
        ...
        begin
            ...
            0:
              begin // dinheiro
                ...
                begin
                  ...
                  ValorTroco := ValorTroco + CdsPedidoFP.FieldByName
                    ('TROCO').AsFloat;
                end;
              end;
            1:
              ...
              begin // Cartão
                ...
                begin
                  ...
                  begin
                    ...
                    if UpperCase(CdsPedidoFP.FieldByName('NOME_REDE').AsString)
                      = 'VISA' then
                      tBand := bcVisa
                    else if UpperCase(CdsPedidoFP.FieldByName('NOME_REDE')
                      .AsString) = 'CIELO' then
                      tBand := bcVisa
                    else if UpperCase(CdsPedidoFP.FieldByName('NOME_REDE')
                      .AsString) = 'MASTERCARD' then
                      tBand := bcMasterCard
                    else if UpperCase(CdsPedidoFP.FieldByName('NOME_REDE')
                      .AsString) = 'REDECARD' then
                      tBand := bcMasterCard
                    else if UpperCase(CdsPedidoFP.FieldByName('NOME_REDE')
                      .AsString) = 'AMEX' then
                      tBand := bcAmericanExpress
                    else if UpperCase(CdsPedidoFP.FieldByName('NOME_REDE')
                      .AsString) = 'SOROCRED' then
                      tBand := bcSorocred
                    else
                      tBand := bcOutros;
          ...
          GravaRegistroA2(SVData, CdsPedidoFP.FieldByName('DESCRICAO').AsString,
            3, (CdsPedidoFP.FieldByName('VALOR').AsFloat -
            CdsPedidoFP.FieldByName('TROCO').AsFloat));
          CdsPedidoFP.Next;
        end;
      end;
      ...
      if CdsPedidos.FieldByName('COO').AsInteger > 0 then
        aMensagem := aMensagem + 'Número do COO do cupom: ' +
          ZeroEsquerda(CdsPedidos.FieldByName('COO').AsString, 6) + #13 + #10;
      // Exive a mensagem do troco
      if ValorTroco > 0 then
        aMensagem := aMensagem + 'Valor do troco: R$' +
          FormatFloat(PAFECF.gMascaraDec, ValorTroco) + #13 + #10;
      // exibe carga tributaria
      {aMensagem := aMensagem + 'Val Aprox trib. Fed. R$' +
      ...
      begin
        if CdsPedidos.FieldByName('OBSNFE_1').AsString <> '' then
          aMensagem := aMensagem + CdsPedidos.FieldByName('OBSNFE_1').AsString
            + #13 + #10;
        if CdsPedidos.FieldByName('OBSNFE_2').AsString <> '' then
          aMensagem := aMensagem + CdsPedidos.FieldByName('OBSNFE_2').AsString
            + #13 + #10;
        if CdsPedidos.FieldByName('OBSNFE_3').AsString <> '' then
          aMensagem := aMensagem + CdsPedidos.FieldByName('OBSNFE_3').AsString
            + #13 + #10;
        if CdsPedidos.FieldByName('OBSNFE_4').AsString <> '' then
          aMensagem := aMensagem + CdsPedidos.FieldByName('OBSNFE_4').AsString
            + #13 + #10;
      end;
      if sPAFNFCe = 1 then
        aMensagem := aMensagem + 'OPERADOR: ' + PAFECF.gNomeOperador +
          ' - Caixa: ' + PAFECF.gCaixa + #13 + #10;
...
procedure TDmWorkCom.DspCNAEGetTableName(Sender: TObject; DataSet: TDataSet;
  var TableName: String);
begin
  TableName := 'CNAE';
end;
...
begin
  ...
  CdsPedidos.FieldByName('VALOR').AsFloat := 0;
  CdsPedidos.FieldByName('VALOR_TOTAL').AsFloat := 0;
  CdsPedidos.FieldByName('NAUTORIZA_VIDALINK').AsString :=
    DmAuxiliar.PBMVidaLink1.Solicitacao.Autorizacao;
  Grava_Dados(CdsPedidos);
  Result := CdsPedidos.FieldByName('ID').AsInteger;
  // REGISTRA OS ITENS
  ...
  begin
    ...
    begin
      ...
      CdsPedidoItens.Edit;
      CdsPedidoItens.FieldByName('VALOR').AsFloat := Solicitacao.Items[i]
        .Preco_Venda;
      CdsPedidoItens.FieldByName('VALOR_TOTAL').AsFloat := aTotalProd;
      CdsPedidoItens.FieldByName('CANCELADO').AsString := 'N';
      CdsPedidoItens.FieldByName('VALOR_REENBOLSO').AsFloat :=
        Solicitacao.Items[i].Valor_Reembolso;
      CdsPedidoItens.FieldByName('VALOR_SUBSIDIO').AsFloat :=
        Solicitacao.Items[i].Valor_Subsidio + Solicitacao.Items[i]
        .Preco_AReceber;
      Grava_Dados(CdsPedidoItens);
...
begin
  ...
  try
    ...
    Q.SQL.Add('and (p.chave_nfe is null or p.chave_nfe = ' +
      QuotedStr('') + ')');
    Q.Open;
    while not Q.Eof do
    begin
      ...
      begin
        try
          ...
          // grava chave NFe
          if DmAcBr.ACBrNFe1.WebServices.Retorno.NFeRetorno.ProtNFe.Items[0]
            .chNFe <> '' then
          begin
            DmWorkCom.GravaChaveNFEPedido
              (DmAcBr.ACBrNFe1.WebServices.Retorno.NFeRetorno.ProtNFe.Items[0]
              .chNFe, DmAcBr.ACBrNFe1.WebServices.Retorno.NFeRetorno.ProtNFe.
              Items[0].nProt, DmAcBr.ACBrNFe1.WebServices.Retorno.NFeRetorno.
              nRec, FormatDateTime('mmss', Now),
              DmAcBr.ACBrNFe1.NotasFiscais.Items[0].NomeArq,
              IntToStr(DmAcBr.ACBrNFe1.WebServices.Retorno.NFeRetorno.ProtNFe.
              Items[0].cStat), DmWorkCom.sFormas, Q.FieldByName('ID').AsInteger,
              DmAcBr.ACBrNFe1.WebServices.Retorno.NFeRetorno.ProtNFe.Items[0]
              .dhRecbto, DmAcBr.ACBrNFe1.WebServices.Retorno.NFeRetorno.ProtNFe.
              Items[0].dhRecbto);
          end;
        except
          on E: Exception do
...
begin
  ...
  begin
    CdsPedidos.FieldByName('CLIENTE').AsString :=
      DmClientes.CdsContatos.FieldByName('NOME').AsString;
    CdsPedidos.FieldByName('END_ENTREGA').AsString :=
      DmClientes.CdsContatos.FieldByName('END_ENTREGA').AsString;
    CdsPedidos.FieldByName('NUM_END_ENTREGA').AsString :=
      DmClientes.CdsContatos.FieldByName('NUM_END_ENTREGA').AsString;
    CdsPedidos.FieldByName('COMPL_END_ENTREGA').AsString :=
      DmClientes.CdsContatos.FieldByName('COMPL_ENT').AsString;
    CdsPedidos.FieldByName('BAIRRO_END_ENTREGA').AsString :=
      DmClientes.CdsContatos.FieldByName('BAIRRO_END_ENTREGA').AsString;
    CdsPedidos.FieldByName('CID_END_ENTREGA').AsString :=
      DmClientes.CdsContatos.FieldByName('CID_END_ENTREGA').AsString;
    CdsPedidos.FieldByName('UF_END_ENTREGA').AsString :=
      DmClientes.CdsContatos.FieldByName('UF_END_ENT').AsString;
    CdsPedidos.FieldByName('CEP_END_ENTREGA').AsString :=
      DmClientes.CdsContatos.FieldByName('CEP_END_ENTREGA').AsString;
    CdsPedidos.FieldByName('CNPJCPF').AsString :=
      DmClientes.CdsContatos.FieldByName('CNPJ_CPF').AsString;
    CdsPedidos.FieldByName('INSCEST').AsString :=
      DmClientes.CdsContatos.FieldByName('INSC_RG').AsString;
  end
  else if pTipo = 1 then // tabela de OS
  begin
    CdsOS.FieldByName('CONTATO').AsString := DmClientes.CdsContatos.FieldByName
      ('NOME').AsString;
    CdsOS.FieldByName('ENDERECO').AsString := DmClientes.CdsContatos.FieldByName
      ('END_ENTREGA').AsString;
    CdsOS.FieldByName('NUMERO').AsString := DmClientes.CdsContatos.FieldByName
      ('NUM_END_ENTREGA').AsString;
    CdsOS.FieldByName('COMPLEMENTO').AsString :=
      DmClientes.CdsContatos.FieldByName('COMPL_ENT').AsString;
    CdsOS.FieldByName('BAIRRO').AsString := DmClientes.CdsContatos.FieldByName
      ('BAIRRO_END_ENTREGA').AsString;
    CdsOS.FieldByName('CIDADE').AsString := DmClientes.CdsContatos.FieldByName
      ('CID_END_ENTREGA').AsString;
    CdsOS.FieldByName('UF').AsString := DmClientes.CdsContatos.FieldByName
      ('UF_END_ENT').AsString;
    CdsOS.FieldByName('CEP').AsString := DmClientes.CdsContatos.FieldByName
      ('CEP_END_ENTREGA').AsString;
    CdsOS.FieldByName('CNPJCPF').AsString := DmClientes.CdsContatos.FieldByName
      ('CNPJ_CPF').AsString;
    CdsOS.FieldByName('INSCRICAORG').AsString :=
      DmClientes.CdsContatos.FieldByName('INSC_RG').AsString;
  end;
...
function TDmWorkCom.LocalizaXMLBanco(var MemoResp: TMemo;
...
begin
  try
    ...
    Q.SQL.Add('where x.chave_nfe = ' +
      QuotedStr('NFe' + FiltraNumero(ChaveNFe)));
    Q.Open;
    if not Q.IsEmpty then
    begin
...
begin
  ...
  try
    ...
    begin
      ...
      //
      aux := 'Data: ' + FormatDateTime('DD/MM/YYYY', Date) + ' - Hora: ' +
        FormatDateTime('00:00:00', Time);
      ...
      while not Item.Eof do
      begin
        aux := ZeroEsquerda(IntToStr(Item.FieldByName('ID_PRODUTO').AsInteger),
          5) + ' - ' + Copy(Item.FieldByName('DESCRICAO').AsString, 1, 32);
        prn.SendData(aux + #13 + #10);
        aux := FormatFloat('0.000', Item.FieldByName('QUANT').AsFloat) +
          '   x   ' + FormatFloat('0.000', Item.FieldByName('PRECO_UNIT')
          .AsFloat) + '   =   ' + FormatFloat('0.000',
...
begin
  ...
  begin
    ...
    PAFECF.ImprimeRelatorioGerencial
      ('CNPJ: ' + Trim(CdsEmpresaCNPJ_CPF.AsString) + ' - IE: ' +
      Trim(CdsEmpresaINSCRICAO_RG.AsString));
    PAFECF.ImprimeRelatorioGerencial(Trim(CdsEmpresaENDERECO.AsString) + ', ' +
      Trim(CdsEmpresaNUMERO.AsString) + ' - ' +
      Trim(CdsEmpresaBAIRRO.AsString));
    PAFECF.ImprimeRelatorioGerencial(Trim(CdsEmpresaCIDADE.AsString) + ' - ' +
      CdsEmpresaUF.AsString);
    PAFECF.ImprimeRelatorioGerencial
      ('------------------------------------------------');
    PAFECF.ImprimeRelatorioGerencial
    ...
    PAFECF.ImprimeRelatorioGerencial
      ('  Não permite aproveitamento de crédido do ICMS ');
    PAFECF.ImprimeRelatorioGerencial
      ('------------------------------------------------');
    PAFECF.ImprimeRelatorioGerencial('Código  Descrição');
    PAFECF.ImprimeRelatorioGerencial
      ('   Quant.    Und.        Vl.Unit        Vl.Total');
    PAFECF.ImprimeRelatorioGerencial
      ('------------------------------------------------');
    // imprime os itens
    ContItens := 0;
    ...
    begin
      ...
      begin
        Inc(ContItens);
        PAFECF.ImprimeRelatorioGerencial(CdsPedidoItensEAN13.AsString + ' - ' +
          Copy(CdsPedidoItensDESCRICAO.AsString, 1, 29));
        PAFECF.ImprimeRelatorioGerencial
          ('   ' + AlinhaCampoValor(CdsPedidoItensQUANTIDADE.AsFloat, 8, 3) +
    ...
    PAFECF.ImprimeRelatorioGerencial
      ('------------------------------------------------');
    PAFECF.ImprimeRelatorioGerencial('QTD. Total de itens: ' +
      IntToStr(ContItens));
    // PAFECF.ImprimeRelatorioGerencial('');
    PAFECF.ImprimeRelatorioGerencial('Valor dos produtos: ' +
      AlinhaCampoValor(CdsPedidosVALOR.AsFloat, 10, 2));
    PAFECF.ImprimeRelatorioGerencial('Valor Desconto....: ' +
      AlinhaCampoValor(CdsPedidosDESCONTO.AsFloat, 10, 2));
    PAFECF.ImprimeRelatorioGerencial('Valor Acrescimos..: ' +
      AlinhaCampoValor(CdsPedidosACRESCIMO.AsFloat, 10, 2));
    PAFECF.ImprimeRelatorioGerencial('Valor total NFCe..: ' +
    ...
    while not CdsPedidoFP.Eof do
    begin
      PAFECF.ImprimeRelatorioGerencial(CdsPedidoFPDESCRICAO.AsString + ' - ' +
    ...
    if ValorTroco > 0 then
    begin
      PAFECF.ImprimeRelatorioGerencial('Valor do troco..: ' +
        AlinhaCampoValor(ValorTroco, 10, 2));
    end;
    PAFECF.ImprimeRelatorioGerencial
      ('------------------------------------------------');
    PAFECF.ImprimeRelatorioGerencial('Tributos totais incidentes: ' +
      AlinhaCampoValor(CdsPedidosVALOR_CARGA_TRIB.AsFloat, 10, 2));
    ...
    PAFECF.ImprimeRelatorioGerencial('Número: ' + CdsPedidosNUMERO_DOC.AsString
      + ' - Série: 001');

    PAFECF.ImprimeRelatorioGerencial('Emissão: ' + FormatDateTime('DD/MM/YYYY',
      CdsPedidosDATA_ENTREGA.AsDateTime) + ' ' + FormatDateTime('HH:MM:SS',
      CdsPedidosHORA_ENTREGA.AsDateTime));
    PAFECF.ImprimeRelatorioGerencial
      ('------------------------------------------------');
    ...
    if CdsConfigNFE_WS_UF_AUX.AsString = 'AM' then
      PAFECF.ImprimeRelatorioGerencial
        ('http://portalnfce.sefaz.am.gov.br/consumidor/consulte-sua-nota/')
    ...
    begin
      PAFECF.ImprimeRelatorioGerencial
        ('CPF/CNPJ: ' + CdsPedidosCNPJCPF.AsString);
      ...
      PAFECF.ImprimeRelatorioGerencial(CdsPedidosCID_END_ENTREGA.AsString +
        ' - ' + CdsPedidosUF_END_ENTREGA.AsString + ' - ' + 'CEP:' +
    ...
    begin
      ...
      begin
        ...
        begin
          ID := DmWorkCom.CdsConfigCLIENTE_PADRAO.AsInteger;
          Cliente := DmClientes.CdsContatos.FieldByName('NOME').AsString;
          Endereco := DmClientes.CdsContatos.FieldByName('END_ENTREGA')
            .AsString;
          Numero := DmClientes.CdsContatos.FieldByName
            ('NUM_END_ENTREGA').AsString;
          Complemento := DmClientes.CdsContatos.FieldByName
        ...
        begin
          ...
          Grava_Dados(CdsPedidoItens);
          // inclui o item na impressora fiscal
          DmProdutos.Dados_Produto(CdsPedidoItens.FieldByName('ean13')
            .AsString);
          if DmWorkCom.RegistraItemECF
            (FloatToStr(CdsPedidoItens.FieldByName('Quantidade').AsFloat)) then
          begin
            PAFECF.AtualizaINI(false);
            // verifica Item Cancelado
            if CdsPedidoItens.FieldByName('CANCELADO').AsString = 'S' then
        ...
        begin
          ...
          begin
            ValorPago :=
              TruncaValor((CdsPedidoFP.FieldByName('VALOR').AsFloat +
              CdsPedidoFP.FieldByName('TROCO').AsFloat) -
              CdsPedidoFP.FieldByName('DESCONTO').AsFloat,
              DmWorkCom.PAFECF.gDecimais);
          end
          else
          begin
          ...
          PAFECF.RegistraFormaPagtoCupom(FormaPag,
            TruncaValor(ValorPago, DmWorkCom.PAFECF.gDecimais));
          CdsPedidoFP.Next;
        end;
        aMensagem := '';
...
begin
  ...
  try
    ...
    begin
      ...
      if DmProdutos.CdsProdutos.FieldByName('CONTROLAR_ESTOQUE').AsString = 'S'
      then
      begin
        // retorna tipo de produto
        ...
        begin
          ...
          begin
            // localiza registro do produto composto na tabela
            if DmProdutos.Dados_Produto
              (DmProdutos.CdsProdutoCompostoID_PRODUTO.AsInteger) then
            begin
              // grava tabela de estoque
              DmProdutos.Grava_EstoqueDia
                (DmProdutos.CdsProdutoCompostoID_PRODUTO.AsInteger);
            end;
            // proximo produto
            DmProdutos.CdsProdutoComposto.Next;
...
procedure TDmWorkCom.GerarRegistro10(pDatai, pDataf: TDateTime;
  pConvenio, pNatureza, pFinalidade: String);
begin
  with DmAcBr.ACBrSintegra1 do
  begin
    Registro10.CNPJ := FiltraNumero(CdsEmpresa.FieldByName('CNPJ_CPF')
      .AsString);
    Registro10.Inscricao := FiltraNumero(CdsEmpresa.FieldByName('INSCRICAO_RG')
      .AsString);
...
begin
  ...
  begin
    ...
    Registro11.Numero := FiltraNumero(CdsEmpresa.FieldByName('NUMERO')
      .AsString);
...
begin
  ...
  begin
    ...
    begin
      ...
      with DmAcBr.ACBrSintegra1 do
      begin
        wregistro50.CPFCNPJ := DmAuxiliar.CdsRegistros.FieldByName
          ('cpfcnpj').AsString;
        wregistro50.Inscricao := DmAuxiliar.CdsRegistros.FieldByName
          ('inscricao').AsString;
        wregistro50.DataDocumento := DmAuxiliar.CdsRegistros.FieldByName
          ('datadocumento').AsDateTime;
        wregistro50.UF := DmAuxiliar.CdsRegistros.FieldByName('uf').AsString;
        wregistro50.modelo := DmAuxiliar.CdsRegistros.FieldByName
          ('modelo').AsString;
        wregistro50.serie := DmAuxiliar.CdsRegistros.FieldByName
          ('serie').AsString;
        wregistro50.Numero := DmAuxiliar.CdsRegistros.FieldByName
          ('numero').AsString;
        wregistro50.CFOP := DmAuxiliar.CdsRegistros.FieldByName('cfop')
          .AsString;
        wregistro50.EmissorDocumento := DmAuxiliar.CdsRegistros.FieldByName
          ('emissordocumento').AsString;
        wregistro50.ValorContabil := DmAuxiliar.CdsRegistros.FieldByName
          ('valorcontabil').AsFloat;
        wregistro50.BasedeCalculo := DmAuxiliar.CdsRegistros.FieldByName
          ('basedecalculo').AsFloat;
        wregistro50.ICMS := DmAuxiliar.CdsRegistros.FieldByName('icms').AsFloat;
        wregistro50.Isentas := DmAuxiliar.CdsRegistros.FieldByName
          ('isentas').AsFloat;
        wregistro50.Outras := DmAuxiliar.CdsRegistros.FieldByName
          ('outras').AsFloat;
        wregistro50.Aliquota := DmAuxiliar.CdsRegistros.FieldByName
          ('aliquota').AsFloat;
        wregistro50.Situacao := DmAuxiliar.CdsRegistros.FieldByName
          ('situacao').AsString;
...
begin
  ...
  begin
    ...
    begin
      ...
      with DmAcBr.ACBrSintegra1 do
      begin
        wregistro51.CPFCNPJ := DmAuxiliar.CdsRegistros.FieldByName
          ('CPFCNPJ').AsString;
        wregistro51.Inscricao := DmAuxiliar.CdsRegistros.FieldByName
          ('INSCRICAO').AsString;
        wregistro51.DataDocumento := DmAuxiliar.CdsRegistros.FieldByName
          ('DATADOCUMENTO').AsDateTime;
        wregistro51.Estado := DmAuxiliar.CdsRegistros.FieldByName('UF')
          .AsString;
        wregistro51.serie := DmAuxiliar.CdsRegistros.FieldByName
          ('SERIE').AsString;
        wregistro51.Numero := DmAuxiliar.CdsRegistros.FieldByName
          ('NUMERO').AsString;
        wregistro51.CFOP := DmAuxiliar.CdsRegistros.FieldByName('CFOP')
          .AsString;
        wregistro51.ValorContabil := DmAuxiliar.CdsRegistros.FieldByName
          ('VALORCONTABIL').AsFloat;
        wregistro51.ValorIpi := DmAuxiliar.CdsRegistros.FieldByName
          ('VALORIPI').AsFloat;
        wregistro51.ValorIsentas := DmAuxiliar.CdsRegistros.FieldByName
          ('ISENTAS').AsFloat;
        wregistro51.ValorOutras := DmAuxiliar.CdsRegistros.FieldByName
          ('OUTRAS').AsFloat;
        wregistro51.Situacao := DmAuxiliar.CdsRegistros.FieldByName
          ('SITUACAO').AsString;
...
begin
  ...
  begin
    ...
    begin
      ...
      with DmAcBr.ACBrSintegra1 do
      begin
        wregistro53.CPFCNPJ := DmAuxiliar.CdsRegistros.FieldByName
          ('CPFCNPJ').AsString;
        wregistro53.Inscricao := DmAuxiliar.CdsRegistros.FieldByName
          ('INSCRICAO').AsString;
        wregistro53.DataDocumento := DmAuxiliar.CdsRegistros.FieldByName
          ('DATADOCUMENTO').AsDateTime;
        wregistro53.Estado := DmAuxiliar.CdsRegistros.FieldByName('UF')
          .AsString;
        wregistro53.modelo := DmAuxiliar.CdsRegistros.FieldByName
          ('MODELO').AsString;
        wregistro53.serie := DmAuxiliar.CdsRegistros.FieldByName
          ('SERIE').AsString;
        wregistro53.Numero := DmAuxiliar.CdsRegistros.FieldByName
          ('NUMERO').AsString;
        wregistro53.CFOP := DmAuxiliar.CdsRegistros.FieldByName('CFOP')
          .AsString;
        wregistro53.Emitente := DmAuxiliar.CdsRegistros.FieldByName
          ('EMISSORDOCUMENTO').AsString;
        wregistro53.BaseST := DmAuxiliar.CdsRegistros.FieldByName
          ('BASEDECALCULOST').AsFloat;
        wregistro53.IcmsRetido := DmAuxiliar.CdsRegistros.FieldByName
          ('ICMSST').AsFloat;
        wregistro53.Despesas := DmAuxiliar.CdsRegistros.FieldByName
          ('OUTRAS_DESP').AsFloat;
        wregistro53.Situacao := DmAuxiliar.CdsRegistros.FieldByName('SITUACAO')
          .AsString;;
...
begin
  ...
  begin
    ...
    begin
      ...
      with DmAcBr.ACBrSintegra1 do
      begin
        wregistro54.CPFCNPJ := DmAuxiliar.CdsRegistros.FieldByName
          ('CPFCNPJ').AsString;
        wregistro54.modelo := DmAuxiliar.CdsRegistros.FieldByName
          ('MODELO').AsString;
        wregistro54.serie := DmAuxiliar.CdsRegistros.FieldByName
          ('SERIE').AsString;
        wregistro54.Numero := DmAuxiliar.CdsRegistros.FieldByName
          ('NUMERO').AsString;
        wregistro54.CFOP := DmAuxiliar.CdsRegistros.FieldByName('CFOP')
          .AsString;
        wregistro54.CST := DmAuxiliar.CdsRegistros.FieldByName('CST').AsString;
        wregistro54.NumeroItem := DmAuxiliar.CdsRegistros.FieldByName
          ('NUMEROITEM').AsInteger;
        wregistro54.codigo := DmAuxiliar.CdsRegistros.FieldByName
          ('CODIGO').AsString;
        wregistro54.Descricao := DmAuxiliar.CdsRegistros.FieldByName
          ('DESCRICAO').AsString;
        wregistro54.Quantidade := DmAuxiliar.CdsRegistros.FieldByName
          ('QUANTIDADE').AsFloat;
        wregistro54.Valor := DmAuxiliar.CdsRegistros.FieldByName
          ('VALOR').AsFloat;
        wregistro54.ValorDescontoDespesa := DmAuxiliar.CdsRegistros.FieldByName
          ('VALORDESCONTO').AsFloat;
        wregistro54.BasedeCalculo := DmAuxiliar.CdsRegistros.FieldByName
          ('BASEDECALCULO').AsFloat;
        wregistro54.BaseST := DmAuxiliar.CdsRegistros.FieldByName
          ('BASEST').AsFloat;
        wregistro54.ValorIpi := DmAuxiliar.CdsRegistros.FieldByName
          ('VALORIPI').AsFloat;
        wregistro54.Aliquota := DmAuxiliar.CdsRegistros.FieldByName
          ('ALIQUOTA').AsFloat;
      ...
      begin
        ...
        wregistro75.Descricao := DmAuxiliar.CdsRegistros.FieldByName
          ('DESCRICAO').AsString;
        wregistro75.Unidade := DmAuxiliar.CdsRegistros.FieldByName
          ('UNIDADE').AsString;
...
begin
  ...
  begin
    ...
    begin
      ...
      with DmAcBr.ACBrSintegra1 do
      begin
        wregistro60A.Emissao := DmAuxiliar.CdsRegistros.FieldByName('EMISSAO')
          .AsDateTime;
        wregistro60A.NumSerie := DmAuxiliar.CdsRegistros.FieldByName
          ('NUMSERIE').AsString;
        wregistro60A.StAliquota := DmAuxiliar.CdsRegAuxiliar.FieldByName
          ('STALIQUOTA').AsString;
        wregistro60A.Valor := DmAuxiliar.CdsRegAuxiliar.FieldByName
          ('VALOR').AsFloat;
...
begin
  ...
  begin
    ...
    begin
      ...
      with DmAcBr.ACBrSintegra1 do
      begin
        wregistro60D.Emissao := DmAuxiliar.CdsRegistros.FieldByName('EMISSAO')
          .AsDateTime;
        wregistro60D.NumSerie := DmAuxiliar.CdsRegistros.FieldByName
          ('NUMSERIE').AsString;
        wregistro60D.codigo := DmAuxiliar.CdsRegAuxiliar1.FieldByName
          ('CODIGO').AsString;
        wregistro60D.Quantidade := DmAuxiliar.CdsRegAuxiliar1.FieldByName
          ('QUANTIDADE').AsFloat;
        wregistro60D.Valor := DmAuxiliar.CdsRegAuxiliar1.FieldByName
          ('VALOR').AsFloat;
        wregistro60D.BasedeCalculo := DmAuxiliar.CdsRegAuxiliar1.FieldByName
          ('BASEDECALCULO').AsFloat;
        wregistro60D.StAliquota := DmAuxiliar.CdsRegAuxiliar1.FieldByName
          ('STALIQUOTA').AsString;
        wregistro60D.ValorIcms := DmAuxiliar.CdsRegAuxiliar1.FieldByName
          ('VALORICMS').AsFloat;
      ...
      begin
        ...
        wregistro75.AliquotaICMS := DmAuxiliar.CdsRegAuxiliar1.FieldByName
          ('PERC_ICMS').AsFloat;
        wregistro75.DataInicial := DmAcBr.ACBrSintegra1.Registro10.DataInicial;
        wregistro75.DataFinal := DmAcBr.ACBrSintegra1.Registro10.DataFinal;
        wregistro75.Descricao := DmAuxiliar.CdsRegAuxiliar1.FieldByName
          ('DESCRICAO').AsString;
        wregistro75.Unidade := DmAuxiliar.CdsRegAuxiliar1.FieldByName
          ('UNIDADE').AsString;
        DmAcBr.ACBrSintegra1.Registros75.Add(wregistro75);
      end;
...
begin
  ...
  begin
    ...
    begin
      ...
      with DmAcBr.ACBrSintegra1 do
      begin
        wregistro60M.Emissao := DmAuxiliar.CdsRegistros.FieldByName('EMISSAO')
          .AsDateTime;
        wregistro60M.NumSerie := DmAuxiliar.CdsRegistros.FieldByName
          ('NUMSERIE').AsString;
        wregistro60M.NumOrdem := DmAuxiliar.CdsRegistros.FieldByName('NUMORDEM')
          .AsInteger;
        wregistro60M.ModeloDoc := DmAuxiliar.CdsRegistros.FieldByName
          ('MODELODOC').AsString;
        wregistro60M.CooInicial := DmAuxiliar.CdsRegistros.FieldByName
          ('COOINICIAL').AsInteger;
        wregistro60M.CooFinal := DmAuxiliar.CdsRegistros.FieldByName('COOFINAL')
          .AsInteger;
        wregistro60M.CRZ := DmAuxiliar.CdsRegistros.FieldByName('CRZ')
          .AsInteger;
        wregistro60M.CRO := DmAuxiliar.CdsRegistros.FieldByName('CRO')
          .AsInteger;
        wregistro60M.VendaBruta := DmAuxiliar.CdsRegistros.FieldByName
          ('VENDABRUTA').AsFloat;
        wregistro60M.ValorGT := DmAuxiliar.CdsRegistros.FieldByName
          ('VALORGT').AsFloat;
        DmAcBr.ACBrSintegra1.Registros60M.Add(wregistro60M);
      end;
      // gera registro 60 adicionais
      GerarRegistro60A(DmAuxiliar.CdsRegistros.FieldByName('CD_REGISTRO60M')
        .AsInteger);
      if p60d then
        GerarRegistro60D(DmAuxiliar.CdsRegistros.FieldByName('EMISSAO')
          .AsDateTime);
      if p60i then
        GerarRegistro60I(DmAuxiliar.CdsRegistros.FieldByName('EMISSAO')
          .AsDateTime);
      if p60r then
        GerarRegistro60R(pDatai, pDataf,
          DmAuxiliar.CdsRegistros.FieldByName('NUMSERIE').AsString);
      //
      DmAuxiliar.CdsRegistros.Next;
    end;
  end;
...
begin
  ...
  begin
    ...
    begin
      ...
      with DmAcBr.ACBrSintegra1 do
      begin
        wregistro61.Emissao := DmAuxiliar.CdsRegistros.FieldByName('EMISSAO')
          .AsDateTime;
        wregistro61.modelo := DmAuxiliar.CdsRegistros.FieldByName
          ('MODELO').AsString;
        wregistro61.serie := DmAuxiliar.CdsRegistros.FieldByName
          ('SERIE').AsString;
        wregistro61.SubSerie := DmAuxiliar.CdsRegistros.FieldByName
          ('SUBSERIE').AsString;
        wregistro61.NumOrdemInicial := DmAuxiliar.CdsRegistros.FieldByName
          ('NUMORDEMINICIAL').AsInteger;
        wregistro61.NumOrdemFinal := DmAuxiliar.CdsRegistros.FieldByName
          ('NUMORDEMFINAL').AsInteger;
        wregistro61.Valor := DmAuxiliar.CdsRegistros.FieldByName
          ('VALOR').AsFloat;
        wregistro61.BasedeCalculo := DmAuxiliar.CdsRegistros.FieldByName
          ('BASEDECALCULO').AsFloat;
        wregistro61.ValorIcms := DmAuxiliar.CdsRegistros.FieldByName
          ('VALORICMS').AsFloat;
        wregistro61.Isentas := DmAuxiliar.CdsRegistros.FieldByName
          ('ISENTAS').AsFloat;
        wregistro61.Outras := DmAuxiliar.CdsRegistros.FieldByName
          ('OUTRAS').AsFloat;
        wregistro61.Aliquota := DmAuxiliar.CdsRegistros.FieldByName
          ('ALIQUOTA').AsFloat;
...
begin
  ...
  begin
    ...
    begin
      ...
      with DmAcBr.ACBrSintegra1 do
      begin
        wregistro61R.MesAno := DmAuxiliar.CdsRegistros.FieldByName
          ('MESANO').AsString;
        wregistro61R.codigo := DmAuxiliar.CdsRegistros.FieldByName
          ('CODIGO').AsString;
        wregistro61R.Qtd := DmAuxiliar.CdsRegistros.FieldByName('QTD').AsFloat;
        wregistro61R.Valor := DmAuxiliar.CdsRegistros.FieldByName
          ('VALOR').AsFloat;
        wregistro61R.BasedeCalculo := DmAuxiliar.CdsRegistros.FieldByName
          ('BASEDECALCULO').AsFloat;
        wregistro61R.Aliquota := DmAuxiliar.CdsRegistros.FieldByName
          ('ALIQUOTA').AsFloat;
...
begin
  ...
  begin
    ...
    begin
      ...
      with DmAcBr.ACBrSintegra1 do
      begin
        wregistro70.CPFCNPJ := DmAuxiliar.CdsRegistros.FieldByName
          ('CPFCNPJ').AsString;
        wregistro70.Inscricao := DmAuxiliar.CdsRegistros.FieldByName
          ('INSCRICAO').AsString;
        wregistro70.DataDocumento := DmAuxiliar.CdsRegistros.FieldByName
          ('DATADOCUMENTO').AsDateTime;
        wregistro70.UF := DmAuxiliar.CdsRegistros.FieldByName('UF').AsString;
        wregistro70.modelo := DmAuxiliar.CdsRegistros.FieldByName
          ('MODELO').AsString;
        wregistro70.serie := DmAuxiliar.CdsRegistros.FieldByName
          ('SERIE').AsString;
        wregistro70.SubSerie := DmAuxiliar.CdsRegistros.FieldByName
          ('SUBSERIE').AsString;
        wregistro70.Numero := DmAuxiliar.CdsRegistros.FieldByName
          ('NUMERO').AsString;
        wregistro70.CFOP := DmAuxiliar.CdsRegistros.FieldByName('CFOP')
          .AsString;
        wregistro70.ValorContabil := DmAuxiliar.CdsRegistros.FieldByName
          ('VALORCONTABIL').AsFloat;
        wregistro70.BasedeCalculo := DmAuxiliar.CdsRegistros.FieldByName
          ('BASEDECALCULO').AsFloat;
        wregistro70.ICMS := DmAuxiliar.CdsRegistros.FieldByName('ICMS').AsFloat;
        wregistro70.Isentas := DmAuxiliar.CdsRegistros.FieldByName
          ('ISENTAS').AsFloat;
        wregistro70.Outras := DmAuxiliar.CdsRegistros.FieldByName
          ('OUTRAS').AsFloat;
        wregistro70.CifFobOutros := DmAuxiliar.CdsRegistros.FieldByName
          ('CIFFOBOUTROS').AsString;
        wregistro70.Situacao := DmAuxiliar.CdsRegistros.FieldByName
          ('SITUACAO').AsString;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        with DmAcBr.ACBrSintegra1 do
        begin
          wregistro74.Data := DmAuxiliar.CdsRegistros.FieldByName('DATA')
            .AsDateTime;
          wregistro74.codigo := DmAuxiliar.CdsRegistros.FieldByName
            ('CODIGO').AsString;
          wregistro74.Quantidade := DmAuxiliar.CdsRegistros.FieldByName
            ('QUANTIDADE').AsFloat;
          wregistro74.ValorProduto := DmAuxiliar.CdsRegistros.FieldByName
            ('VALORPRODUTO').AsFloat;
          wregistro74.CodigoPosse := DmAuxiliar.CdsRegistros.FieldByName
            ('CODIGOPOSSE').AsString;
          wregistro74.CNPJPossuidor := DmAuxiliar.CdsRegistros.FieldByName
            ('CNPJPOSSUIDOR').AsString;
          wregistro74.InscricaoPossuidor := DmAuxiliar.CdsRegistros.FieldByName
            ('INSCRICAOPOSSUIDOR').AsString;
          wregistro74.UFPossuidor := DmAuxiliar.CdsRegistros.FieldByName
            ('UFPOSSUIDOR').AsString;
        ...
        begin
          ...
          wregistro75.AliquotaICMS := DmAuxiliar.CdsRegistros.FieldByName
            ('ALIQUOTA').AsFloat;
          wregistro75.DataInicial :=
            DmAcBr.ACBrSintegra1.Registro10.DataInicial;
          wregistro75.DataFinal := DmAcBr.ACBrSintegra1.Registro10.DataFinal;
          wregistro75.Descricao := DmAuxiliar.CdsRegistros.FieldByName
            ('DESCRICAO').AsString;
          wregistro75.Unidade := DmAuxiliar.CdsRegistros.FieldByName
...
function TDmWorkCom.CapturaQuantVendidaData(pIDProd: Integer;
  pDataMov: TDateTime): Double;
var
  Q: TSQLQuery;
begin
...
begin
  ...
  begin
    ...
    begin
      while not Eof do
      begin
        aIDPedido := DmWorkCom.RegistraFechamentoMesa
          (CdsMesasAbertas.FieldByName('ID').AsInteger);
        if aIDPedido > 0 then
        begin
          ...
          begin
            // captura os dados do cliente
            DmClientes.Dados_Contato
              (DmWorkCom.CdsPedidos.FieldByName('IDCLIENTE').AsInteger);
            with ClienteCupom do
            begin
              ID := DmWorkCom.CdsConfigCLIENTE_PADRAO.AsInteger;
              Cliente := DmClientes.CdsContatos.FieldByName('NOME').AsString;
            ...
            begin
              ...
              begin
                aMensagem := aMensagem + 'ECF: ' + DmWorkCom.PAFECF.gCaixa +
                  ' - Conferência de Mesa - ' + 'CER nº ' +
                  ZeroEsquerda(CdsPedidos.FieldByName('CER').AsString, 4) +
                  ' - COO nº ' + ZeroEsquerda
                  (CdsPedidos.FieldByName('COO_CONF_MESA').AsString, 6) +
                  #13 + #10;
              end
              else
              begin
                aNumMesa := RetornaNumeroMesa(CdsPedidos.FieldByName('ID_MESA')
                  .AsInteger);
                aMensagem := 'Consumo da Mesa ' + aNumMesa +
                  ' – SEM EMISSÃO DE CONFERÊNCIA DE MESA' + #13 + #10;
              end;
              // calcula o percentual de carga
              PercCarga :=
                ((CdsPedidos.FieldByName('VALOR_CARGA_TRIB').AsFloat * 100) /
              ...
              begin
                ...
                aCodMinasLegal :=
                  Copy(FiltraNumero
                  (DmWorkCom.CdsEmpresaCNPJ_CPF.AsString), 1, 8);
                aCodMinasLegal := aCodMinasLegal + FormatDateTime('DDMMYYYY',
                  CdsPedidos.FieldByName('DATA_ENTREGA').AsDateTime);
                aCodMinasLegal := aCodMinasLegal +
                  FloatToStr(TruncaValor(CdsPedidos.FieldByName('VALOR_TOTAL')
              ...
              begin
                aMensagem := aMensagem +
                  'ESTABELECIMENTO INCLUIDO NO PROGRAMA DE' + #13 + #10;
                aMensagem := aMensagem +
                  'CONCESSAO DE CREDITOS - LEI Nº 4.159/08' + #13 + #10;
                aMensagem := aMensagem + 'NOTA LEGAL: ICMS: ' +
                  FormatFloat('0.00', PercCarga) + #13 + #10;
              end;
              if DmWorkCom.CdsEmpresaUF.AsString = 'PB' then
              begin
                aCodCupomMania :=
                  Copy(FiltraNumero(DmWorkCom.CdsEmpresaCNPJ_CPF.AsString),
                  1, 14) + ' ';
                aCodCupomMania := aCodCupomMania + FormatDateTime('DDMMYYYY',
                  CdsPedidos.FieldByName('DATA_ENTREGA').AsDateTime) + ' ';
                aCodCupomMania := aCodCupomMania +
              ...
              if CdsPedidos.FieldByName('ID_MESA').AsInteger > 0 then
                AlteraStatusMesa(CdsPedidos.FieldByName('ID_MESA')
                  .AsInteger, 3);
              PAFECF.AtualizaINI(false);
            end;
          end;
      ...
      begin
        ...
        begin
          ...
          begin
            if (CdsPedidos.FieldByName('COO').AsInteger = 0) then
            begin
              if DmClientes.Dados_Contato(CdsPedidos.FieldByName('IDCLIENTE')
                .AsInteger) then
              begin
                with ClienteCupom do
                begin
                  ID := DmWorkCom.CdsConfigCLIENTE_PADRAO.AsInteger;
                  Cliente := DmClientes.CdsContatos.FieldByName('NOME')
                    .AsString;
                  Endereco := DmClientes.CdsContatos.FieldByName
                    ('END_ENTREGA').AsString;
                  Numero := DmClientes.CdsContatos.FieldByName
                  ...
                  Cep := DmClientes.CdsContatos.FieldByName
                    ('CEP_END_ENTREGA').AsString;
                  UF := DmClientes.CdsContatos.FieldByName
                    ('UF_END_ENT').AsString;
                  CPF := DmClientes.CdsContatos.FieldByName('CNPJ_CPF')
                    .AsString;
                  Identidade := DmClientes.CdsContatos.FieldByName
                    ('INSC_RG').AsString;
                  Placa := '';
              ...
              begin
                ...
                begin
                  ...
                  if (CdsPedidos.FieldByName('NUMERO_DOC').AsString) <> '' then
                  begin
                    DmPosto.AlteraStatusAbastecimento
                      (CdsPedidoItens.FieldByName('ID_ABASTECIMENTO').AsInteger,
              ...
              DmWorkCom.PAFECF.RegistraFormaPagtoCupom
                (DmWorkCom.PAFECF.gDINHEIRO,
                TruncaValor(CdsPedidos.FieldByName('VALOR_TOTAL').AsFloat,
                DmWorkCom.PAFECF.gDecimais));
              aMensagem := '';
              ...
              begin
                aMensagem := aMensagem + 'NF: ' + CdsPedidos.FieldByName
                  ('NUMERO_DOC').AsString + #13 + #10;
              end;
              sCdsInfoCupomPosto.EmptyDataSet;
              RetornaInformacaoAbastecimeneto(sCdsInfoCupomPosto,
                CdsPedidos.FieldByName('ID').AsInteger, false);
              sCdsInfoCupomPosto.First;
              while not sCdsInfoCupomPosto.Eof do
              begin
                aMensagem := aMensagem + '#CF:B' +
                  ZeroEsquerda(sCdsInfoCupomPosto.FieldByName('Bico').AsString,
                  2) + ' ' + 'EI' +
                  ZeroEsquerda(FormatFloat('0.000',
                  sCdsInfoCupomPosto.FieldByName('EI').AsFloat), 11) + ' ' +
                  'EF' + ZeroEsquerda(FormatFloat('0.000',
                  sCdsInfoCupomPosto.FieldByName('EF').AsFloat), 11) + ' ' + 'V'
                  + FormatFloat('0.000', sCdsInfoCupomPosto.FieldByName('EF')
                  .AsFloat - sCdsInfoCupomPosto.FieldByName('EI').AsFloat) + 'A'
                  + #13 + #10;
                sCdsInfoCupomPosto.Next;
              end;
              // exibe carga tributaria
              aMensagem := aMensagem + 'Val Aprox trib. Fed. R$' +
                FormatFloat(PAFECF.gMascaraDec,
                CdsPedidos.FieldByName('VAL_CARGA_FED').AsFloat) + ' Est. R$' +
                FormatFloat(PAFECF.gMascaraDec,
                CdsPedidos.FieldByName('VAL_CARGA_EST').AsFloat) + ' Mun. R$' +
                FormatFloat(PAFECF.gMascaraDec,
              ...
              if Dados_PedidoClassificacao(1,
                CdsPedidos.FieldByName('ID_CLASSIFICACAO').AsInteger) then
              begin
                // baixa estoque
                if CdsPedidoClass.FieldByName('BAIXAR_ESTOQUE').AsString = 'S'
                then
              ...
              while not CdsPedidoFP.Eof do
              begin
                if CdsFormaPag.Locate('ID',
                  CdsPedidoFP.FieldByName('ID_FORMAPAG_CUPOM').AsInteger, [])
                then
                begin
                  if (CdsFormaPag.FieldByName('GERAR_CONTA').AsString = 'S')
                  then
                  begin
                    aValorConta := CdsPedidoFP.FieldByName('VALOR').AsFloat /
                      CdsFormaPag.FieldByName('QUANT_PAGTOS').AsInteger;
                    Gerar_Contas(0, CdsPedidos.FieldByName('ID').AsInteger, 0,
                  ...
                  if (CdsFormaPag.FieldByName('MOVIMENO_CAIXA').AsString = 'S')
                  then
                  begin
                    ...
                    begin
                      ...
                      CdsCaixaMov.FieldByName('VALOR').AsCurrency :=
                        CdsPedidoFP.FieldByName('VALOR').AsFloat;
                      CdsCaixaMov.FieldByName('IDPEDIDO').AsInteger :=
...
begin
  ...
  try
    ...
    begin
      ...
      while not Q.Eof do
      begin
        aEncerranteAnterior := PAFECF.RetornaUltimoEncerranteBico
          (Q.FieldByName('bico_concentrador').AsString);
        aEncerranteAnterior := aEncerranteAnterior +
          Q.FieldByName('quantidade').AsFloat;
        // grava o pedido do abastecimento
        DmPosto.RegistraAbastecimentos(Q.FieldByName('bico_concentrador')
          .AsString, FormatDateTime('dd/mm/yyyy', Now),
          FormatDateTime('hh:mm:ss', Now), aEncerranteAnterior,
          Q.FieldByName('quantidade').AsFloat, Q.FieldByName('valor').AsFloat,
          (Q.FieldByName('quantidade').AsFloat) * Q.FieldByName('valor')
          .AsFloat, false, true, NumeroDoc);
        // grava o encerrante no bico
        DmPosto.GravaEncerranteBico(Q.FieldByName('bico_concentrador').AsString,
          aEncerranteAnterior);
        Q.Next;
      end;
    end;
...
procedure TDmWorkCom.VerificaAlteraStatusAbastecimento(pIDPedido: Integer;
  Cancelamento: Boolean = false);
var
  Q: TSQLQuery;
begin
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          DmPosto.AlteraStatusAbastecimento(Q.FieldByName('id_abastecimento')
            .AsInteger, 3, 0, Q.FieldByName('numero_doc').AsString,
...
begin
  ...
  begin
    ...
    begin
      ...
      with DmAcBr.ACBrSintegra1 do
      begin
        wregistro60I.Emissao := DmAuxiliar.CdsRegistros.FieldByName('EMISSAO')
          .AsDateTime;
        wregistro60I.NumSerie := DmAuxiliar.CdsRegistros.FieldByName
          ('NUMSERIE').AsString;
        wregistro60I.ModeloDoc := DmAuxiliar.CdsRegistros.FieldByName
          ('MODELODOC').AsString;
        wregistro60I.Cupom := DmAuxiliar.CdsRegAuxiliar1.FieldByName
          ('CUPOM').AsString;
        wregistro60I.Item := DmAuxiliar.CdsRegAuxiliar1.FieldByName('ITEM')
          .AsInteger;
        wregistro60I.codigo := DmAuxiliar.CdsRegAuxiliar1.FieldByName
          ('CODIGO').AsString;
        wregistro60I.Quantidade := DmAuxiliar.CdsRegAuxiliar1.FieldByName
          ('QUANTIDADE').AsFloat;
        wregistro60I.Valor := DmAuxiliar.CdsRegAuxiliar1.FieldByName
          ('VALOR').AsFloat;
        wregistro60I.BasedeCalculo := DmAuxiliar.CdsRegAuxiliar1.FieldByName
          ('BASEDECALCULO').AsFloat;
        wregistro60I.StAliquota := DmAuxiliar.CdsRegAuxiliar1.FieldByName
          ('STALIQUOTA').AsString;
        wregistro60I.ValorIcms := DmAuxiliar.CdsRegAuxiliar1.FieldByName
          ('VALORICMS').AsFloat;
...
procedure TDmWorkCom.GerarRegistro60R(pDatai, pDataf: TDateTime;
  NumSerie: String);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      with DmAcBr.ACBrSintegra1 do
      begin
        wregistro60R.MesAno := DmAuxiliar.CdsRegAuxiliar1.FieldByName
          ('MESANO').AsString;
        wregistro60R.codigo := DmAuxiliar.CdsRegAuxiliar1.FieldByName
          ('CODIGO').AsString;
        wregistro60R.Qtd := DmAuxiliar.CdsRegAuxiliar1.FieldByName
          ('QUANTIDADE').AsFloat;
        wregistro60R.Valor := DmAuxiliar.CdsRegAuxiliar1.FieldByName
          ('VALOR').AsFloat;
        wregistro60R.BasedeCalculo := DmAuxiliar.CdsRegAuxiliar1.FieldByName
          ('BASEDECALCULO').AsFloat;
        wregistro60R.Aliquota := DmAuxiliar.CdsRegAuxiliar1.FieldByName
          ('STALIQUOTA').AsString;
...
function TDmWorkCom.VendasIdentificadas(fNomeArquivo, CNPJCPF: String;
  Mes, Ano: Integer): Boolean;
var
...
begin
  ...
  Linha := 'Z2';
  Linha := Linha + ZeroEsquerda
    (FiltraNumero(CdsDadosTecnicos.FieldByName('CNPJ').AsString), 14);
  if Trim(FiltraNumero(CdsDadosTecnicos.FieldByName('INSCEST').AsString)) <> ''
  then
    Linha := Linha + ZeroEsquerda(CdsDadosTecnicos.FieldByName('INSCEST')
      .AsString, 14)
  else
    Linha := Linha + ZeroEsquerda('0', 14);
  Linha := Linha + ZeroEsquerda(CdsDadosTecnicos.FieldByName('INSCMUN')
    .AsString, 14);
  Linha := Linha + AlinhaTXT(Copy(CdsDadosTecnicos.FieldByName('EMPRESA')
    .AsString, 1, 50), 1, 50);
  writeLn(sArquivo, Linha);
...
begin
  ...
  // Dados_Empresa(sIDEmpresa);
  Linha := 'Z3';
  Linha := Linha + AlinhaTXT(CdsDadosTecnicos.FieldByName('NR_LAUDO_TECNICO')
    .AsString, 1, 10);
  Linha := Linha + AlinhaTXT(Copy(CdsDadosTecnicos.FieldByName('DENOMINACAO')
    .AsString, 1, 50), 1, 50);
  Linha := Linha + AlinhaTXT(CdsDadosTecnicos.FieldByName('VERSAO')
    .AsString, 1, 10);
  writeLn(sArquivo, Linha);

end;

procedure TDmWorkCom.CriaRegistroZ4(var TotalReg: Integer;
  pDatai, pDataf: TDateTime; pCNPJCPF: String);
var
  Linha: String;
  Q: TSQLQuery;
begin
    ...
    try
      ...
      begin
        ...
        begin
          Linha := 'Z4';
          Linha := Linha + ZeroEsquerda
            (FiltraNumero(Q.FieldByName('cnpjcpf').AsString), 14);
          Linha := Linha + ZeroEsquerda
            (FiltraNumero(FormatFloat('0.00', Q.FieldByName('total')
            .AsFloat)), 14);
          Linha := Linha + FormatDateTime('YYYYMMDD', pDatai);
          Linha := Linha + FormatDateTime('YYYYMMDD', pDataf);
...
begin
  ...
  if Somar then
    CdsRegistroA2.FieldByName('VALOR').AsFloat :=
      CdsRegistroA2.FieldByName('VALOR').AsFloat + Valor
  else
    CdsRegistroA2.FieldByName('VALOR').AsFloat :=
      CdsRegistroA2.FieldByName('VALOR').AsFloat - Valor;
...
begin
  ...
  try
    ...
    Q.SQL.Add('and (p.chave_nfe is not null or p.chave_nfe <> ' +
      QuotedStr('') + ')');
    Q.Open;
    Result := not Q.IsEmpty;
    Q.Close;
  ...
  begin
    try
      ...
      begin
        ...
        Result := true;
        //
        DmWorkCom.GravaChaveNFEPedido(pChaveNFe, vProtocolo, vReciboRet, '',
          DmAcBr.ACBrNFe1.NotasFiscais.Items[0].NomeArq,
          IntToStr(DmAcBr.ACBrNFe1.WebServices.Retorno.NFeRetorno.ProtNFe.Items
          [0].cStat), DmWorkCom.sFormas, pIDPedido, Now, Now);
      end;
    except
      on Exc: Exception do
      begin
        MessageDlg('Erro ao tentar consultar a NFCe:' + #13 + #10 + Exc.message,
          mtError, [mbOK], 0);
        Result := false;
      end;
    end;
...
{$ENDIF}
  ...
  if Trim(FiltraNumero(sNFSeCodCidade)) <> '' then
    DmAcBr.ACBrNFSe1.configuracoes.Geral.CodigoMunicipio :=
      StrToIntDef(sNFSeCodCidade, 0);
  ...
  DmAcBr.ACBrNFSe1.configuracoes.WebServices.Ambiente :=
    StrToTpAmb(OK, IntToStr(sNFSeTipoAmb + 1));
...
begin
  ...
  begin
    ...
    //rondado ativar controle porta, acerto de erro de pos impressao
    //DmAcbr.ACBrPosPrinter1.ControlePorta := true;
    ...
    if FileExists(ExtractFilePath(Application.ExeName) + 'imagens\Logo_Sat.jpg')
    then
  ...
  begin
    ...
    DmAcBr.ACBrSATExtratoFortes1.Site := 'Fone:' + CdsEmpresaTELEFONE.Text +
      ' - ' + CdsEmpresaSITE.Text;
    if FileExists(ExtractFilePath(Application.ExeName) + 'imagens\Logo_Sat.jpg')
    then
      DmAcBr.ACBrSATExtratoFortes1.PictureLogo.LoadFromFile
        (ExtractFilePath(Application.ExeName) + 'imagens\Logo_Sat.jpg');
    try
...
begin
  ...
  if not DirectoryExists(ExtractFilePath(Application.ExeName) + 'SAT\CFeVenda\')
  then
    CreateDir(ExtractFilePath(Application.ExeName) + 'SAT\CFeVenda\');
  if not DirectoryExists(ExtractFilePath(Application.ExeName) + 'SAT\CFeCanc\')
  then
    CreateDir(ExtractFilePath(Application.ExeName) + 'SAT\CFeCanc\');
  if not DirectoryExists(ExtractFilePath(Application.ExeName) + 'SAT\Envio\')
  then
  ...
  begin
    ...
    //rondado acertar o tipode Ambiente SAT
    if sAMBIENTE_SAT = 0 then
    begin
      //producao
      sAMBIENTE_SAT := 1;
    end
  ...
  //rondado modifical original>>  modelo := TACBrSATModelo(sSAT_MODELO);
    ...
    Config.emit_cRegTribISSQN :=
      TpcnRegTribISSQN(CdsEmpresaCRT_ISSQN.AsInteger);
    ...
    ConfigArquivos.PastaCFeVenda := ExtractFilePath(Application.ExeName) +
      'SAT\CFeVenda\';
    ConfigArquivos.PastaCFeCancelamento := ExtractFilePath(Application.ExeName)
      + 'SAT\CFeCanc\';
    ConfigArquivos.PastaEnvio := ExtractFilePath(Application.ExeName) +
      'SAT\Envio\';
    ...
    begin
      ...
      begin
        ...
        if (Trim(CdsPedidos.FieldByName('CNPJCPF').AsString) <> '') and
          (Trim(CdsPedidos.FieldByName('CNPJCPF').AsString) <> '11111111111')
        then
        begin
          Dest.CNPJCPF := CdsPedidos.FieldByName('CNPJCPF').AsString;
        ...
        begin
          Dest.xNome := 'CONSUMIDOR NÃO INFORMADO';
        end;
        ...
        begin
          ...
          begin
            ...
            //rondado verificar método para testar o ean
           // Prod.cEAN := CdsPedidoItens.FieldByName('EAN13').AsString;
            ...
            begin
              ...
              Imposto.ICMS.pICMS := CdsPedidoItens.FieldByName
                ('ALIQ_ICMS').AsFloat;
            end


            //RONDADO, SE FOR SIMPLES NACIONAL
            else
            begin
              case StrToInt(Copy(DmWorkCom.CdsPedidoItensCST.AsString, 2, 2)) of
                0:
                //RONDADO ORIGINAL csosn102
            ...
            // Pis
            // if (CdsPedidoItens.FieldByName('VALOR_PIS').AsFloat > 0) and ORIGINAL
            ...
            // Cofins
            //if (CdsPedidoItens.FieldByName('VALOR_COFINS').AsFloat > 0) and ORIGINAL
            if (CdsPedidoItens.FieldByName('VALOR_COFINS').AsFloat > 0) and
              (Trim(FiltraNumero(CdsPedidoItens.FieldByName('CST_COFINS')
              .AsString)) <> '') then
            begin
              case StrToInt
                (FiltraNumero(CdsPedidoItens.FieldByName('CST_COFINS')
                .AsString)) of
                01:
              ...
              Imposto.COFINS.vBC := CdsPedidoItens.FieldByName
                ('BASE_COFINS').AsFloat;
              Imposto.COFINS.pCOFINS := CdsPedidoItens.FieldByName
                ('ALIQ_COFINS').AsFloat;
              Imposto.COFINS.vCOFINS := CdsPedidoItens.FieldByName
...
procedure TDmWorkCom.GerarXMLEstoque(pDatai, pDataf: TDateTime;
  pNomeArq: String);
var
  aEstoque: Double;
begin
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          codigo.Numero := DmProdutos.CdsProdutos2ID.AsString;
...
procedure TDmWorkCom.GerarXMLReducao(pDatai, pDataf: TDateTime;
  pNomeArq: String);
var
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            begin
              Identificacao := Cds60A.FieldByName('ALIQUOTA_TRIB').AsString +
                ZeroEsquerda(IntToStr(Contador), 2) + '    ';
              aTributacao := 3;
              aAliquota := 0;
            end
            ...
            begin
              Identificacao := ZeroEsquerda(IntToStr(Contador), 2) + 'T' +
                ZeroEsquerda(Copy(Cds60A.FieldByName('ALIQUOTA_TRIB')
                .AsString, 1, 2), 4);
              aTributacao := 2;
              aAliquota := StrToFloat(Cds60A.FieldByName('ALIQUOTA_TRIB')
                .AsString) / 100;
            end;
            ...
            try
              ...
              Q.Params.ParamByName('pdataemis').AsDate :=
                Cds60MDATA_MOVIMENTO.AsDateTime;
              ...
              begin
                ...
                begin
                  codigo.Tipo := tpcProprio;
                  codigo.Numero := Q.FieldByName('idproduto').AsString;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          if (ZeroEsquerda(Le_Linha_Arquivo(cLinhaArquivo, '|', 3), 3)
            = PAFECF.gCaixa) or
            (ZeroEsquerda(Le_Linha_Arquivo(cLinhaArquivo, '|', 3), 3) = '000')
          then
          begin
            ...
            begin
              ...
              aOK := true;
              CdsPedidos.Append;
              CdsPedidos.FieldByName('NUMDAV').AsString :=
                Le_Linha_Arquivo(cLinhaArquivo, '|', 4);
              CdsPedidos.FieldByName('DATA_PEDIDO').AsDateTime :=
                StrToDateTime(Le_Linha_Arquivo(cLinhaArquivo, '|', 5));
              CdsPedidos.FieldByName('HORA_PEDIDO').AsDateTime :=
                StrToDateTime(Le_Linha_Arquivo(cLinhaArquivo, '|', 6));
              CdsPedidos.FieldByName('DATA_ENTREGA').AsDateTime := Now;
              CdsPedidos.FieldByName('HORA_ENTREGA').AsDateTime := Now;
              CdsPedidos.FieldByName('CONFIRMADA').AsString := 'S';
              CdsPedidos.FieldByName('ENTREGUE').AsString := 'S';
              CdsPedidos.FieldByName('VALOR').AsFloat :=
                StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|',
                7), '.', ','));
              CdsPedidos.FieldByName('DESCONTO').AsFloat :=
                StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|',
                8), '.', ','));
              CdsPedidos.FieldByName('ACRESCIMO').AsFloat :=
                StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|',
              ...
              CdsPedidos.FieldByName('VALOR_TOTAL').AsFloat :=
                ...
                11), '.', ','));
              CdsPedidos.FieldByName('END_ENTREGA').AsString :=
                Le_Linha_Arquivo(cLinhaArquivo, '|', 12);
              CdsPedidos.FieldByName('NUM_END_ENTREGA').AsString :=
                Le_Linha_Arquivo(cLinhaArquivo, '|', 13);
              CdsPedidos.FieldByName('COMPL_END_ENTREGA').AsString :=
                Le_Linha_Arquivo(cLinhaArquivo, '|', 14);
              CdsPedidos.FieldByName('BAIRRO_END_ENTREGA').AsString :=
                Le_Linha_Arquivo(cLinhaArquivo, '|', 15);
              CdsPedidos.FieldByName('CID_END_ENTREGA').AsString :=
                Le_Linha_Arquivo(cLinhaArquivo, '|', 16);
              CdsPedidos.FieldByName('UF_END_ENTREGA').AsString :=
                Le_Linha_Arquivo(cLinhaArquivo, '|', 17);
              CdsPedidos.FieldByName('CEP_END_ENTREGA').AsString :=
                Le_Linha_Arquivo(cLinhaArquivo, '|', 18);
              CdsPedidos.FieldByName('CLIENTE').AsString :=
                Le_Linha_Arquivo(cLinhaArquivo, '|', 19);
              CdsPedidos.FieldByName('CNPJCPF').AsString :=
                Le_Linha_Arquivo(cLinhaArquivo, '|', 20);
              CdsPedidos.FieldByName('INSCEST').AsString :=
                Le_Linha_Arquivo(cLinhaArquivo, '|', 21);
              CdsPedidos.FieldByName('NATOP').AsString :=
                Le_Linha_Arquivo(cLinhaArquivo, '|', 22);
              ...
              CdsPedidos.FieldByName('VALOR_OUTROS').AsFloat :=
                StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|',
                36), '.', ','));
              CdsPedidos.FieldByName('MODELO').AsString :=
                Le_Linha_Arquivo(cLinhaArquivo, '|', 37);
              CdsPedidos.FieldByName('CAIXA').AsString := PAFECF.gCaixa;
              Grava_Dados(CdsPedidos);
            end
            else
            begin
              MessageDlg('O pedido número: ' + aNumDav +
                ', já foi importado anteriormente. Verifique!', mtWarning,
                [mbOK], 0);
              ApagaArquivo := true;
              Break;
            end;
        ...
        begin
          CdsPedidoItens.Append;
          CdsPedidoItens.FieldByName('IDPRODUTO').AsInteger :=
            StrToInt(Le_Linha_Arquivo(cLinhaArquivo, '|', 3));
          CdsPedidoItens.FieldByName('DESCRICAO').AsString :=
            Le_Linha_Arquivo(cLinhaArquivo, '|', 4);
          CdsPedidoItens.FieldByName('QUANTIDADE').AsFloat :=
            StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|', 5),
            '.', ','));
          CdsPedidoItens.FieldByName('UNIDADE').AsString :=
            Le_Linha_Arquivo(cLinhaArquivo, '|', 6);
          CdsPedidoItens.FieldByName('VALOR').AsFloat :=
            StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|', 7),
            '.', ','));
          CdsPedidoItens.FieldByName('DESCONTO').AsFloat :=
            StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|', 8),
            '.', ','));
          CdsPedidoItens.FieldByName('ACRESCIMO').AsFloat :=
            StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|', 9),
            '.', ','));
          CdsPedidoItens.FieldByName('VALOR_TOTAL').AsFloat :=
            StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|', 10),
            '.', ','));
          CdsPedidoItens.FieldByName('EAN13').AsString :=
            Le_Linha_Arquivo(cLinhaArquivo, '|', 11);
          CdsPedidoItens.FieldByName('CST').AsString :=
            Le_Linha_Arquivo(cLinhaArquivo, '|', 12);
          CdsPedidoItens.FieldByName('CFOP').AsString :=
            Le_Linha_Arquivo(cLinhaArquivo, '|', 13);
          CdsPedidoItens.FieldByName('ALIQ_ICMS').AsFloat :=
            StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|', 14),
            '.', ','));
          CdsPedidoItens.FieldByName('BASE_ICMS').AsFloat :=
            StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|', 15),
            '.', ','));
          CdsPedidoItens.FieldByName('VALOR_ICMS').AsFloat :=
            StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|', 16),
            '.', ','));
          CdsPedidoItens.FieldByName('REDUCAO_BASE').AsFloat :=
            StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|', 17),
            '.', ','));
          CdsPedidoItens.FieldByName('MVA').AsFloat :=
            StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|', 18),
            '.', ','));
          CdsPedidoItens.FieldByName('ALIQ_ICMS_ST').AsFloat :=
            StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|', 19),
            '.', ','));
          CdsPedidoItens.FieldByName('BASE_ICMS_ST').AsFloat :=
            StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|', 20),
            '.', ','));
          CdsPedidoItens.FieldByName('VALOR_ICMS_ST').AsFloat :=
            StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|', 21),
            '.', ','));
          CdsPedidoItens.FieldByName('CST_PIS').AsString :=
            Le_Linha_Arquivo(cLinhaArquivo, '|', 22);
          CdsPedidoItens.FieldByName('ALIQ_PIS').AsFloat :=
            StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|', 23),
            '.', ','));
          CdsPedidoItens.FieldByName('BASE_PIS').AsFloat :=
            StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|', 24),
            '.', ','));
          CdsPedidoItens.FieldByName('VALOR_PIS').AsFloat :=
            StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|', 25),
            '.', ','));
          CdsPedidoItens.FieldByName('CST_COFINS').AsString :=
            Le_Linha_Arquivo(cLinhaArquivo, '|', 26);
          CdsPedidoItens.FieldByName('ALIQ_COFINS').AsFloat :=
            StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|', 27),
            '.', ','));
          CdsPedidoItens.FieldByName('BASE_COFINS').AsFloat :=
            StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|', 28),
            '.', ','));
          CdsPedidoItens.FieldByName('VALOR_COFINS').AsFloat :=
            StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|', 29),
            '.', ','));
          CdsPedidoItens.FieldByName('CST_IPI').AsString :=
            Le_Linha_Arquivo(cLinhaArquivo, '|', 30);
          CdsPedidoItens.FieldByName('ALIQ_IPI').AsFloat :=
            StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|', 31),
            '.', ','));
          CdsPedidoItens.FieldByName('BASE_IPI').AsFloat :=
            StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|', 32),
            '.', ','));
          CdsPedidoItens.FieldByName('VALOR_IPI').AsFloat :=
            StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|', 33),
            '.', ','));
          Grava_Dados(CdsPedidoItens);
        ...
        if (aOK) and (UpperCase(Copy(cLinhaArquivo, 2, 1)) = 'P') then
        // forma de pagamento
        begin
          CdsPedidoFP.Append;
          CdsPedidoFP.FieldByName('VALOR').AsFloat :=
            StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|', 3),
            '.', ','));
          CdsPedidoFP.FieldByName('TROCO').AsFloat :=
            StrToFloat(AlteraCaracter(Le_Linha_Arquivo(cLinhaArquivo, '|', 4),
            '.', ','));
          CdsPedidoFP.FieldByName('TIPO_DOCUMENTO').AsString :=
            Le_Linha_Arquivo(cLinhaArquivo, '|', 5);
          CdsPedidoFP.FieldByName('DESCRICAO').AsString :=
            Le_Linha_Arquivo(cLinhaArquivo, '|', 6);
          CdsPedidoFP.FieldByName('VINCULAR').AsString :=
            Le_Linha_Arquivo(cLinhaArquivo, '|', 7);
...
var
  ...
  TextoUnicode := #27#69 + ' '+ CdsEmpresaRAZAO_SOCIAL.AsString
                + #27#70#27#53#13#10;
 ...
 /// WritePrinter(Handle, PAnsiChar(TextoAnsiString), Length(TextoAnsiString), CaracteresImpressos);
  ...
  WritePrinter(Handle, PAnsiChar(TextoAnsiString), Length(TextoAnsiString), CaracteresImpressos);
      ...
      TextoUnicode := ('RELATORIO DETALHADO DO PEDIDO ')
                +#13#10;
  TextoAnsiString := AnsiString(TextoUnicode);
  WritePrinter(Handle, PAnsiChar(TextoAnsiString), Length(TextoAnsiString), CaracteresImpressos);
  ...
  WritePrinter(Handle, PAnsiChar(TextoAnsiString), Length(TextoAnsiString), CaracteresImpressos);
      ...
      begin
        ...
        begin
          Inc(ContItens);
          idescricao:= TrocaCaracterEspecial(CdsPedidoItensDESCRICAO.AsString, false);
...
begin
  ...
  cConteudoArquivo := '';
  cConteudoArquivo := cConteudoArquivo + CdsPedidos.FieldByName('NUMDAV')
    .AsString + '|'; // NRCUPOM
  cConteudoArquivo := cConteudoArquivo + '40' + '|'; // SERIE
  cConteudoArquivo := cConteudoArquivo +
    FiltraNumero(CdsEmpresa.FieldByName('CNPJ_CPF').AsString) + '|';
  // CNPJ EMITENTE
  cConteudoArquivo := cConteudoArquivo + FormatDateTime('dd/mm/yyyy',
    CdsPedidos.FieldByName('DATA_ENTREGA').AsDateTime) + '|'; // DATA
  ...
  cConteudoArquivo := cConteudoArquivo + '|'; // espaço em branco
  cConteudoArquivo := cConteudoArquivo +
    FiltraNumero(CdsPedidos.FieldByName('CNPJCPF').AsString) + '|';
  // CNPJ DESTINO|
  //
  if not DirectoryExists(ExtractFilePath(Application.ExeName) + '\Retorno\')
  then
...
begin
  try
    ...
    Q.Close;
    Q.SQL.Add(
      'select numero_doc from retorna_numero_nfe(:pIDEMpresa,:pModelo,:pAmbiente)');
    Q.Params.ParamByName('pIDEMpresa').AsInteger := sIDEmpresa;
